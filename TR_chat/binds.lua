...
function BindManager:drawBindsList()
    for i = 1, guiInfo.maxBinds do
        if self.binds[i] then
            ...
            dxDrawText(string.format("Bind #%d", i), guiInfo.x + 20/zoom, guiInfo.y + 50/zoom + (i-1) * 40/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 90/zoom + (i-1) * 40/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.title, "left", "center")
            dxDrawText(string.format("Klawisz: %s", key), guiInfo.x + 130/zoom, guiInfo.y + 50/zoom + (i-1) * 40/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 90/zoom + (i-1) * 40/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.title, "left", "center")
...
function BindManager:renderProperty()
    ...
    if self.property.type == "detectKey" then
        ...
        dxDrawText(string.format("%s", pressed and string.format("%s +", pressed) or "Odczytywanie..."), self.property.x, self.property.y + 50/zoom, self.property.x + self.property.w, self.property.y + 40/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", false, false, true)
        dxDrawText("Wybierz i naciśnij na klawiaturze przycisk, pod który chcesz dodać binda. Aby anulować naciśnij ESCAPE.", self.property.x + 10/zoom, self.property.y + 80/zoom, self.property.x + self.property.w - 10/zoom, self.property.y + 150/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "top", true, true, true)
    end
end
    ...
    if arg[1] == "left" and arg[2] == "down" then
        if guiInfo.maxBinds > #self.binds then
            if self:isMouseInPosition(guiInfo.x + (guiInfo.w - 32/zoom)/2, guiInfo.y + 54/zoom + #self.binds * 40/zoom, 32/zoom, 32/zoom) and not self.property then
                ...
                --     return
                -- end
                self:openProperty((sx - 300/zoom)/2, (sy - 150/zoom)/2, 300/zoom, 150/zoom, "Wybór klawisza", "detectKey")
            end
        end
...
function BindManager:canUseBind()
    ...
    if self.slowBind then
        if (getTickCount() - self.slowBind)/5000 < 1 then
            exports.TR_noti:create("Musisz chwilę odczekać przed użyciem kolejnego binda.", "info")
            return false
        end
    end
...
function BindManager:canAddBind(key)
    for i, v in pairs(self.binds) do
        if v.key == key then
            exports.TR_noti:create("Nie możesz dodać drugiego binda pod ten sam klawisz.", "error")
            return false
        end
    end
    return true
