...
function Scoreboard:open(...)
  ...
  self.icons.gmod = dxCreateTexture("files/images/gmod.png", "argb", true, "clamp")
  -- #5ce76a
  ...
  self.searchEdit = exports.TR_dx:createEdit(settings.x + 300/zoom, settings.y + 25/zoom, 450/zoom, 40/zoom, "Szukaj gracza", false, self.searchIcon)
    ...
    if v ~= localPlayer then
      ...
      if id and getElementData(v, "characterData") then
        ...
        if img then
          ...
          --if data.org == '-' then r,g,b = 43, 196, 255 end
        end
      end
    end
  end
...
function Scoreboard:getPlayerJob(plr, hasMask)
  ...
  if job then
    ...
    if job == "mechanic" then return "LAKIERNIK" end
    return "Dorywcza"
  end
  return "-"
end
...
function Scoreboard:getOnlyUpper(text)
  if text == "Just News" then return "NEWS" end
  local upper = ""
...
function Scoreboard:render()
  self:drawBackground(settings.x, settings.y, settings.w, settings.h, tocolor(23,25,31, 255), 25)
  dxDrawRectangle(settings.x, settings.y + 90/zoom, settings.w, 3/zoom, tocolor(43, 196, 255, 255))
  dxDrawImage(settings.x + 25/zoom, settings.y + 8/zoom, 180/zoom, 72/zoom, self.logo, 0, 0, 0, tocolor(255, 255, 255, 255))

  dxDrawText("Ilość online", settings.x + settings.w - 130/zoom, settings.y + 23/zoom, settings.x + settings.w - 30/zoom, settings.y + 23/zoom, tocolor(200, 200, 200, 200), 1/zoom, settings.fonts.online, "center", "top")
  dxDrawText(string.format("%d/%d", self.playersCount, settings.maxPlayers), settings.x + settings.w - 130/zoom, settings.y + 35/zoom, settings.x + settings.w - 30/zoom, settings.y + 80/zoom, tocolor(43, 196, 255, 220), 1/zoom, settings.fonts.onlineCount, "center", "top")
...
function Scoreboard:drawPlayers()
  ...
  if self:isMouseInPosition(settings.x + 110/zoom, settings.y + 106/zoom, 110/zoom, 30/zoom) or settings.sortBy == "name" then
    dxDrawText("Nick gracza", settings.x + 120/zoom, settings.y + 110/zoom, settings.x + 380/zoom, settings.y + 140/zoom, tocolor(255, 255, 255, 230), 1/zoom, settings.fonts.data, "left", "top", true, true)
  else
    dxDrawText("Nick gracza", settings.x + 120/zoom, settings.y + 110/zoom, settings.x + 380/zoom, settings.y + 140/zoom, tocolor(220, 220, 220, 200), 1/zoom, settings.fonts.data, "left", "top", true, true)
  end
  if self:isMouseInPosition(settings.x + 440/zoom, settings.y + 106/zoom, 60/zoom, 30/zoom) or settings.sortBy == "points" then
    dxDrawText("Dośw.", settings.x + 430/zoom, settings.y + 110/zoom, settings.x + 510/zoom, settings.y + 140/zoom, tocolor(255, 255, 255, 230), 1/zoom, settings.fonts.data, "center", "top", true)
  else
    dxDrawText("Dośw.", settings.x + 430/zoom, settings.y + 110/zoom, settings.x + 510/zoom, settings.y + 140/zoom, tocolor(220, 220, 220, 200), 1/zoom, settings.fonts.data, "center", "top", true)
  end
  if self:isMouseInPosition(settings.x + 505/zoom, settings.y + 106/zoom, 115/zoom, 30/zoom) or settings.sortBy == "org" then
    dxDrawText("Organizacja", settings.x + 515/zoom, settings.y + 110/zoom, settings.x + settings.w - 160/zoom, settings.y + 140/zoom, tocolor(255, 255, 255, 230), 1/zoom, settings.fonts.data, "left", "top", true)
  else
    dxDrawText("Organizacja", settings.x + 515/zoom, settings.y + 110/zoom, settings.x + settings.w - 160/zoom, settings.y + 140/zoom, tocolor(220, 220, 220, 200), 1/zoom, settings.fonts.data, "left", "top", true)
  end
  if self:isMouseInPosition(settings.x + settings.w - 240/zoom, settings.y + 106/zoom, 80/zoom, 30/zoom) or settings.sortBy == "job" then
    dxDrawText("Praca", settings.x + settings.w - 240/zoom, settings.y + 110/zoom, settings.x + settings.w - 160/zoom, settings.y + 140/zoom, tocolor(255, 255, 255, 230), 1/zoom, settings.fonts.data, "center", "top", true)
  else
    dxDrawText("Praca", settings.x + settings.w - 240/zoom, settings.y + 110/zoom, settings.x + settings.w - 160/zoom, settings.y + 140/zoom, tocolor(220, 220, 220, 200), 1/zoom, settings.fonts.data, "center", "top", true)
  end
  ...
  for i = 1, settings.displayMax do
    if self.playersList[i + self.scroll] then
      ...
      if isElement(v.plr) then
        local oType = getElementData(v.plr, 'characterOrgType')
        local r, g, b = 43, 196, 255
        ...
        dxDrawText(v.org, settings.x + 515/zoom, y, settings.x + settings.w - 250/zoom, y + 40/zoom, tocolor(r, g, b, 200), 1/zoom, settings.fonts.data, "left", "center", true, true)
        ...
        if self:isMouseInPosition(settings.x + settings.w - 65/zoom, y + 7/zoom, 31/zoom, 25/zoom) then
          ...
          self:drawBackground(cx + 4, cy + 7, width + 10/zoom, 30/zoom, tocolor(34, 36, 43, 255), 5, true)
        ...
        if self:isMouseInPosition(settings.x + settings.w - 130/zoom, y + 4/zoom, 32/zoom, 32/zoom) then
          self:drawBackground(settings.x + settings.w - 130/zoom, y + 4/zoom, 32/zoom, 32/zoom, tocolor(34, 36, 43, 220), 5)
          ...
          self:drawBackground(settings.x + settings.w - 130/zoom, y + 4/zoom, 32/zoom, 32/zoom, tocolor(34, 36, 43, 200), 5)
  ...
  dxDrawText("Możesz wyszukać gracza po Nicku bądź ID wpisując je w wyszukiwarce na górze. Możesz również sortować listę naciskając na nazwę kategorii.", settings.x, settings.y + settings.h - 35/zoom, settings.x + settings.w, settings.y + settings.h, tocolor(220, 220, 220, 100), 1/zoom, settings.fonts.info, "center", "center", true)
end

function Scoreboard:setPrivateMessage(...)
  if arg[1] == localPlayer then
    exports.TR_noti:create("Nie możesz napisać do samego siebie.", "error")
    return
  end
  self:close()
...
function Scoreboard:getPlayerName(plr)
  ...
  if getElementData(plr, "characterMask") and exports.TR_hud:isInDmZone(plr) then
    return string.format("Nieznajomy #%03d", id), true
  end
...
function Scoreboard:getAdminInfo(rank)
  ...
  if string.find(rank, "globalmoderator") then return self.icons.gmod, "#5ce76a" end
...
function Scoreboard:checkPing()
  ...
  if ping > 160 and not self.showedPing then
    ...
    if self.pingError >= 5 then
      ...
      self.pingNoti = exports.TR_noti:create("Wykryto słabe połączenie internetowe. Abyś nie spowodował żadnej szkodliwej akcji wszystkie twoje ruchy zostały ograniczone.", "noNetwork", false, true)
