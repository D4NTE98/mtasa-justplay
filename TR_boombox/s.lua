...
function destroyPlayerBoomboxEvent()
    ...
    triggerClientEvent(client, "equipmentResponse", resourceRoot, "Boombox został schowany.", "success", true)
end
...
function adminRemoveBoombox(targetID)
    ...
    if not targetID then triggerClientEvent(source, "showCustomMessage", resourceRoot, "#008a2eUżycie komendy", "#438f5c/bboff (ID/Nick)", "files/images/command.png") return end
    ...
    exports.TR_noti:create(source, string.format("Boombox gracza %s został wyłączony.", getPlayerName(target)), "success")
end
