...
local dataInfo = {
	seatNames = {"wszystkich pasażerów", "pasażera z przodu", "pasażera lewo tył", "pasażera prawo tył"},
	seatTaxiNames = {"Nie naliczaj opłaty", "Przejazd opłaca pasażer z przodu", "Przejazd opłaca pasażer lewo tył", "Przejazd opłaca pasażer prawo tył"},
...
function VehicleInteraction:scrollKey(...)
	...
	elseif arg[1] == "arrow_r" then
		...
		elseif self.VehicleInteractions[self.scroll].action == 9 then
			...
			self.VehicleInteractions[self.scroll].text = plr and string.format("Przejazd opłaca %s", getPlayerName(plr)) or dataInfo.seatTaxiNames[newTaxiPayer + 1]
...
function VehicleInteraction:render()
	...
	self:animate()
	dxDrawRectangle(0, 0, sx, sy, tocolor(1,1,1, 160 * self.alpha))
	...
	if arg[1] == 4 then
		if vehType == "Boat" then
			if getElementSpeedValue(arg[2], "km/h") > 1 then
				exports.TR_noti:create("Nie możesz zarzucić kotwicy podczas płynięcia.", "error")
				return false
			end

		elseif vehType == "Bike" then
			if getElementSpeedValue(arg[2], "km/h") > 0.1 then
				exports.TR_noti:create("Nie możesz ustawić stopki podczas jazdy.", "error")
				return false
			end

		elseif getElementSpeedValue(arg[2], "km/h") > 0.1 then
			exports.TR_noti:create("Nie możesz zaciągnąć ręcznego w trakcie jazdy.", "error")
			return false
			...
			if hoses then
				if self:getCount(hoses) >= 1 then
					exports.TR_noti:create("Nie możesz użyć ręcznego, ponieważ ktoś w tym momencie korzysta z węża strażackiego.", "error")
					return false
				end
			end
		end
...
function VehicleInteraction:buildDriver(veh, seat)
	...
	if not isElementFrozen(veh) then
		table.insert(self.VehicleInteractions, {
			text = "Hamulec",
			desc = isBike and "Ustaw stópkę" or "Zaciągnij ręczny",
		...
		table.insert(self.VehicleInteractions, {
			text = "Hamulec",
			desc = isBike and "Złóż stópkę" or "Zrzuć ręczny",
	...
	table.insert(self.VehicleInteractions, {
		text = "Wysadź wszystkich pasażerów",
		icon = "remove",
		desc = passengers,
		action = 5,
		index = 0,
	...
	if model == 438 or model == 420 then
		...
		table.insert(self.VehicleInteractions, {
			text = plr and string.format("Przejazd opłaca %s", getPlayerName(plr)) or dataInfo.seatTaxiNames[selected + 1],
			icon = "taxi",
			desc = passengers,
			action = 9,
			index = selected,
