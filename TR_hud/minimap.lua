...
function Minimap:constructor()
    ...
    self.textures.circle = dxCreateTexture("files/images/circle.png", "argb", true, "clamp")
...
function Minimap:renderMap()
    ...
    if getElementData(localPlayer, 'characterUID') == 2 then
       -- dxDrawImage(guiInfo.map.x + guiInfo.map.w - 50/zoom, guiInfo.map.y, guiInfo.map.w / 3.6 / zoom, guiInfo.map.h / 3.6 / zoom, 'files/images/voice.png', 0, 0, 0, tocolor(255, 255, 255, 255 * settings.alpha))
    end

    dxDrawImage(guiInfo.map.x, guiInfo.map.y, guiInfo.map.w, guiInfo.map.h, self.textures.circle, 0, 0, 0, tocolor(23,25,31, 255 * settings.alpha))
...
function Minimap:renderRadarLocation()
	...
	if settings.customLocation then
		dxDrawText(settings.customLocation, guiInfo.map.x, guiInfo.map.y+guiInfo.map.h, guiInfo.map.x+guiInfo.map.w, guiInfo.map.y+guiInfo.map.h+30/zoom, tocolor(255, 255, 255, 200 * settings.alpha), 1/zoom, self.fonts.area, "center", "center")

    elseif city == "Unknown" and zone == "Unknown" then
		dxDrawText("Ocean", guiInfo.map.x, guiInfo.map.y+guiInfo.map.h, guiInfo.map.x+guiInfo.map.w, guiInfo.map.y+guiInfo.map.h+30/zoom, tocolor(255, 255, 255, 200 * settings.alpha), 1/zoom, self.fonts.area, "center", "center")

    else
		dxDrawText(string.format("%s | %s", city, zone), guiInfo.map.x, guiInfo.map.y+guiInfo.map.h, guiInfo.map.x+guiInfo.map.w, guiInfo.map.y+guiInfo.map.h+30/zoom, tocolor(255, 255, 255, 200 * settings.alpha), 1/zoom, self.fonts.area, "center", "center")
	end
end
