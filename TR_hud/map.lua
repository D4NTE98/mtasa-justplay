...
local guiInfo = {
  ...
  legendText = "#2cb5e9PPM #c8c8c8- Opcje mapy\n#2cb5e9LPM #c8c8c8- Przesuwanie mapy\n#2cb5e9Scroll #c8c8c8- Przybliż/oddal mapę",
...
function Map:switch(...)
          ...
          if self:isMouseInPosition(self.options.windowPos.x, self.options.windowPos.y + i * 30/zoom, 200/zoom, 30/zoom) then
            if v[2] == "gps" then
              ...
              if city == "Unknown" and zone == "Unknown" then
                exports.TR_noti:create("Trasa do wybranego punktu nie została znaleziona.", "error")
                removeGPS()
              else
                if #GPS.road > 0 then
                  exports.TR_noti:create("GPS został wyłączony.", "gps")
                  removeGPS()
                else
            ...
            elseif v[2] == "target" then
              ...
              if not pos then
                exports.TR_noti:create("Nie posiadasz wyznaczonego żadnego celu.", "error")
                self.options = nil
                return
              end
              if int ~= 0 or dim ~= 0 then
                exports.TR_noti:create("Twój cel znajduje się wewnątrz jakiegoś budynku.", "error")
                self.options = nil
                return
              end
            ...
            elseif v[2] == "teleport" then
              ...
              setTimer(function()
                ...
                exports.TR_noti:create("Przeniosłeś się we wskazane miejsce.", "success")
  ...
  elseif arg[1] == "mouse2" and arg[2] then
    ...
    if #GPS.road > 0 then
      ...
      self.avaliableOptions = {
        {"Nawiguj tutaj", "gps"},
        {"Nawiguj do celu", "target"},
      }
    end

    local isAdmin = exports.TR_admin:isPlayerDeveloper()
    if isAdmin then
      table.insert(self.avaliableOptions, {"Teleportuj się", "teleport"})
    end
  end
end
...
function Map:render()
  ...
  dxDrawRectangle(guiInfo.legend.x, guiInfo.legend.y, guiInfo.legend.w, guiInfo.legend.h, tocolor(16,18,23, 255 * self.anim))
  dxDrawRectangle(guiInfo.legend.x + guiInfo.legend.w - 3/zoom, 0, 3/zoom, sy, tocolor(44,181,233, 200 * self.anim))
  dxDrawImage(guiInfo.legend.x + 90/zoom, guiInfo.legend.y + 95/zoom, 227/zoom, 90/zoom, ":TR_login/assets/images/logo.png", 0, 0, 0, tocolor(255, 255, 255, 255 * self.anim))
  ...
  dxDrawText("Naciśnij na nazwę blipa aby go pokazać / ukryć", guiInfo.legend.x, guiInfo.legend.y + 300/zoom, guiInfo.legend.x + guiInfo.legend.w, guiInfo.legend.y + 295/zoom, tocolor(160, 160, 160, 200 * self.anim), 1/zoom, guiInfo.fonts.legend, "center", "bottom", false, false, false, true)
...
function Map:renderOptions()
  ...
  self:drawOptionsBackground(self.options.windowPos.x, self.options.windowPos.y, 200/zoom, #self.avaliableOptions * 30/zoom + 32/zoom, tocolor(23,25,31, 255 * self.anim), 5)
  dxDrawText("Wybierz opcję", self.options.windowPos.x, self.options.windowPos.y, self.options.windowPos.x + 200/zoom, self.options.windowPos.y + 30/zoom, tocolor(44,181,233, 255 * self.anim), 1/zoom, guiInfo.fonts.zone, "center", "center")
  ...
  if self.onBlip.blip ~= blip then
    ...
    elseif tonumber(self.onBlip.icon) == 2 then
      ...
      if vehicle and blip then
        ...
        self.onBlip.descTitle = "Informacje o pojeździe:"
...
function Map:renderTarget()
  ...
  dxDrawRectangle(0, 0, guiInfo.target.w,guiInfo.target.h, tocolor(16,18,23, 255))
