...
function InteriorSystem:render()
    ...
    dxDrawImage(guiInfo.x + 20/zoom, guiInfo.y + 20/zoom, guiInfo.h - 40/zoom, guiInfo.h - 40/zoom, string.format(":TR_markers/files/images/%s.png", self.icon), 0, 0, 0, tocolor(44,181,233, 255 * self.alpha))

    dxDrawText(string.format("#aaaaaaNaciśnij %sE #aaaaaaaby %s.", '#2cb5e9', self.isExit and "wyjść" or "wejść"), guiInfo.x + guiInfo.h, guiInfo.y + 20/zoom, guiInfo.h - 40/zoom, guiInfo.y + guiInfo.h - 10/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "left", "bottom", false, false, false, true)
    if self.data then
        if self.data[1] then dxDrawText(self.data[1], guiInfo.x + guiInfo.h, guiInfo.y + 10/zoom, guiInfo.h - 40/zoom, guiInfo.y + guiInfo.h - 10/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "left", "top") end
...
function InteriorSystem:useInterior()
    ...
    if data.closed then
        exports.TR_noti:create("Drzwi do tego budynku są zamknięte.", "error")
        return;
    end;
    ...
    if self.time then
        ...
        if time.hour <= self.time[1] and time.hour >= self.time[2] then
            exports.TR_noti:create("Drzwi są zamknięte.", "error")
            return
        end
    end
    local time = math.random(3, 5)
    if self.isExit then
        exports.TR_dx:showLoading(time * 1000, "Wczytywanie świata")
    else
        exports.TR_dx:showLoading(time * 1000, "Wczytywanie interioru")
    end
    ...
    setTimer(function()
        ...
        if self.isExit then
            ...
            if data.int == 0 and data.dim == 0 then
                ...
                exports.TR_hud:setRadarCustomLocation("Wnętrze budynku | "..data.title, true)
            else
                checkCustomInside = true
            end
        else
            exports.TR_weather:setCustomWeather(0, 12, 0, 9999)
            exports.TR_hud:setRadarCustomLocation("Wnętrze budynku | "..self.data[1], true)
        end
