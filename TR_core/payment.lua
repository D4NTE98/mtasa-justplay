...
function Payment:render(...)
  dxDrawRectangle(0, 0, sx, sy, tocolor(16,18,23, 100 * self.alpha), true)
  self:drawBackground(settings.x, settings.y, settings.w, settings.h, tocolor(23,25,31, 255 * self.alpha), 15, true)

  dxDrawText("Wybierz swoją metodę płatności", settings.x, settings.y + 20/zoom, settings.x + settings.w, settings.y + settings.h, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, settings.fonts.title, "center", "top", false, false, true)
  if self.amount == 0 then
    dxDrawText("Za darmo", settings.x, settings.y + 50/zoom, settings.x + settings.w, settings.y + settings.h, tocolor(44,181,233, 255 * self.alpha), 1/zoom, settings.fonts.amount, "center", "top", false, false, true)
  else
    dxDrawText(string.format("Do zapłaty: $%.2f", self.amount), settings.x, settings.y + 50/zoom, settings.x + settings.w, settings.y + settings.h, tocolor(44,181,233, 255 * self.alpha), 1/zoom, settings.fonts.amount, "center", "top", false, false, true)
  end

  self:renderPayment(settings.x + 20/zoom, "Gotówka", self.cash, self.plrCash)
  self:renderPayment(settings.x + settings.w - 290/zoom, "Karta", self.credit, self.plrBankmoney)
  self:renderCancel()
...
function Payment:renderCancel()
  ...
  dxDrawText("Anuluj transakcje", settings.x + 238/zoom, settings.y + settings.h - 50/zoom, settings.x + settings.w, settings.y + settings.h - 18/zoom, tocolor(44,181,233, alpha * self.alpha), 1/zoom, settings.fonts.amount, "left", "center", false, false, true)
end
...
function Payment:renderPayment(x, name, img, amount)
  ...
  if self:isMouseInPosition(x, settings.y + 90/zoom, 270/zoom, 290/zoom) and amount and amount >= self.amount then
    self:drawBackground(x, settings.y + 90/zoom, 270/zoom, 290/zoom, tocolor(42,45,51, alpha * self.alpha), 15, true)
  else
    self:drawBackground(x, settings.y + 90/zoom, 270/zoom, 290/zoom, tocolor(34,36,43, alpha * self.alpha), 15, true)
  end
  ...
  if amount then
    dxDrawText(string.format("$%.2f", amount), x, settings.y + 330/zoom, x + 270/zoom, settings.y + settings.h - 20/zoom, tocolor(44,181,233, alpha * self.alpha), 1/zoom, settings.fonts.amount, "center", "top", false, false, true)
  else
    dxDrawText("Brak karty", x, settings.y + 330/zoom, x + 270/zoom, settings.y + settings.h - 20/zoom, tocolor(44,181,233, alpha * self.alpha), 1/zoom, settings.fonts.amount, "center", "top", false, false, true)
  end
end
...
function createPaymentScreen(...)
  ...
  exports.TR_dx:setResponseEnabled(true, "Trwa akceptowanie transakcji")
end
