...
function createVehicleForShop(shopName, index)
    ...
    local veh = createVehicle(model, v.pos, v.rot, "SPRZEDAZ")
...
function shopBuyVehicle(state, data)
    if state then
        ...
        exports.TR_discord:remoteDsc(tf.." | "..string.format("**%s** zakupił pojazd **%s** w salonie, za kwotę **$%.2f**", getPlayerName(source), getVehicleNameFromModel(data.model), data.toPay), "pojazdy");
