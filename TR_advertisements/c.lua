...
function Advertisements:open(sender, text)
    ...
    if self.senderPlayer then
        ...
        if id then
            self.sender = string.format("[%d] %s ogłasza:", id, sender)
        else
            self.sender = sender .. " ogłasza:"
        end
    else
        self.sender = sender .. " ogłasza:"
    end
...
function Advertisements:render()
    ...
    self:checkTime()
    self:drawBackground(guiInfo.x, guiInfo.y, guiInfo.w, guiInfo.h, tocolor(23,25,31, 255 * self.alpha), 4)
    dxDrawText(self.sender, guiInfo.x + 10/zoom, guiInfo.y + 2/zoom, guiInfo.x + guiInfo.w, guiInfo.h, tocolor(44,183,236, 255 * self.alpha), 1/zoom, self.fonts.sender, "left", "top")
    dxDrawText(self.text, guiInfo.x + 10/zoom, guiInfo.y + 22/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.h, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.text, "left", "top", true, true)
end
