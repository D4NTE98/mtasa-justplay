...
local guiInfo = {
    ...
    options = {
        ...
        {
            text = "Program Rozszerzony",
            desc = "Mycie wodą, szczotkowanie",
            img = "files/images/default.png",
            price = 80,
            cleanAmount = 450,
        },
        {
            text = "Program Premium",
            desc = "Mycie wodą, mycie chemią, szczotkowanie",
            img = "files/images/premium.png",
            price = 130,
            cleanAmount = 600,
        },
        {
            text = "Program Premium Max",
            desc = "Mycie wodą, mycie chemią, szczotkowanie, woskowanie",
...
function CarWash:constructor(...)
    ...
    self:createNoti("Aby otworzyć panel myjni zgaś silnik, zaciągnij ręczny i zamknij wszystkie okna.", 3, true)
...
function CarWash:open()
    ...
    showCursor(true)
    self.exitButton = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 55/zoom, 250/zoom, 40/zoom, "Zamknij okno", "blue")
...
function CarWash:render()
    ...
    dxDrawImage((sx - 160/zoom)/2, guiInfo.y + 20/zoom, 148/zoom, 130/zoom, self.textures.logo, 0, 0, 0, tocolor(255, 255, 255, 255 * self.alpha))
    ...
    for i, v in pairs(guiInfo.options) do
        if self:isMouseInPosition(guiInfo.x + 20/zoom, guiInfo.y + 170/zoom + 75/zoom * (i-1), guiInfo.w - 40/zoom, 70/zoom) then
            self:drawBackground(guiInfo.x + 20/zoom, guiInfo.y + 170/zoom + 75/zoom * (i-1), guiInfo.w - 40/zoom, 70/zoom, tocolor(34,36,43, 255 * self.alpha), 11)
        else
            self:drawBackground(guiInfo.x + 20/zoom, guiInfo.y + 170/zoom + 75/zoom * (i-1), guiInfo.w - 40/zoom, 70/zoom, tocolor(16,18,23, 255 * self.alpha), 11)
        end
