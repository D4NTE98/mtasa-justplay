...
addCommandHandler("brud", function(cmd, val)
    local veh = getPedOccupiedVehicle(localPlayer)
    if not veh then outputChatBox("NIE SIEDZISZ W POJEŹDZIE") return end
    if not val or tonumber(val) == nil then outputChatBox("/brud (wartość)") return end

    setElementData(veh, "vehicleGrunge", tonumber(val))
    outputConsole("Dziala na "..val)
end)