...
function VehicleSpawner:spawnVehiclesCoroutine()
	...
	if vehicles and #vehicles > 0 then
		...
		print("[TR_vehicles] Załadowano " .. self.totalCount .. " samochodów w czasie " .. getTickCount() - self.startTime .. "ms.")

		exports.TR_vehicleExchange:updateVehicleExchange()
	else
		print("[TR_vehicles] Brak samochodów.")
	end
...
function canEnterVehicle(plr, seat, jacked, door)
	...
	if seat == 0 and not canUseVehicle(source, plr) then
		exports.TR_noti:create(plr, "Ten pojazd nie jest twoją własnością.", "error")
		exports.TR_noti:create(plr, "Nie posiadasz wytrychu by ukraść ten pojazd.", "error")
		cancelEvent()
		return
	end
end
...
function updateEngineState(plr, seat, jacked)
	...
	if getElementModel(source) == 510 then
		setVehicleDoorState(source, true)
	end
end
...
function canUseVehicle(vehicle, plr)
	...
	if getElementModel(vehicle) == 420 then return true end
...
function setVehicleWheelBlock(veh, wheel)
	...
	if wheel then
		exports.TR_noti:create(client, "Blokada na koło została założona.", "success")
		exports.TR_mysql:querry("UPDATE tr_vehicles SET wheelBlock = ? WHERE ID = ? LIMIT 1", wheel, vehData.ID)
	else
		exports.TR_noti:create(client, "Blokada na koło została zdjęta.", "success")
...
function canVehicleEnter(plr, vehicle, jacked)
	...
	if muteData and muteData[1] then
		exports.TR_noti:create(plr, "Nie możesz wsiąść za kółko, ponieważ masz zakaz prowadzenia pojazdów nałożony przez administratora.", "error", 5)
	  	return false
	end
...
function vr()
	...
	outputDebugString("[TR_vehicles] Pojazdy pod wodą: " ..vehiclesInWater)
end
