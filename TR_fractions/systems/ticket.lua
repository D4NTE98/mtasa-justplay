...
function Ticket:render()
    ...
    dxDrawText("Wystawianie mandatu", guiInfo.x + 10/zoom, guiInfo.y + 10/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 10/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "top", true, true)
    ...
    for i = 1, 9 do
        ...
        if v then
            if self:isMouseInPosition(guiInfo.x + 10/zoom, guiInfo.y + 90/zoom + (i-1) * 40/zoom, guiInfo.w - 24/zoom, 40/zoom) then
                dxDrawRectangle(guiInfo.x + 10/zoom, guiInfo.y + 90/zoom + (i-1) * 40/zoom, guiInfo.w - 24/zoom, 40/zoom, tocolor(42,45,51, 255 * self.alpha))
            else
                dxDrawRectangle(guiInfo.x + 10/zoom, guiInfo.y + 90/zoom + (i-1) * 40/zoom, guiInfo.w - 24/zoom, 40/zoom, tocolor(34,36,43, 255 * self.alpha))
            end

            if v.selected then
                dxDrawText(v.name, guiInfo.x + 20/zoom, guiInfo.y + 90/zoom + (i-1) * 40/zoom, guiInfo.x + 10/zoom, guiInfo.y + 90/zoom + i * 40/zoom, tocolor(28,89,119, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "center")
                dxDrawText(string.format("$%.2f", v.price), guiInfo.x + 20/zoom, guiInfo.y + 90/zoom + (i-1) * 40/zoom, guiInfo.x + guiInfo.w - 24/zoom, guiInfo.y + 90/zoom + i * 40/zoom, tocolor(28,89,119, 255 * self.alpha), 1/zoom, self.fonts.info, "right", "center")
    ...
    if #self.options > 9 then
        ...
        dxDrawRectangle(guiInfo.x + guiInfo.w - 14/zoom, guiInfo.y + 90/zoom, 4/zoom, 360/zoom, tocolor(42,45,51, 255 * self.alpha))
        dxDrawRectangle(guiInfo.x + guiInfo.w - 14/zoom, guiInfo.y + 90/zoom + barY, 4/zoom, barHeight, tocolor(34,36,43, 255 * self.alpha))
    else
        dxDrawRectangle(guiInfo.x + guiInfo.w - 14/zoom, guiInfo.y + 90/zoom, 4/zoom, 360/zoom, tocolor(34,36,43, 255 * self.alpha))
    end
end
