...
function ObjectSystem:open()
    ...
    bindKey("arrow_up", "down", self.func.switchObject)
    bindKey("arrow_down", "down", self.func.switchObject)
...
function ObjectSystem:close()
    ...
    unbindKey("arrow_up", "down", self.func.switchObject)
    unbindKey("arrow_down", "down", self.func.switchObject)
...
function ObjectSystem:render()
    ...
    dxDrawText(guiInfo.avaliableObjects[self.type][self.selected].name, guiInfo.x, guiInfo.y + 115/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 115/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top")

    dxDrawText(string.format("Postawione obiekty: #2cb5e9%d#dcdcdc/#2cb5e9%d", self.createdObjects, guiInfo.objectLimit), guiInfo.x, guiInfo.y + 150/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 180/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", false, false, false, true)
    dxDrawText("Aby postawić obiekt użyj #2cb5e9LPM", guiInfo.x, guiInfo.y + 180/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 210/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", false, false, false, true)
    dxDrawText("Aby usunąć postawione obiekty użyj #2cb5e9PPM", guiInfo.x, guiInfo.y + 200/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 230/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", false, false, false, true)
    dxDrawText("Aby zmienić obiekt użyj klawiszy #2cb5e9q(<) #dcdcdclub #2cb5e9e (>)", guiInfo.x, guiInfo.y + 220/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 250/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", false, false, false, true)
end
...
function ObjectSystem:switchObject(...)
    ...
    if arg[1] == "arrow_up" then
        self.selected = self.selected - 1
        if self.selected < 1 then self.selected = #guiInfo.avaliableObjects[self.type] end

    elseif arg[1] == "arrow_down" then
        self.selected = self.selected + 1
        if self.selected > #guiInfo.avaliableObjects[self.type] then self.selected = 1 end
    end
    self:attachObject()
