...
function Panel:createList(...)
    ...
    if arg[2] == "take" then
        ...
        exports.TR_noti:create("Zgłoszenie zostało przyjęte.", "success", 6)
        exports.TR_jobs:createInformation(false, string.format("Jak najszybciej udaj się na miejsce zgłoszenia. Lokalizacja: %s.", self.selectData.destination))
    ...
    elseif arg[2] == "taken" then
        exports.TR_noti:create("Te zgłoszenie zostało już przyjęte.", "error")

    elseif arg[2] == "removed" then
        exports.TR_noti:create("Zgłoszenie zostało usunięte.", "success")

    elseif arg[2] == "old" then
        exports.TR_noti:create("Zgłoszenie straciło ważność.", "error")
    end
...
function Panel:render()
    ...
    dxDrawText("Zgłoszenia służb", guiInfo.x + 10/zoom, guiInfo.y, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 40/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    for i = 1, 6 do
        if self.requests[i] then
            if self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 60/zoom * (i-1), guiInfo.w, 60/zoom) then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 60/zoom * (i-1), guiInfo.w, 60/zoom, tocolor(34,36,43, 255 * self.alpha))
            end
            ...
            dxDrawText(string.format("godz: %s", self.requests[i].time), guiInfo.x + 15/zoom, guiInfo.y + 60/zoom + 60/zoom * (i-1), guiInfo.x + 150/zoom, guiInfo.y + 98/zoom + 60/zoom * (i-1), tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "left", "bottom", true, false)
            ...
            dxDrawText("Odległość:", guiInfo.x + 15/zoom, guiInfo.y + 62/zoom + 60/zoom * (i-1), guiInfo.x + guiInfo.w - 15/zoom, guiInfo.y + 110/zoom + 60/zoom * (i-1), tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "right", "top", true, false)
    ...
    dxDrawText("JEŚLI Z TREŚCI ZGŁOSZENIA WYNIKA, ŻE BĘDĄ POTRZEBNE RÓWNIEŻ INNE FRAKCJE DO PRZEPROWADZENIA ODPOWIEDNICH DZIAŁAŃ, ZAWIADOM ICH FUNKCJONARIUSZY UŻYWAJĄC KRÓTKOFALÓWKI! ZGŁOSZENIA TRACĄ WAŻNOŚĆ PO 10 MIN.", guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 60/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h, tocolor(28,89,119, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center", true, true)
end
...
function updateBlipPos()
    ...
    if getDistanceBetweenPoints3D(bpos, ppos) < 20 then
        ...
        exports.TR_noti:create("Dotarłeś na miejsce zgłoszenia.", "success")
    end
end
