...
function addFractionRequest(fraction, text)
    if requests[fraction][client] then exports.TR_noti:create(client, "Posiadasz już jedno zgłoszenie wysłane do służb.", "error") return end
    requests[fraction][client] = {pos = {getElementPosition(client)}, time = getRequestTime(), text = text, tick = getTickCount()}

    exports.TR_noti:create(client, "Służby zostały poinformowane. Za niedługo ktoś powinien się zjawić.", "success")
...
function selectFractionRequest(fraction, player)
    if requests[fraction][player] then
        ...
        if isElement(player) then
            triggerClientEvent(player, "showCustomMessage", resourceRoot, "#d89932Dyspozytor", "#ac7a28Zgłoszenie zostało przyjęte. Odpowiednie służby powinny niebawem znaleźć się we wskazanej lokalizacji.", "files/images/msg_received.png")
        end
    else
