...
function onEnterFractionVehicle(plr, seat, jacked)
    ...
    if not plrFraction then exports.TR_noti:create(plr, "Musisz być na służbie frakcyjnej aby móc wsiąść do tego pojazdu.", "error"); cancelEvent() return end
    if plrFraction[4] ~= fractionID then exports.TR_noti:create(plr, "Musisz być na służbie frakcyjnej aby móc wsiąść do tego pojazdu.", "error"); cancelEvent() return end
    ...
    if not hasLicence or not hasLicence[1] then exports.TR_noti:create(plr, "Musisz posiadać odpowiedniego prawa jazdy aby móc prowadzić ten pojazd.", "error"); cancelEvent() return end

    local rankID = exports.TR_mysql:querry("SELECT rankID FROM tr_fractionsPlayers WHERE playerUID = ? LIMIT 1", uid)
    if not rankID or not rankID[1] then exports.TR_noti:create(plr, "Musisz być na służbie frakcyjnej aby móc wsiąść do tego pojazdu.", "error"); cancelEvent() return end
    ...
    if not hasPermission or not hasPermission[1] then exports.TR_noti:create(plr, "Nie posiadasz uprawnień do kierowania tym pojazdem.", "error"); cancelEvent() return end
end
