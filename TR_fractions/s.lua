    ...
    for i, v in pairs(getElementsByType("player")) do
        ...
        if duty then
            if duty[3] == characterDuty[3] then
                triggerClientEvent(v, "showCustomMessage", resourceRoot, string.format("#dd3ed0[FRAKCJA] [%d] %s", id, name), msg, "files/images/diathermy.png")
            end
        end
    end
end
...
function syncPlayerFractionMinutes(time)
    ...
    if time and time[1] then
        exports.TR_mysql:querry("UPDATE tr_fractionsDutyTimes SET minutes = minutes + 1, count = ? WHERE ID = ?", calculatePlayerMoney(tonumber(time[1].minutes) + 1, tonumber(plrRank[1].level), tonumber(maxRank[1].level), 4500, 9300, 1), time[1].ID)
    else
        exports.TR_mysql:querry("INSERT INTO tr_fractionsDutyTimes (playerUID, minutes, count, day) VALUES (?, 1, ?, CURDATE())", uid, calculatePlayerMoney(1, tonumber(plrRank[1].level), tonumber(maxRank[1].level), 4500, 9300, 1))
    end
end
