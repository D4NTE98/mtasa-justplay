...
function FractionJobStart:constructor(...)
    ...
    self.buttons.start = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h - 50/zoom, 235/zoom, 40/zoom, jobID == self.jobID and "Zakończ służbę" or "Rozpocznij służbę")
...
function FractionJobStart:render()
    ...
    dxDrawText(self.fractionName, guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    dxDrawText("Aby rozpocząć tutaj służbę musisz należeć do frakcji. Aby się do niej dostać należy przejść rekrutację znajdującą się na forum serwera, która jest prowadzona przez lidera.", guiInfo.x + 20/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 110/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
    dxDrawText("Po zakończonej służbie należy się odbić, aby móc przystąpić do pracy dorywczej.", guiInfo.x + 20/zoom, guiInfo.y + 120/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(8,89,119, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
end
...
function FractionJobStart:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.start then
        ...
        if not self.jobID then exports.TR_noti:create("Nie możesz rozpocząć służby w tym miejscu, ponieważ nie jesteś członkiem tej frakcji.", "error") return end
        ...
        if not job then
            ...
            for i, v in pairs(slots) do
                if getPedWeapon(localPlayer, v) > 0 then
                    exports.TR_noti:create("Nie możesz rozpocząć służby, ponieważ posiadasz wyekwipowaną broń prywatną.", "error")
                    return
                end
            end
        ...
        elseif job and job == self.jobID then
            ...
            exports.TR_noti:create("Nie możesz rozpocząć służby w tym miejscu, ponieważ jesteś już gdzieś zatrudniony.", "error")
        end
    end
end
...
function FractionJobStart:reponse(...)
  ...
  if arg[1] == "start" then
    ...
    exports.TR_dx:setButtonText(self.buttons.start, "Zakończ służbę")

    local info = self.fractionType == "police" and "Udaj się do przebieralni i ubierz odpowiedniego skina lub od razu wybierz się na strzelnicę i odbierz swoje wyposażenie." or "Udaj się do przebieralni i ubierz odpowiedniego skina."
	exports.TR_jobs:createInformation(self.fractionName, info)

	exports.TR_noti:create("Pomyślnie rozpocząłeś służbę.", "success")
  ...
  elseif arg[1] == "end" then
	...
	exports.TR_dx:setButtonText(self.buttons.start, "Rozpocznij służbę")

    exports.TR_noti:create("Pomyślnie zakończyłeś swoją służbę.", "success")
...
function addedFractionReport(fraction)
    ...
    exports.TR_noti:create("Przyszło nowe zgłoszenie frakcyjne.", "info")
end
