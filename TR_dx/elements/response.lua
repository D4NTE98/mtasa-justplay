...
function Response:draw()
  ...
  dxDrawRectangle(self.x, self.y, self.w, self.h, tocolor(23,25,31, 255 * self.alpha), true)

  self.rot = self.rot >= 360 and self.rot - 360 or self.rot + 5
  dxDrawImage(self.x + 10/zoom, self.y + 5/zoom, 40/zoom, 40/zoom, guiData.response.spinner, -self.rot, 0, 0, tocolor(44,181,233, 255 * self.alpha), true)
...
function Response:setEnabled(...)
  if arg[1] then
    ...
    self.tick = getTickCount()
    self.enabled = true
    self.text = arg[2] and arg[2] or "Czekanie na odpowiedź serwera"
  else
    self.sAlpha = self.alpha
    self.anim = "hide"
    self.tick = getTickCount()
