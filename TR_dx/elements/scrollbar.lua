  ...
  for i = 1, self.visibleCount do
    ...
    if i == 1 then
      if self.visibleCount < #self.data then
        self:drawUpLeft(self.x, self.y + self.rowH * (i-1), self.w, self.rowH, tocolor(34,36,43, alpha * self.alpha), 5, true)
      else
        self:drawUp(self.x, self.y + self.rowH * (i-1), self.w, self.rowH, tocolor(34,36,43, alpha * self.alpha), 5, true)
      end

    elseif i == self.visibleCount or id == #self.data then
      if self.visibleCount < #self.data then
        self:drawDownLeft(self.x, self.y + self.rowH * (i-1), self.w, self.rowH, tocolor(34,36,43, alpha * self.alpha), 5, true)
      else
        self:drawDown(self.x, self.y + self.rowH * (i-1), self.w, self.rowH, tocolor(34,36,43, alpha * self.alpha), 5, true)
      end
    else
      dxDrawRectangle(self.x, self.y + self.rowH * (i-1), self.w, self.rowH, tocolor(34,36,43, alpha * self.alpha), true)
    end

    local textColor = {110, 119, 132}
    if self.clickable then
      local color = self.selected[id] and {42,45,51} or {34,36,43}
      textColor = self.selected[id] and {110, 119, 132} or textColor
...
function Scrollbar:drawBar()
  ...
  local alpha = 255
  if not self.background then alpha = 0 end
  self:drawRight(self.x + self.w, self.y, 10/zoom, self.h, tocolor(34,36,43, alpha * self.alpha), 5, true)
  self:drawBackground(self.x + self.w, self.y + self.barY, 10/zoom, self.barHeight, tocolor(44,181,233, 200 * self.alpha), 5, true)
  self:drawBackground(self.x + self.w, self.y + self.barY, 10/zoom, self.barHeight, tocolor(44,181,233, 230 * self.alpha * (self.barHover)), 5, true)
end
