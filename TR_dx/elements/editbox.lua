...
function Editbox:constructor(...)
    ...
    --guiSetFont(self.element, getFont(self.fontSize, "Lato-Bold"))
...
function Editbox:draw()
    ...
    if self.selected then
        self:drawBackground(self.x-2/zoom, self.y-2/zoom, self.w+4/zoom, self.h+4/zoom, tocolor(34, 36, 43, 255 * self.alpha), 5, true)
        self:drawBackground(self.x, self.y, self.w, self.h, tocolor(23, 25, 31, 255 * self.alpha), 5, true)
    else
        self:drawBackground(self.x-2/zoom, self.y-2/zoom, self.w+4/zoom, self.h+4/zoom, tocolor(34, 36, 43, 255 * self.alpha), 5, true)
	end
    ...
    local textLength = dxGetTextWidth(text, 1/zoom, getFont(self.fontSize, "Lato-Bold"))
	...
	if self.selectedCaret then
		...
		elseif self.selectedCaret > self.caret then
			local caretStart = dxGetTextWidth(utf8.sub(text, 1, self.caret), 1, getFont(self.fontSize, "Lato-Bold"))
			local caretEnd = dxGetTextWidth(utf8.sub(text, 1, self.selectedCaret), 1, getFont(self.fontSize, "Lato-Bold"))
			local caretSize = caretEnd - caretStart
			dxDrawRectangle(posX + caretStart, self.smallTextHeight - 2/zoom, caretSize, self.rtH - (self.smallTextHeight + 8), tocolor(23, 25, 31, self.selected and 60 or 30))

		elseif self.selectedCaret < self.caret then
			local caretStart = dxGetTextWidth(utf8.sub(text, 1, self.selectedCaret), 1, getFont(self.fontSize, "Lato-Bold"))
			local caretEnd = dxGetTextWidth(utf8.sub(text, 1, self.caret), 1, getFont(self.fontSize, "Lato-Bold"))
			local caretSize = caretEnd - caretStart
			dxDrawRectangle(posX + caretStart, self.smallTextHeight - 2/zoom, caretSize, self.rtH - (self.smallTextHeight + 8), tocolor(23, 25, 31, self.selected and 60 or 30))
		end
	end
    ...
    if self.placeholder ~= "" then
        if zoom ~= 1 or self.h <= 30/zoom then
            if text == "" then
                dxDrawText(utf8.upper(self.placeholder), posX, 0, self.rtW, self.rtH, tocolor(200, 200, 200, 255), 1, getFont(self.fontSize, "Lato-Bold"), self.textAlignHorizontal, "center", true, false, false)
            else
                dxDrawText(text or "", posX, 0, self.rtW, self.rtH, tocolor(200, 200, 200, 255), 1, getFont(self.fontSize, "Lato-Bold"), self.textAlignHorizontal, "center", true, false, false)
            end
        else
            if text == "" then
                dxDrawText(utf8.upper(self.placeholder), 0, 0, self.rtW, self.smallTextHeight + (self.rtH - (self.smallTextHeight + 4)) * (1-self.lastSelectedAnim), tocolor(150, 150, 150, 255 * self.lastSelectedAnim), 1, getFont(self.smallFontSize, "Lato-Bold"), "left", "center", true, false, false)
                dxDrawText(utf8.upper(self.placeholder), 0, 0, self.rtW, self.smallTextHeight + (self.rtH - (self.smallTextHeight + 4)) * (1-self.lastSelectedAnim), tocolor(150, 150, 150, 255 * (1-self.lastSelectedAnim)), 1 - 0.25 * self.lastSelectedAnim, getFont(self.fontSize, "Lato-Bold"), "left", "center", true, false, false)
            else
                dxDrawText(utf8.upper(self.placeholder), 0, 0, self.rtW, self.smallTextHeight, tocolor(150, 150, 150, 255), 1, getFont(self.smallFontSize, "Lato-Bold"), "left", "center", true, false, false)
                dxDrawText(text or "", posX, 0, self.rtW, self.rtH - 9, tocolor(200, 200, 200, 255), 1, getFont(self.fontSize, "Lato-Bold"), self.textAlignHorizontal, self.textAlignVertical, true, false, false)
            end
        end
    else
        dxDrawText(text or "", posX, 0, self.rtW, self.rtH - 9, tocolor(200, 200, 200, 255), 1, getFont(self.fontSize, "Lato-Bold"), self.textAlignHorizontal, self.textAlignVertical, true, false, false)
    ...
    if self.selected then
        if editSettings.separator[1] then
            local toCarethLength = dxGetTextWidth(utf8.sub(text, 0, self.caret), 1, getFont(self.fontSize, "Lato-Bold"))
    ...
    if self.image then
        if self.selected then
            self:drawBackground(self.x-2/zoom, self.y-2/zoom, self.h+4/zoom, self.h+4/zoom, tocolor(34, 36, 43, 255 * self.alpha), 5, true)
            self:drawBackground(self.x, self.y, self.h, self.h, tocolor(23, 25, 31, 255 * self.alpha), 5, true)
        else
           self:drawBackground(self.x, self.y, self.h, self.h, tocolor(34, 36, 43, 255 * self.alpha), 5, true)
        end
	...
	if self.masked then
		if self.selected then
			if isMouseInPosition(self.x + self.w - self.h + 10/zoom, self.y + 10/zoom, self.h - 20/zoom, self.h - 20/zoom) then
				dxDrawImage(self.x + self.w - self.h + 10/zoom, self.y + 10/zoom, self.h - 20/zoom, self.h - 20/zoom, self.showHidden and "files/images/show.png" or "files/images/hide.png", 0, 0, 0, tocolor(220, 220, 220, 255 * self.alpha), true)
			else
				dxDrawImage(self.x + self.w - self.h + 10/zoom, self.y + 10/zoom, self.h - 20/zoom, self.h - 20/zoom, self.showHidden and "files/images/show.png" or "files/images/hide.png", 0, 0, 0, tocolor(170, 170, 170, 255 * self.alpha), true)
			end

			if guiData.capsOn then
				dxDrawImage(self.x + self.w - self.h + 4/zoom - (self.h - 26/zoom), self.y + 13/zoom, self.h - 26/zoom, self.h - 26/zoom, "files/images/caps.png", 0, 0, 0, tocolor(170, 170, 170, 255 * self.alpha), true)
			end
		else
			if guiData.capsOn then
				dxDrawImage(self.x + self.w - self.h + 13/zoom, self.y + 13/zoom, self.h - 26/zoom, self.h - 26/zoom, "files/images/caps.png", 0, 0, 0, tocolor(170, 170, 170, 255 * self.alpha), true)
			end
		end
    end
...
function Editbox:getPosForText()
	...
	if self.posX then
		...
		elseif -(self.posX - posX) >= (fieldW/4 * 3) then
			local current = dxGetTextWidth(utf8.sub(text, 0, self.caret), 1, getFont(self.fontSize, "Lato-Bold"))
			local oneBefore = dxGetTextWidth(utf8.sub(text, 0, self.caret - 1), 1, getFont(self.fontSize, "Lato-Bold"))
...
function Editbox:getEditCaretFromMouse()
    ...
    for v in utf8.gmatch(self.text, ".") do
		...
		local testLen = dxGetTextWidth(text, 1/zoom, getFont(self.fontSize, "Lato-Bold"))
...
function setEditX(...)
    if type(arg[1]) == "table" then
        for _, v in pairs(arg[1]) do
            createdEdits[v[1]]:setX(v[2])
        end
    else
        createdEdits[arg[1]]:setX(arg[2])
    end
    return true
end
...
function setEditY(...)
    if type(arg[1]) == "table" then
        for _, v in pairs(arg[1]) do
            createdEdits[v[1]]:setY(v[2])
        end
    else
        createdEdits[arg[1]]:setY(arg[2])
    end
    return true
end
