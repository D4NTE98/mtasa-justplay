...
function Pizzaboy:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako dostawca pizzy.", "job")
    exports.TR_jobs:createInformation(jobSettings.name, "Udaj się do szatni, aby założyć firmowy strój.")

    exports.TR_jobs:setPlayerTargetPos(368.19998168945, -116.17266845703, 1001.9995117188, 5, 1, "Użyj interakcji (e) i naciśnij na szafkę aby się przebrać")
    return true
end
...
function Pizzaboy:render()
    ...
    if getDistanceBetweenPoints3D(Vector3(getElementPosition(localPlayer)), Vector3(getElementPosition(self.point))) < 50 then
        if not self.isNear then
            exports.TR_jobs:createInformation(jobSettings.name, string.format("Zabierz pizzę ze skutera i zanieś ją klientowi pod drzwi.\n Pozostałe zamówienia: %d", self.pizzaCount), 240/zoom)
            self.isNear = true
        end

    else
        if self.isNear then
            exports.TR_jobs:createInformation(jobSettings.name, string.format("Wsiądź na skuter i udaj się do klienta aby dostarczyć jego zamówienie.\n Pozostałe zamówienia: %d", self.pizzaCount), 240/zoom)
            self.isNear = nil
        end
    end
end
...
function Pizzaboy:chiefMarkerHit(hit, md)
    ...
    exports.TR_jobs:createInformation(jobSettings.name, string.format("Wsiądź na skuter, który czeka na ciebie na parkingu z prawej strony lokalu.\n Pozostałe zamówienia: %d", self.pizzaCount), 240/zoom)
end
...
function Pizzaboy:createChiefMarker()
    ...
    setElementData(self.chiefMarker, "markerData", {
        title = "Odbiór pizzy",
        desc = "Wejdź w marker aby odebrać listę zamówień.",
    }, false)
...
function Pizzaboy:pointMarkerHit(hit, md)
    ...
    if not self.holdingPizza then exports.TR_noti:create("Musisz zabrać pizzę z pojazdu.", "error") return end
...
function Pizzaboy:createPoint()
    ...
    setElementData(self.point, "markerData", {
        title = "Dostarczenie pizzy",
        desc = "Wejdź w marker aby dostarczyć pizze.",
    }, false)
    ...
    exports.TR_jobs:setPlayerTargetPos(points[r].x, points[r].y, points[r].z - 0.5, 0, 0, "Dostarcz pizze do klienta")
...
function Pizzaboy:refreshPizzaCount()
    if self.pizzaCount >= 1 then
        ...
        exports.TR_jobs:createInformation(jobSettings.name, "Wróć do restauracji po kolejne zamówienia.")
        exports.TR_jobs:setPlayerTargetPos(-1806.7744140625, 943.859375, 24.890625 - 0.5, 0, 0, "Wróć do restauracji po kolejne zamówienia")
...
function Pizzaboy:getPizzaFromVehicle()
    if getDistanceBetweenPoints3D(Vector3(getElementPosition(localPlayer)), Vector3(getElementPosition(self.point))) < 50 then
        ...
        exports.TR_jobs:createInformation(jobSettings.name, "Zanieś pizze klientowi pod drzwii.", 240/zoom)
        return true
    else
        exports.TR_noti:create("Nie możesz wyjąć pizzy, ponieważ jesteś zbyt daleko od miejsca docelowego, a nie możesz dostraczyć pizzy zimnej.", "error")
        return false
    end
end
...
function Pizzaboy:payForWork()
    ...
    if self.ownedUpgrades[2] then
        if math.random(1, 10) >= 5 then
            ...
            payment = payment + tip
            exports.TR_noti:create(string.format("Otrzymałeś napiwek w wysokości $%.2f.", tip), "money")
        end
    else
