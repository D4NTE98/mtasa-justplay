...
function SantaJob:constructor(...)
    ...
    setElementData(self.marker, "markerData", {
        title = "Punkt ładowania piasku",
        desc = "Wejdź w marker aby załadować piasek na pakę.",
    }, false)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę odśnieżania ulic.", "job", 5)
    ...
    if self.selectedState == "snowPlow" then
        exports.TR_jobs:createInformation(jobSettings.name, string.format("Jeździj po ulicach Angel Pine i okolicach aby odśnieżać teren. Pilnuj aby twój kompan jechał za tobą.\n Współpracownik: %s", self.otherDriver.name or ""))

    elseif self.selectedState == "sand" then
        exports.TR_jobs:createInformation(jobSettings.name, string.format("Utrzymuj odpowiednią odległość od swojego kompana aby rozsypywać sól. Gdy tylko ci się skończy nie zapomnij go o tym poinformować.\n Współpracownik: %s", self.otherDriver.name or ""), guiInfo.h + 30/zoom)
    end
...
function SantaJob:onStartExit(plr)
    ...
    exports.TR_noti:create("Z pojazdu możesz wysiąść tylko na terenie firmy. Chyba nie chcesz zostawiać swojego współpracownika samego w pracy?", "error")
end
...
function SantaJob:resetSand(el, md)
    ...
    if self.selectedState == "sand" then
        if self.sand > 0 then
            exports.TR_noti:create("W naczepie pozostał jeszcze piasek.", "error")
            return
        end
        ...
        exports.TR_noti:create("Nie mozesz załadować piasku na traktor.", "error")
    end
end
...
function SantaJob:render()
    if not isElement(self.otherDriver.player) or not getElementData(self.otherDriver.player, "inJob") then
        ...
        self:destroy()
        exports.TR_noti:create("Twój współpracownik wyszedł z pracy, więc została ona zakończona.")
        return
    end
    ...
    if self.selectedState == "sand" then
        ...
        if veh then
            ...
            if self.tractor then
                ...
                elseif not self.isFar then
                    self.isFar = true
                    exports.TR_noti:create("Odjechałeś zbyt daleko od współpracownika. Podjedź bliżej aby kontynuować pracę.", "error")
                end
            end
        end
        ...
        if (getTickCount() - self.takeSand)/5000 > 1 and not self.isFar then
            ...
            if self.sand <= 0 then
                self.sand = 0
                exports.TR_noti:create("W twoim wozie skończył się piasek. Poinformuj o tym swojego kompana i wróć na bazę się załadować. Możecie wrócić razem lub może on na ciebie poczekać w tym miejscu.", "info", 15)
                return
            end
            self.takeSand = getTickCount()
        end
