...
function TargetPanel:render()
    ...
    dxDrawText("Wybierz trasę autobusu", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    ...
    for i = 1, 10 do
        ...
        if v then
            ...
            dxDrawText("Trasa linii "..v.number, guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom + 40/zoom * i, color, 1/zoom, self.fonts.text, "left", "center")
        end
    end
end
    ...
    for i = 1, 10 do
        ...
        if v then
            if self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.w - 6/zoom, 40/zoom) then
                ...
                exports.TR_noti:create(string.format("Trasa linii %d została wybrana.", avliableCourses[i].number), "success")
                self:close()
            end
        end
    end
