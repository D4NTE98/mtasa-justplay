...
function Landmowers:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako ogrodnik.", "job")

    guiInfo.map.y = (sy - 200/zoom)/2 + 30/zoom
    exports.TR_jobs:createInformation(jobSettings.name, string.format("Jeździj kosiarką po parku by przyciąć trawę do odpowiedniej wysokości.\n Zapełnienie kosza: 0/%dl", guiInfo.mowerCapacity), guiInfo.map.h + 40/zoom)
...
function Landmowers:updateMap()
    ...
    dxDrawRectangle(mapPos.x - guiInfo.map.w, mapPos.y - guiInfo.map.h, guiInfo.map.oW + guiInfo.map.w * 2, guiInfo.map.h, tocolor(23,25,31, 100))
    dxDrawRectangle(mapPos.x - guiInfo.map.w, mapPos.y + guiInfo.map.oH, guiInfo.map.oW + guiInfo.map.w * 2, guiInfo.map.h, tocolor(23,25,31, 100))
    dxDrawRectangle(mapPos.x - guiInfo.map.w, mapPos.y, guiInfo.map.w, guiInfo.map.oH, tocolor(23,25,31, 100))
    dxDrawRectangle(mapPos.x + guiInfo.map.oW, mapPos.y, guiInfo.map.w, guiInfo.map.oH, tocolor(23,25,31, 100))
            ...
            if self.grid[col][row] then
                if self.capacity + 0.1 < guiInfo.mowerCapacity and not self.emptyMower and not self.blockGetting then
                    if getDistanceBetweenPoints2D(guiInfo.map.w/2, guiInfo.map.h/2, mapPos.x + (col - 1) * guiInfo.map.segmentSize + guiInfo.map.segmentSize/2, mapPos.y + (row - 1) * guiInfo.map.segmentSize + guiInfo.map.segmentSize/2) < 8 then
                        ...
                        exports.TR_jobs:createInformation(jobSettings.name, string.format("Jeździj kosiarką po parku by przyciąć trawę do odpowiedniej wysokości.\n Zapełnienie kosza: %.1f/%dl", self.capacity, guiInfo.mowerCapacity), guiInfo.map.h + 40/zoom)
                    end
                end
            end
        end
    ...
    for col = math.max(nearCol - 9, 1), nearCol + 9 do
        for row = math.max(nearRow - 6, 1), nearRow + 6 do
            if self.grid[col][row] then
                dxDrawRectangle(mapPos.x + (col - 1) * guiInfo.map.segmentSize, mapPos.y + (row - 1) * guiInfo.map.segmentSize, guiInfo.map.segmentSize, guiInfo.map.segmentSize, tocolor(23,25,31, 100))
            end
        end
    end
    ...
    if self.capacity + 0.1 >= guiInfo.mowerCapacity and not self.emptyMower then
        ...
        guiInfo.map.y = (sy - 200/zoom)/2 + 30/zoom
        exports.TR_jobs:createInformation(jobSettings.name, "Kosz jest w pełni zapełniony. Udaj się do punktu wysypki aby wysypać skoszoną trawę.", guiInfo.map.h + 40/zoom)
    end
...
function Landmowers:createEmptyPoint()
    ...
    setElementData(self.marker, "markerData", {
        title = "Punkt zsypu",
        desc = "Wjedź w marker aby opróżnić.",
    }, false)
    setElementData(self.marker, "markerIcon", "landmower", false)

    exports.TR_jobs:setPlayerTargetPos(guiInfo.emptyPoint.x, guiInfo.emptyPoint.y, guiInfo.emptyPoint.z, 0, 0, "Opróżnij kosz")
...
function Landmowers:emptyMarkerHit(el, md)
    ...
    exports.TR_jobs:createInformation(jobSettings.name, string.format("Trwa wysypywanie trawy...", guiInfo.mowerCapacity), guiInfo.map.h + 10/zoom)
    exports.TR_jobs:setPlayerTargetPos(false)
...
function Landmowers:onExitLandmower(plr)
    ...
    self.blockMap = true
    exports.TR_jobs:createInformation(jobSettings.name, "Aby kontynuować pracę wróć do kosiarki.")
end
...
function Landmowers:shapeLeave(...)
    ...
    exports.TR_noti:create("Nie możesz wyjechać poza strefę koszenia.", "error")
end
