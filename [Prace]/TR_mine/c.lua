...
function Mine:constructor(...)
    ...
    setElementData(self.markers.rockDrop, "markerData", {
        title = "Zrzut kamieni",
        desc = "Wejdź w marker aby wrzucić kamień na taśmociąg.",
    }, false)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako górnik.", "job")
    return true
end
...
function Mine:setStage(...)
    ...
    if self.state == "getDynamite" then
        exports.TR_jobs:setPlayerTargetPos(567.42248535156, 818.82946777344, -29.482440948486, 0, 0, "Podnieś dynamity")
        exports.TR_jobs:createInformation(jobSettings.name, "Udaj się do składu materiałów wybuchowych i zabierz dynamity.")
        exports.TR_jobs:setPaymentTime()

    elseif self.state == "placeDynamite" then
        exports.TR_jobs:setPlayerTargetPos(489.6328125, 779.7353515625, -21.5938247680, 0, 0, "Umieść ładunki")
        exports.TR_jobs:createInformation(jobSettings.name, "Podejdź do ściany wydobywczej aby umieścić ładunki.")

    elseif self.state == "placingDynamite" then
        exports.TR_jobs:setPlayerTargetPos(false)
        exports.TR_jobs:createInformation(jobSettings.name, "Włóż dynamity w szczeliny w ścianie wydobywczej.")
    ...
    elseif self.state == "detonate" then
        exports.TR_jobs:setPlayerTargetPos(513.1025390625, 817.7216796875, -23.931089401245, 0, 0, "Odsuń się od ściany")
        exports.TR_jobs:createInformation(jobSettings.name, "Odsuń się od ściany aby zdetonować ładunki wybuchowe.")
    ...
    elseif self.state == "getRock" then
        ...
        exports.TR_jobs:setPlayerTargetPos(491.921875, 781.9775390625, -22.074180603027, 0, 0, "Podnieś kamień")
        exports.TR_jobs:createInformation(jobSettings.name, "Zabierz rozłupany ze ściany kamień.")

    elseif self.state == "throwRock" then
        self.pickedRock = nil
        self.createdRocks = true
        exports.TR_jobs:setPlayerTargetPos(562.787109375, 822.814453125, -22.727143859863, 0, 0, "Wyrzuć kamień")
        exports.TR_jobs:createInformation(jobSettings.name, "Wyrzuć kamień na taśmociąg.")
    end
end
