...
function Containers:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako przewoźnik kontenerów.", "job")
    exports.TR_jobs:createInformation(jobSettings.name, "Podjedź pod magazyn aby załadować kontener na pakę.")
...
function Containers:destroy()
    ...
    exports.TR_hud:setRadarCustomLocation("Wnętrze budynku | Container Delivery", true)
...
function Containers:createDeliveryPoint()
    ...
    exports.TR_jobs:createInformation(jobSettings.name, string.format("Dowieź kontener w jak najlepszym stanie do klienta.\n Uszkodzenia ładunku:"), 80/zoom)
    exports.TR_jobs:setPlayerTargetPos(guiInfo.marker.defPos.x, guiInfo.marker.defPos.y, guiInfo.marker.defPos.z - 2, 0, 0, "Dowieź kontener do klienta")
...
function Containers:createLoadingPoints()
    ...
    for i, v in pairs(guiInfo.loadingPoints) do
        ...
        setElementData(marker, "markerData", {
            ...
            desc = "Wjedź tyłem aby załadować kontener.",
        }, false)
    ...
    exports.TR_jobs:setPlayerTargetPos(-1847.65625, 107.9453125, 14.96875, 0, 0, "Załaduj kontener na bazie")
...
function Containers:hitLoadingPoint(source, ...)
    ...
    exports.TR_noti:create("Aby się załadować ustaw się tyłem do bramy.", "error")
end
...
function Containers:hitUnloadingPoint()
    ...
    if self:isInPlace() then
        if not isElementFrozen(veh) then
            exports.TR_noti:create("Zaciągnij ręczny aby oddać ładunek.", "info")
            return
        end
    ...
    exports.TR_noti:create("Aby rozładować ciężarówkę musisz ustawić się kontenerem pod hakiem suwnicy.", "error")
end
...
function Containers:setLoadingAnim(veh, rot)
    ...
    exports.TR_jobs:createInformation(jobSettings.name, "Trwa ładowanie kontenera na pojazd.")
end
...
function Containers:setUnloadingAnim(veh)
    ...
    self:removeMarkers()
    exports.TR_jobs:createInformation(jobSettings.name, "Trwa rozładowywanie kontenera z pojazdu.")
    self.goBack = true
end
...
function Containers:stopAnim()
    ...
    elseif self.animType == "unloading" then
        ...
        exports.TR_jobs:createInformation(jobSettings.name, "Wróć na bazę aby ponownie się załadować i odebrać wynagrodzenie za trasę.")
