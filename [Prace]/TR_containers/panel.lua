...
function TargetPanel:render()
    ...
    dxDrawText("Wybierz transport", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    ...
    for i = 1, 12 do
        ...
        if v then
            ...
            if not self.hasLongRoads and v.isFar then
                ...
                dxDrawText("BRAK UPRAWNIEŃ", guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 16/zoom, guiInfo.y + 50/zoom + 40/zoom * i, tocolor(170, 170, 170, 60 * self.alpha), 1/zoom, self.fonts.small, "right", "center")
            ...
            elseif v.taken then
                ...
                if time <= 0 then
                    ...
                    dxDrawText("ŁADUNEK GOTOWY", guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 16/zoom, guiInfo.y + 50/zoom + 40/zoom * i, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.small, "right", "center")

                else
                    -- dxDrawLine(guiInfo.x + 10/zoom, guiInfo.y + 69/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 69/zoom + 40/zoom * (i-1), tocolor(170, 170, 170, 60 * self.alpha), 2)
                    dxDrawText(string.format("%s | %s", city, zone), guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom + 40/zoom * i, tocolor(170, 170, 170, 60 * self.alpha), 1/zoom, self.fonts.text, "left", "center")

                    dxDrawText("ŁADUNEK ZA "..self:secondsToClock(time), guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 16/zoom, guiInfo.y + 50/zoom + 40/zoom * i, tocolor(170, 170, 170, 60 * self.alpha), 1/zoom, self.fonts.small, "right", "center")
                end
