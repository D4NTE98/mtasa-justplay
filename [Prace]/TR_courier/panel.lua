...
function TargetPanel:constructor(...)
    ...
    self.buttons = {}
    self.buttons.startJob = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 260/zoom, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Załaduj paczki", "green")
...
function TargetPanel:render()
    ...
    dxDrawText("Wybierz paczki do doręczenia", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    dxDrawText(string.format("Ilość zabieranych paczek: %d/%d", self.selectedPackages, self.maxPackages), guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 10/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.text, "left", "center")
    ...
    for i = 1, 10 do
        ...
        if v then
            if v.tick then
                ...
                dxDrawText("ŁADUNEK ZA "..self:secondsToClock(time), guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 16/zoom, guiInfo.y + 50/zoom + 40/zoom * i, tocolor(120, 120, 120, 100 * self.alpha), 1/zoom, self.fonts.small, "right", "center")
                ...
                dxDrawText("ŁADUNEK GOTOWY", guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 16/zoom, guiInfo.y + 50/zoom + 40/zoom * i, color, 1/zoom, self.fonts.small, "right", "center")
            end
        end
    end
...
function TargetPanel:buttonClick(btn)
    if btn == self.buttons.startJob then
        ...
        if self.selectedPackages <= 0 then exports.TR_noti:create("Nie możesz wyjechać bez żadnej paczki.", "error") return end
        if self.selectedPackages > self.maxPackages then exports.TR_noti:create("Masz zbyt dużo wybranych paczek do dostarczenia.", "error") return end
