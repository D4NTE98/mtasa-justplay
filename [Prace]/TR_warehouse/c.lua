...
local guiInfo = {
    ...
    boxPoints = {
        ...
        },
        {
            ...
            name = "Regał z cukrem",
            plural = "kartonami cukru",
            texture = "sugar",
        },
    },

    clickTimeInterval = {30, 40},
...
function Warehouse:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako magazynier.", "job")
    exports.TR_jobs:createInformation(jobSettings.name, "Przebierz się w ubranie robocze.")
    exports.TR_jobs:setPlayerTargetPos(2254.0112304688, -221.82743835449, 96.182502746582, 5, 5, "Użyj interakcji (e) i naciśnij na szafkę aby się przebrać")
...
function Warehouse:endJobForce()
    if getElementInterior(localPlayer) ~= 5 and getElementDimension(localPlayer) ~= 5 then
        ...
        exports.TR_noti:create("Opuściłeś miejsce pracy. Zostałeś automatycznie zwolniony.", "info")
...
function Warehouse:render()
    ...
    elseif self.selectedKey then
        if (getTickCount() - self.tick)/2000 >= 1 then
            ...
            exports.TR_noti:create("Straciłeś równowagę i upuściłeś paczkę.", "error")
    ...
    dxDrawText("Aby nie upuścić paczki naciskaj odpowiednie\nprzyciski gdy będziesz tracić równowagę.", guiInfo.center.x, guiInfo.center.y + guiInfo.center.h + 10/zoom, guiInfo.center.x + guiInfo.center.w, guiInfo.center.y, tocolor(255, 255, 255, 255), 1/zoom, self.fonts.main, "center", "top")
end
...
function Warehouse:createDeliveryTruck(blockMsg)
    ...
    exports.TR_jobs:createInformation(jobSettings.name, "Nadjeżdża samochód dostawczy. Przygotuj się do jego rozładunku.")
end
...
function Warehouse:waitForNextTruck()
    ...
    exports.TR_jobs:createInformation(jobSettings.name, "Poczekaj na kolejną cieżarówkę z dostawą. Powinna się za chwilę zjawić na miejscu.")
end
...
function Warehouse:createVehicleMarker()
    ...
    if guiInfo.skins[getElementModel(localPlayer)] then
        exports.TR_jobs:createInformation(jobSettings.name, string.format("Rozładuj samochód. \nRozładowane paczki: %d/%d.", self.taken, self.toTake))
    end
    ...
    setElementData(self.takeBoxMarker, "markerData", {
        title = "Strefa rozładunku",
        desc = "Zabierz paczkę i zanieś ją w odpowiednie miejsce."
    }, false)
...
function Warehouse:takeBoxHit(el, md)
    ...
    if self:getElementSpeed(localPlayer, 1) > 25 then exports.TR_noti:create("Podejdź na spokojnie po paczkę, nie biegaj bo jeszcze ją zrzucisz zanim weźmiesz.", "info") return end

    self.taken = self.taken + 1
    if self.taken > self.toTake then
        self.timer = setTimer(self.func.closeVehicleTrunk, 5000, 1)
        exports.TR_jobs:setPlayerTargetPos(false)
        exports.TR_jobs:createInformation(jobSettings.name, "Kierowca wypisze tylko papiery ukończenia dostawy i odjedzie ze stanowiska.")
    ...
    exports.TR_jobs:createInformation(jobSettings.name, "Zanieś paczkę na odpowiedni regał. Postaraj się jej nie upuścić. \nZawartość paczki:", 170/zoom)
end
...
function Warehouse:removeBox(angry)
    ...
    if self.taken == self.toTake then
        exports.TR_jobs:createInformation(jobSettings.name, "Poinformuj kierowcę, że wszystkie paczki zostały rozładowane.")
        exports.TR_jobs:setPlayerTargetPos(2255.5432128906, -199.31307983398, 95.681617736816, 5, 5, "Podejdź aby porozmawiać z kierowcą")
    else
        exports.TR_jobs:createInformation(jobSettings.name, string.format("Rozładuj samochód. \nRozładowane paczki: %d/%d.", self.taken, self.toTake))
        exports.TR_jobs:setPlayerTargetPos(2255.5432128906, -199.31307983398, 95.681617736816, 5, 5, "Podejdź aby rozładować paczkę")
    end
...
function Warehouse:createDeliveryPoint(data)
    ...
    setElementData(self.deliveryMarker, "markerData", {
        title = data.name,
        desc = string.format("Tutaj należy odłożyć paczkę z %s.", data.plural),
    }, false)
    ...
    exports.TR_jobs:setPlayerTargetPos(data.pos.x, data.pos.y, data.pos.z - 0.5, 5, 5, "Zanieś paczkę na odpowiedni regał")
    exports.TR_jobs:setPaymentTime()
...
function Warehouse:takeoutBoxHit(source, el, md)
    ...
    if type ~= self.boxType then
        exports.TR_noti:create("Paczka została odłożona na złe miejsce. Nie otrzymujesz żadnego wynagrodzenia.", "error")
        exports.TR_jobs:resetPaymentTime()
    else
