...
local guiInfo = {
    ...
    fuelDelivery = {
        ...
        Vector3(448.185546875, 1548.5595703125, 11.49),
...
function FuelDelivery:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako transporter paliwowy.", "job")
...
function FuelDelivery:startJob()
    ...
    exports.TR_jobs:resetPaymentTime()
    exports.TR_jobs:createInformation(jobSettings.name, "Podjedź pod szyb naftowy, wyciągnij wąż ze swojej cysterny i podepnij się do rafinerii aby napełnić swój zbiornik.", guiInfo.vehDmg.h)
    exports.TR_jobs:setPlayerTargetPos(495.7109375, 1421.2158203125, 3.7537870407104, 0, 0, "Napełnij cysternę")
    ...
    if getElementData(localPlayer, "hoseEndPos") then
        if self.state == "loadOil" then
            ...
            if self.inTank == self.maxInTank then
                ...
                self:removeMarkers()
                exports.TR_jobs:createInformation(jobSettings.name, "Wróć do bazy aby rozładować zebraną ropę i załadować benzynę.", guiInfo.vehDmg.h)
                exports.TR_jobs:setPlayerTargetPos(2695.478515625, 2743.9921875, 11.5203125, 0, 0, "Rozładuj cysternę")
        ...
        elseif self.state == "unloadOil" then
            ...
            if self.inTank == 0 then
                ...
                exports.TR_jobs:createInformation(jobSettings.name, "Trwa napełnianie benzyną...", guiInfo.vehDmg.h)
                exports.TR_jobs:setPlayerTargetPos(false)
            end
...
function FuelDelivery:findNextDeliveryPoint()
    ...
    guiInfo.vehDmg.y = (sy - 102/zoom)/2 + 30/zoom
    exports.TR_jobs:createInformation(jobSettings.name, "Udaj się na stację i rozładuj ładunek, aby z dystrybutorów znów mogła lać się świeżutka benzyna.", guiInfo.vehDmg.h)
    exports.TR_jobs:setPlayerTargetPos(pos.x, pos.y, pos.z + 1, 0, 0, "Dostarcz benzynę")
