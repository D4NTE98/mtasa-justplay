...
local guiInfo = {
    tickets = {
        ...
        studentsTexts = {"Chciałbym bilet ulgowy."},
        standardTexts = {"Chciałbym bilet normalny."},
    },

    hourEarning = {4939393, 49494949494},
...
function Train:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako maszynista.", "job")
    ...
    triggerServerEvent("createTrainJobVehicle2", resourceRoot)
    return true
end
...
function Train:createNextMarker()
    self:removeMarker()
    ...
    setElementData(self.marker, "markerData", {
        title = "Stacja",
        desc = "Zatrzymaj się na stacji aby zabrać pasażerów.",
    }, false)

    self.blip = createBlip(pos.marker, 0, 2, 255, 60, 60, 255)
    setElementData(self.blip, "icon", 22, false)

    exports.TR_jobs:setPlayerTargetPos(pos.marker.x, pos.marker.y, pos.marker.z - 0.2, 0, 0, "Zatrzymaj się na stacji.")
    exports.TR_jobs:createInformation(jobSettings.name, "Udaj się na kolejne miejsce postoju aby zabrać pasażerów z stacji.")
...
function Train:onMarkerHit(plr, md)
    if plr ~= localPlayer or not md then return end

    self.noti = exports.TR_noti:create("Zatrzymaj się aby zabrać ludzi z stacji.", "info", 10, true)
    self.checker = setTimer(self.func.checkStop, 100, 0)
end

function Train:onMarkerLeave(plr, md)
    if plr ~= localPlayer or not md then return end
    if isTimer(self.checker) then killTimer(self.checker) end
...
function Train:checkStop()
    ...
    if speed < 2 then
        ...
        exports.TR_jobs:createInformation(jobSettings.name, "Poczekaj aż pasażerowie wsiądą i wysiądą.")
        self.timer = setTimer(self.func.moveAllNpcsInside, 10000, 1)
    end
end

function Train:setNpcGoOut()
...
function Train:moveAllNpcsInside()
    for i, v in pairs(self.npcs) do
...
function Train:openTicketGui()
    ...
    exports.TR_jobs:createInformation(jobSettings.name, "Wydaj pasażerom odpowiednie bilety.")
end

function Train:closeTicketGui()
    self.alpha = 1
    self.state = "closing"
    self.tick = getTickCount()
    self.blockMove = nil
...
function Train:render()
    ...
    self:drawBackground(guiInfo.tickets.x, guiInfo.tickets.y, guiInfo.tickets.w, guiInfo.tickets.h, tocolor(16,18,23, 255 * self.alpha), 15)
    dxDrawText("Wybierz odpowiedni bilet", guiInfo.tickets.x, guiInfo.tickets.y, guiInfo.tickets.x + guiInfo.tickets.w, guiInfo.tickets.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    ...
    if not self.ticketRequest or self.ticketsToGive == 0 then
        dxDrawText("Poczekaj aż kolejny pasażer wsiądzie do pojazdu.", guiInfo.tickets.x + 10/zoom, guiInfo.tickets.y + 275/zoom, guiInfo.tickets.x + guiInfo.tickets.w - 10/zoom, guiInfo.tickets.y + guiInfo.tickets.h, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.text, "center", "center", true, true)
    else
        dxDrawText(string.format("Pasażer:\n%s", self.ticketRequest.text), guiInfo.tickets.x + 10/zoom, guiInfo.tickets.y + 275/zoom, guiInfo.tickets.x + guiInfo.tickets.w - 10/zoom, guiInfo.tickets.y + guiInfo.tickets.h, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.text, "center", "center", true, true)
    end
end

function Train:moveNpcsToEnter()
    local x, y, z = self:getPosition(getPedOccupiedVehicle(localPlayer), Vector3(0, 2, 0))
    local count = 0
    for i, v in pairs(self.npcs) do
        local pos = Vector3(getElementPosition(v))
...
function Train:onMouseClick(...)
    ...
    if self:isMouseInPosition((sx - 330/zoom)/2, guiInfo.tickets.y + 50/zoom, 330/zoom, 112/zoom) then
        if self.ticketRequest.type == "student" then
            exports.TR_noti:create("Wydałeś poprawny bilet.", "success")
            self.goodTickets = self.goodTickets + 1
            usedTicket = true
        else
            exports.TR_noti:create("Wydałeś niepoprawny bilet.", "error")
...
function Train:setTicketRequest()
...
function Train:calculatePayment()
...
-- Utils
function Train:drawBackground(x, y, rx, ry, color, radius, post)
    rx = rx - radius * 2
    ry = ry - radius * 2
    x = x + radius
    y = y + radius
...
function Train:getPosition(element, vec)
...
function Train:setElementSpeed(element, unit, speed)
...
function Train:getElementSpeed(theElement, unit)
...
function Train:getPointFromDistanceRotation(x, y, dist, angle)
...
function Train:findRotation( x1, y1, x2, y2 )
    local t = -math.deg( math.atan2( x2 - x1, y2 - y1 ) )
    return t < 0 and t + 360 or t
end

function Train:isMouseInPosition(x, y, width, height)
...
function Train:isEventHandlerAdded( sEventName, pElementAttachedTo, func )
...
addEvent("endTrainWork", true)
addEventHandler("endTrainWork", root, endJob)