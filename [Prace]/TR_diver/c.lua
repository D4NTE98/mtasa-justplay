...
function Diver:constructor(...)
    ...
    guiInfo.fontsSizes.oxygen = dxGetTextWidth("Tlen:", 1/zoom, self.fonts.main)
    guiInfo.fontsSizes.capacity = dxGetTextWidth("Pojemność:", 1/zoom, self.fonts.main)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako nurek.", "job")
    exports.TR_jobs:createInformation(jobSettings.name, "Zanurkuj w poszukiwaniu zatopionych przedmiotów. Jeśli zbierzesz minimum 25kg możesz wrócić na bazę i oddać je kapitanowi.")
...
function Diver:enterOxygenMarker(el, md)
    ...
    exports.TR_jobs:createInformation(jobSettings.name, "Trwa uzupełnianie butli tlenem...")
    ...
    setTimer(function()
        ...
        if self.capacity == self.maxCapacity then
            exports.TR_jobs:createInformation(jobSettings.name, "Nie udźwigniesz już więcej. Wróć na bazę odłożyć znaleziska.")
            exports.TR_jobs:setPlayerTargetPos(-913.8994140625, 2686.330078125, 41.3702621459966, 0, 0, "Odłóż skarby")
        else
            exports.TR_jobs:createInformation(jobSettings.name, "Zanurkuj w poszukiwaniu zatopionych przedmiotów. Jeśli zbierzesz minimum 25kg możesz wrócić na bazę i oddać je kapitanowi.")
        end

        exports.TR_noti:create("Poziom tlenu został uzupełniony.", "success")
    end, 5000, 1)
end

function Diver:enterCapacityMarker(el, md)
    if not el or not md or el ~= localPlayer then return end
    if self.capacity < 25 then exports.TR_noti:create("Aby móc odebrać wynagrodzenie musisz przynieść minimum 25kg.", "info") return end
    self.capacity = 0

    setElementData(localPlayer, "animation", {"BOMBER", "BOM_Plant"})
    exports.TR_jobs:createInformation(jobSettings.name, "Trwa wyładowywanie zebranych łupów...")
...
function Diver:enterTargetMarker(el, md)
    ...
    if pointRand == 1 then
        ...
        exports.TR_noti:create("W środku znalazłeś stare łachmany.", "success")
        self:createPoint()

    elseif pointRand <= 5 then
        triggerServerEvent("giveDivingJobItem", resourceRoot, "gold")
        exports.TR_noti:create("W środku znalazłeś złoty mieszek.", "success")
        self:createPoint()

    elseif pointRand <= 50 then
        triggerServerEvent("giveDivingJobItem", resourceRoot, "silver")
        exports.TR_noti:create("W środku znalazłeś srebrną szkatułkę.", "success")
        self:createPoint()
        ...
        exports.TR_noti:create(string.format("Wydobyłeś skrzynię ważącą %.2fkg.", kgs), "success")
    ...
    triggerServerEvent("givePlayerJobPoints", resourceRoot, getResourceName(getThisResource()), 1, "punkt")
end
...
function Diver:createMarkers()
    ...
    setElementData(self.oxygenRefill, "markerData", {
        title = "Punkt napełniania butli",
        desc = "Wejdź w marker aby napełnić butlę tlenem.",
    }, false)
    ...
    setElementData(self.takoutTrash, "markerData", {
        title = "Punkt odkładania skarbów",
        desc = "Wejdź w marker aby odłożyć zebrane przez siebie przedmioty.",
    }, false)
...
function Diver:createPoint()
    ...
    exports.TR_jobs:setPlayerTargetPos(randPos.x, randPos.y, randPos.z, 0, 0, "Zbierz skrzynkę")
end
...
function Diver:render()
    ...
    dxDrawText("Tlen:", guiInfo.hud.x + guiInfo.hud.w + 10/zoom, guiInfo.hud.y, guiInfo.hud.x + guiInfo.hud.w + 10/zoom, guiInfo.hud.y + guiInfo.hud.h, tocolor(255, 255, 255, 255), 1/zoom, self.fonts.main, "left", "center", false, false, false, true)
    ...
    dxDrawText("Pojemność:", guiInfo.hud.x + guiInfo.hud.w + 10/zoom, guiInfo.hud.y + guiInfo.hud.h, guiInfo.hud.x + guiInfo.hud.w + 10/zoom, guiInfo.hud.y + guiInfo.hud.h * 2, tocolor(255, 255, 255, 255), 1/zoom, self.fonts.main, "left", "center", false, false, false, true)
    ...
    if plrPos.z < 40 then
        if self.oxygen <= 0 then
            ...
            exports.TR_noti:create("Tlen w butli się skończył. Jezioro szczęśliwie wyrzuciło cię na brzeg, lecz straciłeś cały zdobyty łup.", "info", 10)

            exports.TR_jobs:createInformation(jobSettings.name, "Uzupełnij butlę z tlenem aby móc powrócić do pracy.")
            exports.TR_jobs:setPlayerTargetPos(-898.8828125, 2686.923828125, 41.3702621459966, 0, 0, "Napełnij butlę tlenem")
