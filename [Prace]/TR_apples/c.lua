...
function Apples:constructor(...)
    ...
    exports.TR_noti:create("Rozpocząłeś pracę jako sadownik.", "job")
    exports.TR_jobs:createInformation(jobSettings.name, "Weź koszyk z Waltona znajdującego się na wjeździe do sadu.")
...
function Apples:blockVehicleEnter(plr)
    ...
    cancelEvent()
    exports.TR_noti:create("Nie możesz wsiąść do tego pojazdu podczas pracy.", "error")
end
...
function Apples:createMarkers()
    ...
    setElementData(self.basketMarker, "markerData", {
        title = "Bagażnik waltona",
        desc = "Podejdź aby wziąć koszyk.",
    }, false)
    ...
    for i, v in pairs(guiInfo.markers) do
        ...
        setElementData(marker, "markerData", {
            title = "Jabłonka",
            desc = "Podejdź aby zerwać jabłka.",
        }, false)
    ...
    exports.TR_jobs:setPlayerTargetPos(1016.2822265625, -337.43801879883, 74.2921875 - 0.5, 0, 0, "Weź koszyk z jabłkami.")
...
function Apples:enterBasketMarker(plr, md)
    ...
    if self.appleCount < self.appleToGet and self.hasBasket then
        triggerEvent("showCustomMessage", localPlayer, "Wieslaw Norter", "Co mi taki pusty koszyk przynosisz? Do pełna ładuj!", "files/images/npc.png")
        return
    end
    ...
    exports.TR_jobs:createInformation(jobSettings.name, string.format("Udaj się pod wybraną jabłonkę, aby zebrać z niej jabłka.\n Pojemność koszyka: %d/%d", self.appleCount, self.appleToGet))
...
function Apples:payForJob()
    ...
    if payment <= 0 then
        exports.TR_noti:create("Przyniosłeś zbyt dużo zepsutych jabłek. Kwota zarobionych pieniędzy wyniosła $0.", "error")
        return
    end
...
function Apples:closeTree()
    ...
    if self.appleCount >= self.appleToGet then
        exports.TR_jobs:createInformation(jobSettings.name, "Masz już pełen koszyk. Udaj się do samochodu aby wziąć nowy.")
        exports.TR_jobs:setPlayerTargetPos(1016.2822265625, -337.43801879883, 74.2921875 - 0.5, 0, 0, "Odłóż koszyk z jabłkami.")
    else
        exports.TR_jobs:createInformation(jobSettings.name, string.format("Udaj się pod wybraną jabłonkę, aby zebrać z niej jabłka.\n Pojemność koszyka: %d/%d", self.appleCount, self.appleToGet))
    end
...
function Apples:render()
    ...
    dxDrawText("Naciśnij SHIFT aby odejść od drzewa.", 0, 0, sx, sy - 14/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.exit, "center", "bottom")
end
    ...
    if arg[1] == "left" then
        if arg[2] == "down" then
            ...
            if self:isMouseInPosition(guiInfo.box.x + 60/zoom, guiInfo.box.y + 10/zoom, guiInfo.box.w - 120/zoom, guiInfo.box.h - 30/zoom) then
                ...
                if self.appleCount >= self.appleToGet then
                    self:moveAppleToDefaultPosition()
                    exports.TR_noti:create("Twój koszyk jest pełny i nie pomieści już żadnego jabłka.", "error")
                    return
                end
