...
function ShowInfo:render()
    ...
    self:drawBackground(x, y, w, h, tocolor(23,25,31, 255 * self.alpha), 14)

    if self.showingData.type == "constant" then
        local size = (h - 30/zoom) * self.progress.upper
        dxDrawImage((x + w/2) - size/2 - (guiInfo.w - size - 20/zoom)/2 * self.progress.leftAlpha, (y + h/2) - size/2, size, size, self.textures.calendar, 0, 0, 0, tocolor(44,181,233, 255 * self.progress.upperAlpha * self.alpha))
    ...
    elseif self.showingData.type == "end" then
        ...
        dxDrawImage((x + w/2) - size/2 - (guiInfo.w - size - 20/zoom)/2 * self.progress.leftAlpha, (y + h/2) - size/2, size, size, self.textures.sad, 0, 0, 0, tocolor(44,181,233, 255 * self.progress.upperAlpha * self.alpha))
    end

    dxDrawText(self.showingData.title, guiInfo.x + 100/zoom, guiInfo.y + 5/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 20/zoom, tocolor(44,181,233, 255 * self.progress.textAlpha * self.alpha), 1/zoom, self.fonts.main, "center", "top")
    ...
    if self.showingData.type == "end" then
        dxDrawText(self.showingData.gift or "", guiInfo.x + 100/zoom, guiInfo.y + 40/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 10/zoom, tocolor(44,181,233, 255 * self.progress.textAlpha * self.alpha), 1/zoom, self.fonts.desc, "center", "bottom")
    else
        dxDrawText(self.showingData.gift or "", guiInfo.x + 100/zoom, guiInfo.y + 40/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 10/zoom, tocolor(28,89,119, 255 * self.progress.textAlpha * self.alpha), 1/zoom, self.fonts.desc, "center", "bottom")
    end
end
