...
function Trunk:setHeaderText()
    if self.type == 0 then
        self.header = "Ekwipunek gracza"

    elseif self.type == 1 then
        self.header = "Zawartość bagażnika"

    elseif self.type == 2 then
        self.header = "Zawartość sejfu"

    elseif self.type == 20 then
        self.header = "Zawartość skrzyń"
    end
end
...
function Trunk:getMaxSize(model)
    ...
    elseif self.type == 1 then
        return 100000--exports.TR_hud:getVehicleTrunkCapacity(model) * 1000
...
function Trunk:render()
    ...
    dxDrawText(self.header or "Schowane przedmioty", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 40/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "center")
    ...
    for i = 1, 7 do
        ...
        if item then
            ...
            if not item.used then
                ...
                dxDrawImage(guiInfo.x + 15/zoom, guiInfo.y + 40/zoom + (i - 1) * 60/zoom, 50/zoom, 50/zoom, item.icon, 0, 0, 0, tocolor(44,181,233, alpha * self.alpha))
                dxDrawText(string.format("%s %s", item.name, item.stackable and "("..item.value2..")" or ""), guiInfo.x + 75/zoom, guiInfo.y + 43/zoom + (i - 1) * 60/zoom, guiInfo.x + guiInfo.w - 85/zoom, guiInfo.y + 70/zoom + (i - 1) * 60/zoom, tocolor(220, 220, 220, alpha * self.alpha), 1/zoom, self.fonts.name, "left", "top", true)
                dxDrawText(item.description, guiInfo.x + 75/zoom, guiInfo.y + 60/zoom + (i - 1) * 60/zoom, guiInfo.x + guiInfo.w - 85/zoom, guiInfo.y + 85/zoom + (i - 1) * 60/zoom, tocolor(150, 150, 150, alpha * self.alpha), 1/zoom, self.fonts.category, "left", "bottom", true)
            end
    ...
    if #self.items > 7 then
        ...
        dxDrawRectangle(guiInfo.x + guiInfo.w - 5/zoom, guiInfo.y + 40/zoom, 4/zoom, 290/zoom, tocolor(34,36,43, 255 * self.alpha))
        dxDrawRectangle(guiInfo.x + guiInfo.w - 5/zoom, guiInfo.y + 40/zoom + barY, 4/zoom, barHeight, tocolor(42,45,51, 255 * self.alpha))
    else
        dxDrawRectangle(guiInfo.x + guiInfo.w - 5/zoom, guiInfo.y + 40/zoom, 4/zoom, (guiInfo.h - 80/zoom), tocolor(34,36,43, 255 * self.alpha))
    end
    ...
    dxDrawLine(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 40/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 40/zoom, tocolor(44,181,233, 255 * self.alpha))
    dxDrawText(string.format("Zapełnienie: %.3fkg / %dkg", self.weight/1000, self.maxWeight/1000), guiInfo.x + 35/zoom, guiInfo.y + guiInfo.h - 40/zoom, guiInfo.x + guiInfo.w - 15/zoom, guiInfo.y + guiInfo.h, tocolor(150, 150, 150, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "center", false, false, true, true)
end
    ...
    elseif btn == "mouse1" then
        for i = 1, 7 do
            ...
            if item then
                if self:isMouseInPosition(guiInfo.x + guiInfo.w - 40/zoom, guiInfo.y + 60/zoom + (i - 1) * 60/zoom, 20/zoom, 20/zoom) then
                    if item.used then
                        exports.TR_noti:create("Nie możesz zabrać tego przedmiotu.", "error")
                        return
                    end
...
function Trunk:canInsertItem(item)
    ...
    if item.type == 10 then
        if getElementData(localPlayer, "weapon:used") then
            exports.TR_noti:create("Nie możesz schować tego przedmiotu gdy masz wyekwipowaną broń!", "error")

            local time = getRealTime()
            local tf = string.format("[%02d:%02d %02d.%02d]", time.hour, time.minute, time.monthday, time.month)
            exports.TR_discord:remoteDsc(tf.." | "..string.format("**%s** próbował schować ammo do bagażnika, mająć użytą broń palną.", getPlayerName(localPlayer)), "ac")

            return false
        end;
    end

    if self.weight + itemWeight >= self.maxWeight then exports.TR_noti:create("Nie możesz schować tego przedmiotu, ponieważ nie ma już miejsca.", "error") return false end
    return true
end
