...
function EggOpener:render()
    self:drawBackground(guiInfo.x - guiInfo.w - 30/zoom, guiInfo.y - 50/zoom, guiInfo.w * 3 + 60/zoom, guiInfo.h + 95/zoom, tocolor(23,25,31, 255), 11)
    dxDrawText("Wybierz prezent", guiInfo.x - guiInfo.w - 30/zoom, guiInfo.y - 50/zoom, guiInfo.x - guiInfo.w - 30/zoom + guiInfo.w * 3 + 60/zoom, guiInfo.y - 10/zoom, tocolor(44,181,233, 255), 1/zoom, self.fonts.title, "center", "center", false, false, false)
    dxDrawText("Wybierz jeden prezent aby otrzymać nagrodę, która się w nim chowa.", guiInfo.x - guiInfo.w - 30/zoom, guiInfo.y - 50/zoom, guiInfo.x - guiInfo.w - 30/zoom + guiInfo.w * 3 + 60/zoom, guiInfo.y + guiInfo.h + 35/zoom, tocolor(170, 170, 170, 255), 1/zoom, self.fonts.info, "center", "bottom", true, true, false)
...
function EggOpener:renderEgg(x, eggID)
    if self:isMouseInPosition(x, guiInfo.y, guiInfo.w, guiInfo.h) and not self.selectedEgg then
        dxDrawImage(x, guiInfo.y, guiInfo.w, guiInfo.h, "files/images/items/gift.png", 0, 0, 0, tocolor(44,181,233, 255))

    elseif self.selectedEgg == eggID then
        dxDrawImage(x, guiInfo.y, guiInfo.w, guiInfo.h, "files/images/items/gift.png", 0, 0, 0, tocolor(44,181,233, 255))

    else
        dxDrawImage(x, guiInfo.y, guiInfo.w, guiInfo.h, "files/images/items/gift.png", 0, 0, 0, tocolor(44,181,233, 180))
    end
end
...
function EggOpener:getEggPrize()
    ...
    if rand <= 27 then
        exports.TR_noti:create("Z prezentu wypadły unikalne neony.", "success")
        return "neon", 0

    elseif rand <= 51 then
        exports.TR_noti:create("Z prezentu wypadło konto Diamond na okres 1 dnia.", "success")
        return "diamond", 1

    elseif rand <= 201 then
        exports.TR_noti:create("Z prezentu wypadło konto Gold na okres 3 dni.", "success")
        return "gold", 1
    else
        local value = math.random(250, 2500)
        exports.TR_noti:create(string.format("Z prezentu wypadło $%d.", value), "success")
        return "money", value
    end
end
