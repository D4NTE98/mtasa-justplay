...
function Trading:createTrade(plr)
    ...
    self.openerNoti = exports.TR_noti:create(string.format("Handel z graczem %s rozpoczęty. Aby kontynuować wymianę zamknij pozostałe okna.", self.tradingData.playerName), "trade", 0, true)
end

function Trading:cancelTrade(info)
    if info then exports.TR_noti:create(string.format("Gracz %s anulował handel.", self.tradingData.playerName), "error") end
    self:close()
end
...
function Trading:tryOpen()
    ...
    if not isElement(self.tradingData.player) then self:removeOpener("Gracz wyszedł z gry. Handel został anulowany.") return end
    if getDistanceBetweenPoints3D(Vector3(getElementPosition(localPlayer)), Vector3(getElementPosition(self.tradingData.player))) > 3 then self:removeOpener("Gracz oddalił się zbyt daleko. Handel został anulowany.") return end
    self:open()
end
...
function Trading:open()
    ...
    self.gui.moneyEdit = exports.TR_dx:createEdit(guiData.x + 10/zoom, guiData.y + guiData.h - 150/zoom, guiData.w/2 - 20/zoom, 40/zoom, "Gotówka", false, self.gui.money)
    self.gui.acceptButton = exports.TR_dx:createButton(guiData.x + 10/zoom, guiData.y + guiData.h - 100/zoom, guiData.w/2 - 20/zoom, 40/zoom, "Zatwierdź")
    self.gui.cancelButton = exports.TR_dx:createButton(guiData.x + 10/zoom, guiData.y + guiData.h - 50/zoom, guiData.w/2 - 20/zoom, 40/zoom, "Anuluj handel")
...
function Trading:setTradeState(state)
    if state == "confirm" then
        ...
        exports.TR_dx:setButtonText(self.gui.acceptButton, "Potwierdź wymianę")
    else
...
function Trading:addItem(plr, item)
    ...
    if index then
        ...
        if #self.items[plr] >= 15 then exports.TR_noti:create("Limit przedmiotów na jeden handel został osiągnięty.", "error") return end
    ...
    if btn == self.gui.acceptButton then
        if self.slowButton then
            if (getTickCount() - self.slowButton)/3000 < 1 then
                exports.TR_noti:create("Status wymiany możesz zmieniać raz na 3 sekundy.", "error")
                return
            end
        end
        ...
        if self.btnState == "cancel" then
            ...
            if tonumber(self.money) < 0 then
                exports.TR_noti:create("Kwota pieniężna jest niepoprawna.", "error")
                return
            end
            ...
            if tonumber(plrData.money) < tonumber(self.money) then
                exports.TR_noti:create("Nie posiadasz przy sobie tyle gotówki.", "error")
                return
            end
            ...
            if self.tradingData.state == "accept" then
                ...
                exports.TR_dx:setButtonText(self.gui.acceptButton, "Anuluj")
            end
        ...
        elseif self.btnState == "accept" then
            self.btnState = "cancel"
            exports.TR_dx:setButtonText(self.gui.acceptButton, "Zatwierdź")
            GUI.eq:setTradeBlock(false)
            GUI.eq:reselectItem()
        ...
        elseif self.btnState == "confirm" then
            ...
            exports.TR_dx:setButtonText(self.gui.acceptButton, "Wymiana potwierdzona")
...
function Trading:render()
    ...
    self:drawBackground(guiData.x, guiData.y, guiData.w, guiData.h, tocolor(23,25,31, 255 * self.alpha), 15)
    dxDrawRectangle(guiData.x + guiData.w/2 - 1/zoom, guiData.y, 2/zoom, guiData.h, tocolor(16,18,23, 255 * self.alpha))

    dxDrawText("Twoja oferta:", guiData.x + 10/zoom, guiData.y, guiData.x + guiData.w - 10/zoom, guiData.y + 40/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.title, "left", "center")
    dxDrawText(string.format("Oferta %s:", self.tradingData.playerName), guiData.x + guiData.w/2 + 10/zoom, guiData.y, guiData.x + guiData.w - 10/zoom, guiData.y + 40/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.title, "left", "center", true)

    -- Acceptation status
    self:drawBackground(guiData.x + guiData.w/2 + 10/zoom, guiData.y + guiData.h - 50/zoom, guiData.w/2 - 20/zoom, 40/zoom, tocolor(16,18,23, 255 * self.alpha), 5)
    if self.tradingData.state == "accept" then
        dxDrawText("Zatwierdzone", guiData.x + guiData.w/2, guiData.y + guiData.h - 50/zoom, guiData.x + guiData.w - 10/zoom, guiData.y + guiData.h - 10/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "center")
    elseif self.tradingData.state == "confirm" then
        dxDrawText("Brak potwierdzenia", guiData.x + guiData.w/2, guiData.y + guiData.h - 50/zoom, guiData.x + guiData.w - 10/zoom, guiData.y + guiData.h - 10/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "center")
    elseif self.tradingData.state == "confirmed" then
        dxDrawText("Wymiana potwierdzona", guiData.x + guiData.w/2, guiData.y + guiData.h - 50/zoom, guiData.x + guiData.w - 10/zoom, guiData.y + guiData.h - 10/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "center")
    else
        dxDrawText("Nie zatwierdzone", guiData.x + guiData.w/2, guiData.y + guiData.h - 50/zoom, guiData.x + guiData.w - 10/zoom, guiData.y + guiData.h - 10/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "center")
    end
    -- Money status
    self:drawBackground(guiData.x + guiData.w/2 + 10/zoom, guiData.y + guiData.h - 100/zoom, guiData.w/2 - 20/zoom, 40/zoom, tocolor(16,18,23, 255 * self.alpha), 5)
    self:drawBackground(guiData.x + guiData.w/2 + 10/zoom, guiData.y + guiData.h - 100/zoom, 40/zoom, 40/zoom, tocolor(34,36,43, 255 * self.alpha), 5)
...
function Trading:renderItems(owner, x, y, scroll, render)
    if not self.items[owner] then
        dxDrawText("Gracz nie dodał żadnych przedmiotów.", x, y, x, guiData.y + guiData.h - 80/zoom, tocolor(150, 150, 150, 255 * self.alpha), 1/zoom, self.fonts.items, "left", "top")
    else
        y = y + 10/zoom
        for i = 1, render do
        ...
        if #self.items[owner] > render then
            ...
            dxDrawRectangle(x + guiData.w/2 - 20/zoom, guiData.y + 40/zoom, 2/zoom, height, tocolor(34,36,43, 255 * self.alpha))
            dxDrawRectangle(x + guiData.w/2 - 20/zoom, guiData.y + 40/zoom + barY, 2/zoom, barHeight, tocolor(42,45,51, 255 * self.alpha))
        else
            dxDrawRectangle(x + guiData.w/2 - 20/zoom, guiData.y + 40/zoom, 2/zoom, height, tocolor(34,36,43, 255 * self.alpha))
        end
    end
