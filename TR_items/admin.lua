...
function CreateItem:render()
    ...
    dxDrawText("Tworzenie przedmiotów", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255), 1/zoom, self.fonts.main, "center", "center")
    ...
    for i = 1, guiInfo.visibleItems do
        ...
        if details then
            ...
            if self.selectedItem == i + self.scroll then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + (i-1) * 30/zoom, guiInfo.w, 30/zoom, tocolor(42,45,51, 255))
                color = tocolor(44,181,233, 255)

            elseif self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + (i-1) * 30/zoom, guiInfo.w, 30/zoom) then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + (i-1) * 30/zoom, guiInfo.w, 30/zoom, tocolor(34,36,43, 255))
            end
