...
function Shop:open()
    ...
    self.buttons = {}
    self.buttons.exit = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, guiInfo.w/2 - 20/zoom, 40/zoom, "Zamknij sklep")
    self.buttons.buy = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - (guiInfo.w/2 - 20/zoom) - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, guiInfo.w/2 - 20/zoom, 40/zoom, "Zakup przedmioty", "blue")
...
function Shop:render()
    ...
    dxDrawText(self.shopName, guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 40/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.name, "center", "center")
    ...
    if #self.shopProducts > 6 then
        ...
        dxDrawRectangle(guiInfo.x + guiInfo.w - 8/zoom, guiInfo.y + 50/zoom, 4/zoom, 350/zoom, tocolor(42,45,51, 255 * self.alpha))
        dxDrawRectangle(guiInfo.x + guiInfo.w - 8/zoom, guiInfo.y + 50/zoom + barY, 4/zoom, barHeight, tocolor(34,36,43, 255 * self.alpha))
    else
        dxDrawRectangle(guiInfo.x + guiInfo.w - 8/zoom, guiInfo.y + 50/zoom, 4/zoom, 350/zoom, tocolor(42,45,51, 255 * self.alpha))
    end
end
...
function Shop:drawShopItem(item, i)
    ...
    dxDrawText("$"..item.showPrice, guiInfo.x + guiInfo.w - 30/zoom, guiInfo.y + move, guiInfo.x + guiInfo.w - 50/zoom, guiInfo.y + 50/zoom + move, tocolor(44,181,233, 200 * self.alpha), 1/zoom, self.fonts.price, "right", "center")
...
function Shop:clickButton(...)
    ...
    elseif arg[1] == self.buttons.buy then
        if not self:canBuy() then exports.TR_noti:create("Nie wybrałeś żadnego przedmiotu.", "error") return end
...
function Shop:getItemDetails(item)
    local name, description, icon, type = "Nieznany przedmiot", "Nikt nie jest pewny co to jest i skąd sie tutaj wzięło.", "unknown"
...
function buyShopItem(...)
    ...
    if arg[1] then
        ...
        exports.TR_noti:create("Pomyślnie zakupiłeś wybrane przedmioty.", "success")
