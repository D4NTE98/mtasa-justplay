...
function Equipment:setFavourite(...)
    ...
    if arg[3] then
        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Przedmiot został dodany do ulubionych.")
    else
        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Przedmiot został usunięty z ulubionych.")
    end
end
...
function Equipment:removeItem(...)
    ...
    if not arg[3] then
        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Przedmiot został pomyślnie zniszczony.")
...
function Equipment:mergeItems(plr, id, type, value, value2)
    ...
    triggerClientEvent(plr, "equipmentResponse", resourceRoot, "Przedmioty zostały połączone w jeden.")
end
...
function Equipment:useItem(...)
    ...
    if arg[4] == itemTypes.clothes then
        ...
        if arg[3] then
            if customModel then
                if data.skin ~= model then triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Nie możesz założyć tego skina, ponieważ masz już jakiś ubrany.", "error") return end
            else
                if getElementModel(arg[1]) ~= 0 then triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Nie możesz założyć tego skina, ponieważ masz już jakiś ubrany.", "error") return end
            end
        end

        if not arg[3] then
            if customModel then
                if getElementModel(arg[1]) ~= 0 then triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Nie możesz zdjąć tego skina w tym momencie.", "error") return end
            else
                if getElementModel(arg[1]) ~= tonumber(data.skin) then triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Nie możesz zdjąć tego skina w tym momencie.", "error") return end
            end
        end

        local model, text = arg[5], "Skin został założony."
        if data.skin == model then
            model = 0
            text = "Skin został zdjęty."
        end
    ...
    elseif arg[4] == itemTypes.weapon then
        if not arg[3] then
            if tonumber(arg[6]) == 1 and (tonumber(arg[7]) >= 4 and tonumber(arg[7]) <= 12) then
                ...
                triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Wędka została schowana.", "success", true)

            else
                exports.TR_weaponSlots:takeWeapon(arg[1], arg[5], 999999999)
                triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Broń została schowana.", "success", true)
            end
        else
            ...
            if tonumber(arg[6]) == 1 and (tonumber(arg[7]) >= 4 and tonumber(arg[7]) <= 12) then
                ...
                triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Wędka została wyekwipowana.", "success", true)

            elseif weaponsWithoutAmmo[weapon] then
                exports.TR_weaponSlots:giveWeapon(arg[1], weapon, 999999, false)
                triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Broń została wyekwipowana.", "success", true)
                ...
                if ammo and ammo[1].ammunition then
                    if ammo[1].ammunition == 0 then
                        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Nie posiadasz amunicji do tej broni.", "error", false)
                        return
                    end
    ...
    elseif arg[4] == itemTypes.armor then
        if not arg[3] then
            ...
            setPedArmor(arg[1], 0)
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Kamizelka kuloodporna została zdjęta.", "success", true)

        else
            local armor = getElementData(arg[1], "armorID")
            if armor then
                triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Masz już ubraną jedną kamizelkę.", "error", false)
                return
            end

            setElementData(arg[1], "armorID", arg[2])
            setPedArmor(arg[1], arg[5])
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Kamizelka kuloodporna została ubrana.", "success", true)
        end
        exports.TR_mysql:querry(string.format("UPDATE tr_items SET used = %s WHERE ID = ? LIMIT 1", arg[3] and "1" or "NULL"), arg[2])
    ...
    elseif arg[4] == itemTypes.armorplate then
        ...
        if not armorID then
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Nie masz ubranej żadnej kamizelki kuloodpornej.", "error", false)
            return
        end

        if armor == 100 then
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Kamizelka musi być uszkodzona aby móc wymienić w niej płytę.", "error", false)
            return
        end
        ...
        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Płyta w kamizelce została wymieniona.", "success")
    ...
    elseif arg[4] == itemTypes.mask then
        if getElementData(arg[1], "characterMask") and arg[3] then
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, "Masz już założoną jedną kominiarkę.", "error")
            return
        end

        setElementData(arg[1], "characterMask", arg[3] and true or false)
        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot, arg[3] and "Założyłeś kominiarkę." or "Zdjąłeś kominiarkę.", "success", true)
        exports.TR_mysql:querry(string.format("UPDATE tr_items SET used = %s WHERE ID = ? LIMIT 1", arg[3] and "1" or "NULL"), arg[2])
    ...
    elseif arg[4] == itemTypes.gift then
        ...
        if arg[7] == 0 then
            ...
            if rand == 1 then
                ...
                exports.TR_noti:create(arg[1], "W środu prezentu znajduje się Ubranie Świąteczne.", "gift")

            elseif rand <= 51 then
                local names = {"DeMoNeK_", "NyAngela", "mAtiego", "Tryptaministy"}
                ...
                exports.TR_noti:create(arg[1], string.format("W środu prezentu znajduje się podpis %s.", names[autograph+1]), "gift")

            else
                exports.TR_core:giveMoneyToPlayer(arg[1], 500)
                exports.TR_noti:create(arg[1], "W środu prezentu znajduje się $500.", "gift")
            end
    ...
    elseif arg[4] == itemTypes.neon then
        ...
        if not veh then
            exports.TR_noti:create(arg[1], "Nie siedzisz w żadnym pojeździe.", "error")
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot)
            return
        end
        if getVehicleType(veh) ~= "Automobile" then
            exports.TR_noti:create(arg[1], "Nie możesz zamontować neonów w tym pojeździe.", "error")
            triggerClientEvent(arg[1], "equipmentResponse", resourceRoot)
            return
        end
        ...
        exports.TR_noti:create(arg[1], "Neony zostały pomyślnie założone.", "success")
        exports.TR_mysql:querry("DELETE FROM tr_items WHERE ID = ? LIMIT 1", arg[2])
        triggerClientEvent(arg[1], "equipmentResponse", resourceRoot)
        self:open(arg[1])
    end
...
function Equipment:givePlayerPremium(plr, type, days)
    ...
    if type == 0 then
        ...
        if rows > 0 then self:updatePlayerPremium(plr, "gold", string.format("Pomyślnie przedłużyłeś konto Gold o %ddni.", days)) return true end
        ...
        if rows > 0 then self:updatePlayerPremium(plr, "gold", string.format("Pomyślnie odebrałeś konto Gold na %ddni.", days)) return true end
        return true
    ...
    elseif type == 1 then
        ...
        if rows > 0 then self:updatePlayerPremium(plr, "diamond", string.format("Pomyślnie przedłużyłeś konto Diamond o %ddni.", days)) return true end
        ...
        if rows > 0 then self:updatePlayerPremium(plr, "diamond", string.format("Pomyślnie odebrałeś konto Diamond na %ddni.", days)) return true end
        return true
    end
end
...
function createInteractionItem(type, variant, variant2, value, value2)
    ...
    exports.TR_noti:create(client, "Przedmiot został pomyślnie utworzony.", "success")
...
function performTrade(plr, clientItems, plrItems, clientMoney, plrMoney)
    ...
    exports.TR_noti:create(plr, "Handel został pomyślnie zakończony.", "success")
    exports.TR_noti:create(client, "Handel został pomyślnie zakończony.", "success")
...
function declineTrade(client, plr)
    triggerClientEvent(plr, "closeTrade", resourceRoot)
    exports.TR_noti:create(plr, "Handel został anulowany. Coś poszło nie tak.", "error")
    exports.TR_noti:create(client, "Handel został anulowany. Coś poszło nie tak.", "error")
end
...
function takeoutItemFromStash(itemID, stashType, stashID, itemName)
    ...
    if stashType == 0 then
        ...
        exports.TR_discord:sendChannelMsg("playerTookItems", {
            ...
            text = string.format("zabrał %s graczowi %s.", itemName, nick[1].username),
        })
    end
end
