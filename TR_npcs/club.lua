...
function makeClubDance(model, clubId, price)
    ...
    if price then
        exports.TR_noti:create(string.format("Zakupiłeś prywatny taniec za $%.2f.", price), "success")
    else
        exports.TR_noti:create("Ze względu na posiadany status, prywatny taniec jest darmowy.", "success")
    end
...
function nextAnim()
    ...
    if not data.anims[data.selected] then
        exports.TR_noti:create("Twój czas na prywatny taniec dobiegł końca.", "info")
