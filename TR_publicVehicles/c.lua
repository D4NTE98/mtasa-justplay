...
function PrivateVehicles:onClientRender()
    ...
    for vehicle, data in pairs(self.elements) do
        ...
        if vx and vy and clear and dist < 40 then
             ...
             dist = dist/30
              dxDrawText(data.owner and string.format("Pojazd publiczny\n#999999(%s)", data.owner) or "Pojazd publiczny\n#999999(Wolny)", vx, vy, vx, vy, tocolor(255, 255, 255, 255 * alpha), 1/zoom - dist, self.font, "center", "center", false, false, false, true)
        end
    end
    if self.vehicle and isElement(self.vehicle) then
        if self.time and self.time.start and self.time.finish then
            ...
            if progress <= 1.0 and self.update ~= math.floor(time) then
                ...
                if self.notification then
                    exports.TR_noti:setText(self.notification, string.format("Zsiadłeś z wypożyczonego roweru.\nPojazd zniknie za %ds.", time) )
                end
