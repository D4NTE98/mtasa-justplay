...
function AddVehicle:constructor(...)
    ...
    self.buttons.submit = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h - 50/zoom, 235/zoom, 40/zoom, "Dodaj pojazd")
...
function AddVehicle:render()
    ...
    dxDrawText("Dodawanie pojazdu do organizacji", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    dxDrawText("Wybierz pojazd, który chcesz dodać do organizacji. Lider bądź inna osoba, która ma uprawnienia do zarządzania organizacją będzie mogła zdecydować, czy chce przyjąć twój samochód czy nie. Po jego akceptacji każdy członek organizacji będzie mógł prowadzić ten pojazd.", guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 15/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "top", false, true)
    -- dxDrawText("Koszt zmiany tablicy rejestracyjnej\nwynosi #d4af37$3000#aaaaaa.", guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h/2, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 55/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "bottom", false, false, false, true)
...
function AddVehicle:renderAddVehicle()
    for i = 1, 5 do
        ...
        if v then
            if i + self.scroll == self.selected then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 275/zoom, 40/zoom, tocolor(44,181,233, 200 * self.alpha))
            ...
            elseif self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 275/zoom, 40/zoom) then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 275/zoom, 40/zoom, tocolor(34,36,43, 255 * self.alpha))
                dxDrawText(v.name, guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + 275/zoom, guiInfo.y + 90/zoom + 40/zoom * (i-1), tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "center", true, true)
            else
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 275/zoom, 40/zoom, tocolor(42,45,51, 255 * self.alpha))
    ...
    if #self.vehicles > 5 then
        ...
        dxDrawRectangle(guiInfo.x + 275/zoom, guiInfo.y + 50/zoom, 4/zoom, 200/zoom, tocolor(42,45,51, 255 * self.alpha))
        dxDrawRectangle(guiInfo.x + 275/zoom, guiInfo.y + 50/zoom + barY, 4/zoom, barHeight, tocolor(34,36,43, 255 * self.alpha))
    else
        dxDrawRectangle(guiInfo.x + 275/zoom, guiInfo.y + 50/zoom, 4/zoom, 200/zoom, tocolor(34,36,43, 255 * self.alpha))
    end
end
...
function AddVehicle:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.submit then
        if not self.selected then exports.TR_noti:create("Musisz wybrać pojazd.", "error") return end
...
function AddVehicle:response(state)
    if state then
        ...
        exports.TR_noti:create("Pojazd został dodany do listy akceptacji organizacji.", "success")
    else
        exports.TR_noti:create("Nie udało się dodać pojazdu do organizacji.", "error")
    end
