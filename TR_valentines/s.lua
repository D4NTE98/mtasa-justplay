function playerSelectEventPrize(prize)
    ...
    if prize == "money" then
        exports.TR_core:giveMoneyToPlayer(client, 8000)
    ...
    elseif prize == "gold" then
        local _, rows = exports.TR_mysql:querry(string.format("UPDATE tr_accounts SET gold = DATE_ADD(gold, INTERVAL %d DAY) WHERE username = ? AND gold >= NOW() LIMIT 1", 5), getPlayerName(client))
        if rows > 0 then
            updatePlayerPremium(client, "gold")

        else
            local _, rows = exports.TR_mysql:querry(string.format("UPDATE tr_accounts SET gold = DATE_ADD(NOW(), INTERVAL %d DAY) WHERE username = ? AND (gold <= NOW() OR gold IS NULL) LIMIT 1", 5), getPlayerName(client))
            if rows > 0 then
                updatePlayerPremium(client, "gold")
            end
        end
    ...
    elseif prize == "diamond" then
        local _, rows = exports.TR_mysql:querry(string.format("UPDATE tr_accounts SET diamond = DATE_ADD(diamond, INTERVAL %d DAY) WHERE username = ? AND diamond >= NOW() LIMIT 1", 2), getPlayerName(client))
        if rows > 0 then
            updatePlayerPremium(client, "diamond")

        else
            local _, rows = exports.TR_mysql:querry(string.format("UPDATE tr_accounts SET diamond = DATE_ADD(NOW(), INTERVAL %d DAY) WHERE username = ? AND (diamond <= NOW() OR diamond IS NULL) LIMIT 1", 2), getPlayerName(client))
            if rows > 0 then
                updatePlayerPremium(client, "diamond")
            end
        end
