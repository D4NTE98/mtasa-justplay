...
function Help:createButtons()
    ...
    self.buttons = {}
    self.buttons.back = exports.TR_dx:createButton(guiInfo.back.x, guiInfo.back.y, guiInfo.back.w, guiInfo.back.h, "Wróć", "grey")
    self.buttons.link = exports.TR_dx:createButton(guiInfo.back.x, guiInfo.back.y - 50/zoom, guiInfo.back.w, guiInfo.back.h, "blue")
...
function Help:drawCategories()
    ...
    self:drawBackground(guiInfo.category.x, guiInfo.category.y, guiInfo.category.w, guiInfo.category.h, tocolor(16,18,23, 255 * self.alpha * (1 - self.alphaCategory)), 11)
...
function Help:drawSelectedCategory()
    ...
    elseif self.category == "premium" or self.nextCategory == "premium" then
        ...
        dxDrawText("RÓŻNICE POMIĘDZY POSIADANIEM\nKONTA DANEGO TYPU", guiInfo.premium.x, guiInfo.premium.y - 30/zoom, guiInfo.premium.x + guiInfo.premium.w/2, guiInfo.premium.y + 40/zoom, tocolor(170, 170, 170, 255 * self.alpha * self.alphaCategory), 1/zoom, self.fonts.premium, "center", "center", false, false, false, true)
        ...
        dxDrawText("STANDARD", guiInfo.premium.x + guiInfo.premium.w/2, guiInfo.premium.y + 20/zoom, guiInfo.premium.x + guiInfo.premium.w/2 + guiInfo.premium.w/6, guiInfo.premium.y + 40/zoom, tocolor(221, 221, 221, 255 * self.alpha * self.alphaCategory), 1/zoom, self.fonts.category, "center", "top", false, false, false, true)
...
function Help:selectCategory(category, logoY)
    ...
    elseif category == "updates" then
        exports.TR_dx:showScroll(self.scroll)
        exports.TR_dx:setScrollText(self.scroll, "Wczytywanie nowości...")
    ...
    elseif category == "info" then
        ...
        exports.TR_dx:setButtonText(self.buttons.link, "Kopiuj link do forum")

    elseif category == "premium" then
        self.scrollPremium = 0
        exports.TR_dx:showButton(self.buttons.link)
        exports.TR_dx:setButtonText(self.buttons.link, "Kopiuj link do sklepu")
    end
...
function Help:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.link then
        if self.category == "info" then setClipboard("x") end
        if self.category == "premium" then setClipboard("x") end
        exports.TR_noti:create("Link został skopiowany do schowka. Otwórz przeglądarkę i wklej go w adres przeglądania.", "success", 5)
    end
end
