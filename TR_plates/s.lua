...
function createNPC()
    ...
    exports.TR_npc:addDialogueText(dialogue, "Chciałbym przerejestrować pojazd.", {pedResponse = "Proszę wypełnić wniosek.", responseTo = "Dzień dobry.", img = "plate", trigger = "changeVehiclePlate"})
...
function changeVehiclePlate()
    ...
    if not vehicles or #vehicles < 1 then exports.TR_noti:create(source, "Nie posiadasz żadnego pojazdu, który możesz przerejestrować.", "error") return end
...
function playerChangePlateVehicle(state, data)
    if state then
        ...
        triggerClientEvent(source, "plateChangeResponse", resourceRoot, "Pojazd został pomyślnie przerejestrowany.", "success")
        return
    end
