...
function Plates:render()
    ...
    dxDrawText("Przejerestrowywanie pojazdu", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    dxDrawText("Wpisz tekst tablicy rejestracyjnej.", guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h/2, guiInfo.x + guiInfo.w - 15/zoom, guiInfo.y + guiInfo.h/2 - 35/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "bottom", false, false, false, true)
    dxDrawText("Koszt zmiany tablicy rejestracyjnej\nwynosi #2cb5e9$3000#aaaaaa.", guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h/2, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 55/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "bottom", false, false, false, true)
    self:renderPlates()
end
...
function Plates:renderPlates()
    for i = 1, 5 do
        ...
        if v then
            if i + self.scroll == self.selected then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 250/zoom, 40/zoom, tocolor(44,181,233, 200 * self.alpha))
            ...
            elseif self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 250/zoom, 40/zoom) then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 250/zoom, 40/zoom, tocolor(42,45,51, 255 * self.alpha))
                dxDrawText(v.name, guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + 240/zoom, guiInfo.y + 90/zoom + 40/zoom * (i-1), tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "center", true, true)
            else
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 250/zoom, 40/zoom, tocolor(34,36,43, 255 * self.alpha))
    ...
    if #self.vehicles > 5 then
        ...
        dxDrawRectangle(guiInfo.x + 250/zoom, guiInfo.y + 50/zoom, 4/zoom, 200/zoom, tocolor(34,36,43, 255 * self.alpha))
        dxDrawRectangle(guiInfo.x + 250/zoom, guiInfo.y + 50/zoom + barY, 4/zoom, barHeight, tocolor(42,45,51, 255 * self.alpha))
    else
        dxDrawRectangle(guiInfo.x + 250/zoom, guiInfo.y + 50/zoom, 4/zoom, 200/zoom, tocolor(42,45,51, 255 * self.alpha))
    end
end
...
function Plates:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.submit then
        ...
        if text == data.plate then exports.TR_noti:create("Musisz wpisać nową rejestrację.", "error") return end
