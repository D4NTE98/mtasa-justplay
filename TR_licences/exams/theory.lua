...
function LicenceTheory:render()
    self:animate()
    self:drawBackground(guiInfo.x, guiInfo.y, guiInfo.w, guiInfo.h, tocolor(16,18,23, 255 * self.alpha), 14)
    dxDrawText("Egzamin teoretyczny", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    ...
    if self.selectedQuestion then
        ...
        dxDrawText("Możliwe odpowiedzi:", guiInfo.x + 25/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 10/zoom, self.answerY - 5/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.title, "left", "bottom", true, true)
    ...
    elseif self.ended then
        dxDrawText("WYNIK EGZAMINU", guiInfo.x + 10/zoom, guiInfo.y + 160/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", true, true)
        local color = self.result and {67, 166, 31} or {166, 31, 31}
        dxDrawText(string.format(self.result and "Pozytywny (%d/%d)" or "Negatywny (%d/%d)", self.questionsGood, self.questionsCount), guiInfo.x + 10/zoom, guiInfo.y + 190/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(color[1], color[2], color[3], 255 * self.alpha), 1/zoom, self.fonts.text, "center", "top", true, true)
        ...
        dxDrawText("Egzamin teoretyczny polega na wybraniu spośród dostępnych odpowiedzi określenia, które jest prawdziwe i zgodne z prawem.", guiInfo.x + 10/zoom, guiInfo.y + 110/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.text, "center", "top", true, true)

        dxDrawText("PYTANIA EGZAMINACYJNE", guiInfo.x + 10/zoom, guiInfo.y + 210/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", true, true)
        dxDrawText(string.format("Egzamin składa się z %d pytań.\nAby uzyskać wynik pozytywny należy poprawnie odpowiedzieć na %d pytań.", self.questionsCount, self.questionsNeeded), guiInfo.x + 10/zoom, guiInfo.y + 240/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.text, "center", "top", true, true)
    end
end
...
function LicenceTheory:createNextQuestion()
    if #self.questions < 1 then
        ...
        self.buttons.close = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Zamknij okno")
...
function LicenceTheory:clickButton(...)
    if arg[1] == self.buttons.start then
        ...
        self.buttons.next = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Następne pytanie")
    ...
    elseif arg[1] == self.buttons.next then
        ...
        exports.TR_noti:create("Aby przejść do następnego pytania musisz zaznaczyć jedną z odpowiedzi.", "error")
...
function openLicenceTheory(plr, md)
    ...
    if not boughtExams["theory"][licence] then exports.TR_noti:create("Aby rozpocząć ten egzamin musisz go najpierw opłacić przy biurku.", "error") return end
