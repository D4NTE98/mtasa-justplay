...
addEvent("client -> addReport", true)
addEventHandler("client -> addReport", root, 
    function(text, player)
        if #reporty > 15 then
            table.remove(reporty, 1);
        end;
        table.insert(reporty, {text, player});
    end
);

addEvent("client -> removeReport", true)
addEventHandler("client -> removeReport", root, 
    function(player)
        for i=#reporty,1,-1 do
            if reporty[i][2] == tonumber(player) then
...
addCommandHandler('rapsy', function(plr, cmd)
    if getElementData(localPlayer, 'characterUID') == 2 then
        for i,v in ipairs(reporty) do
            table.remove(reporty, i)
        end
    end
end)
    ...
    for i = 1, 17 do
        if self.chatLogs[i] then
            if self.chatLogs[i].suspected then
                dxDrawText(string.format("%s", self.chatLogs[i].text), 20/zoom, y + 5/zoom + i * 15/zoom, sx - 5/zoom, y + 15/zoom + i * 20/zoom, tocolor(255, 160, 160, 200), 1/zoom, self.fonts.reports, "left", "top", true, true, false, true)

            else
                dxDrawText(string.format("%s", self.chatLogs[i].text), 20/zoom, y + 5/zoom + i * 15/zoom, sx - 5/zoom, y + 15/zoom + i * 20/zoom, tocolor(255, 255, 255, 200), 1/zoom, self.fonts.reports, "left", "top", true, true, false, true)
            end
         end
    end
end
...
function AdminInfo:renderReportLogs()
    ...
    dxDrawText("LISTA REPORTÓW:", 300/zoom, y, sx - 5/zoom, y + 20/zoom, tocolor(255, 255, 255, 200), 1/zoom, self.fonts.reports, "left", "center", true, true)
    for i = 1, 12 do
        if self.reportsList[i] then
            dxDrawText(string.format("%s", self.reportsList[i]), 300/zoom, y + i * 15/zoom, sx - 5/zoom, y + 15/zoom + i * 20/zoom, tocolor(255, 255, 255, 200), 1/zoom, self.fonts.reports, "left", "top", true, true)
        end
    end
end]]
...
function AdminInfo:renderReportLogs()
    ...
    dxDrawText('Lista reportów', sx-115, sy/3+8, 17+dxGetTextWidth("Lista reportów")+sx-115, 20+sy/3+8, tocolor(255, 255, 255, 200), 1/zoom, self.fonts.reports, "right", "top", false, true, false, false, false)
    dxDrawText(y, sx, sy/3+14, sx-7, 50, tocolor(255, 255, 255, 200), 1/zoom, self.fonts.reports, "right", "top", false, true, false, false, false)
end
        ...
        if scx and scy and getDistanceBetweenPoints3D(x, y, z, px, py, pz) < 40 then
            ...
            if vehicleID then
                if string.len(vehicleID) > 4 then
                    status = string.format("Prywatny %s", string.sub(vehicleID, 8, string.len(vehicleID)))
                end
            end
...
function AdminInfo:renderServerStatus()
    ...
    if #self.serverStatus > 0 then
        ...
        dxDrawText("Nie zarejestrowano żadnego zużycia", 600/zoom, y + 20/zoom, 200/zoom, y + 20/zoom, tocolor(255, 255, 255, 255), 1/zoom, self.fonts.reports, "left", "top")
    end
    dxDrawText(string.format("Synchronizacja: %s %s", self.serverPackets[3], self.serverPackets[7]), 600/zoom, (sy - 40/zoom), 200/zoom, (sy - 40/zoom), tocolor(255, 255, 255, 255), 1/zoom, self.fonts.reports, "left", "top")
end
...
function performTechnicalPause(time)
    ...
    technicalPause.noti = exports.TR_noti:create(string.format("Przerwa techniczna rozpocznie się za %d %s.\nZakończ wszystkie ważne czynności.", time, getTechnicalMinutes(time)), "system", time * 60)
...
function updateTechnicalPause()
    ...
    exports.TR_noti:setText(technicalPause.noti, string.format("Przerwa techniczna rozpocznie się za %d %s.\nZakończ wszystkie ważne czynności.", technicalPause.time, getTechnicalMinutes(technicalPause.time)))
end

function removeTechnicalPause()
    if technicalPause.noti then
        exports.TR_noti:create("Przerwa techniczna zakończyła się.\nMożesz powrócić do normalnej rozgrywki.", "system")
