...
function WarnSystem:show(plr, admin, reason)
    ...
    if getPlayerName(localPlayer) == plr then
        ...
        self.reason = nil
        self.victim = nil
        exports.TR_noti:create(string.format("%s został ostrzeżony przez %s.\nPowód: %s", plr, admin, reason), "penalty", 16)
    end
end
...
function WarnSystem:render()
    ...
    if self.victim then
        ...
        --dxDrawImage(0, 0, sx, sy, 'files/images/sq.jpg');
        dxDrawText("OTRZYMAŁEŚ OSTRZEŻENIE!", 0, 0, sx, 300/zoom, tocolor(245, 47, 47, 255 * self.alpha), 1/zoom, self.fonts.warn, "center", "center")
