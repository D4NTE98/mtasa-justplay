...
function toggleGate(toggle, blockQuestion)
	if toggle then
		...
		setElementData(settingsOX.counter, "text", "Proszę czekać na następne pytanie")
...
function setQuestion(text, isTrue)
	...
	setElementData(settingsOX.counter, "text", "Pytanie:\n"..settingsOX.Question)
...
function setCounter(time)
	...
	setTimer(function()
		if settingsOX.time > 1 then
			...
			setElementData(settingsOX.counter, "text", string.format("Czas na odpowiedź:\n%d %s", settingsOX.time, getSeconds(settingsOX.time) ) )
		else
			toggleGate(false)
