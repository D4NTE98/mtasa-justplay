...
local guiInfo = {
    ...
    texts = {
        private = "Aby móc przejechać na drugą stronę musisz uiścić opłatę\nprzejazdową. Jeśli chcemy utrzymywać autostradę cały czas w\nidealnym stanie - trzeba skądś wziąć na to fundusze.\nPrzejazd autostradą kosztuje więc #2cb5e9$25.",
        public = "Witamy na autostradzie miedzymiastowej stanu San Andreas.\nJako iż pracujesz w naszym stanie, w ramach hołdu twojej\nciężkiej pracy przejazd naszymi autostradami samochodem\nsłużbowym jest całkowicie #2cb5e9darmowy.",
    },
}
...
function HighwayGate:createCols()
    ...
    for i, v in pairs(positions) do
        ...
        setElementData(marker, "markerData", {
            title = "Bramki autostradowe",
            desc = "Zatrzymaj się aby przejechać przez bramki.",
        }, false)
    end
...
function HighwayGate:open()
    ...
    self.buttons = {}
    self.buttons.cancel = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Anuluj przejazd", "grey")
    self.buttons.open = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 260/zoom, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, self.isPrivate and "Zapłać za przejazd" or "Otwórz szlaban", "blue")
...
function HighwayGate:render()
    ...
    dxDrawText("Bramki autostradowe", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
...
function HighwayGate:onReponse(state)
    ...
    if state then
        ...
        exports.TR_noti:create("Nie możesz otworzyć szlabanu dopóki nie będzie on w pełni zamknięty.", "error")
    end
end
