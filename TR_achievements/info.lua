...
function ShowAchievement:render()
    ...
    self:drawBackground(x, y, w, h, tocolor(23,25,31, 255 * self.alpha), 15)

    -- Upper medal
    self.rot = self.rot + 0.3
    if self.rot >= 360 then self.rot = self.rot - 360 end
    local size = (h - 30/zoom) * self.progress.upper
    dxDrawImage((x + w/2) - size/2 - (guiInfo.w - size - 20/zoom)/2 * self.progress.leftAlpha, (y + h/2) - size/2 - 15/zoom, size, size, "files/images/medal.png", self.rot, 0, 0, tocolor(44,181,233, 255 * self.progress.upperAlpha * self.alpha))
