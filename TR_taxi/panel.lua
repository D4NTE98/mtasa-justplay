...
function Panel:constructor(...)
    ...
    self.declineOrder = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Anuluj aktualne zlecenie", "red")
...
function Panel:createList(...)
    ...
    if arg[2] == "take" then
        ...
        exports.TR_noti:create(string.format("Zgłoszenie zostało przyjęte.\nLokalizacja: %s", self.selectData.destination), "success", 6)
        exports.TR_jobs:createInformation("Taksówkarz", string.format("Udaj się do %s aby odebrać gracza %s.", self.selectData.destination, self.selectData.playerName))
    ...
    elseif arg[2] == "taken" then
        exports.TR_noti:create("Te zgłoszenie zostało już zaakceptowane przez innego taksówkarza.", "error")
    end
...
function Panel:render()
    ...
    dxDrawText("Przyjmowanie zgłoszeń", guiInfo.x + 10/zoom, guiInfo.y, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 40/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    for i = 1, 8 do
        if self.requests[i] then
            if self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.w, 40/zoom) then
                dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.w, 40/zoom, tocolor(34,36,43, 255 * self.alpha))
            end

            dxDrawText(string.format("%s (%s)", self.requests[i].playerName, self.requests[i].destination), guiInfo.x + 20/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 90/zoom + 40/zoom * (i-1), tocolor(110,119,132, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "center")
            dxDrawText(string.format("Odległość: %dm", self.requests[i].dist), guiInfo.x + 20/zoom, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 90/zoom + 40/zoom * (i-1), tocolor(110,119,132, 255 * self.alpha), 1/zoom, self.fonts.info, "right", "center")
        end
    end
end
...
function Panel:handleClick(...)
    ...
    if arg[1] == "left" and arg[2] == "down" then
        for i = 1, 8 do
            if self.requests[i] then
                if self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), guiInfo.w, 40/zoom) then
                    if not isElement(self.requests[i].player) then exports.TR_noti:create("Te zgłoszenie jest już nieaktualne, ponieważ gracz opuścił serwer.", "info") return end
                    if isElement(guiInfo.blip) then exports.TR_noti:create("Nie możesz odebrać innego zgłoszenia, dopóki nie zrealizujesz aktualnego.", "error") return end
                    if self.requests[i].player == localPlayer then exports.TR_noti:create("Nie możesz przyjąć swojego zgłoszenia.", "error") return end
...
function Panel:declineTaxiRequest(btn)
    ...
    if btn == self.declineOrder then
        ...
        exports.TR_jobs:createInformation("Taksówkarz", "Poczekaj na zgłoszenie, lub samemu udaj się w jakieś często odwiedzane miejsce aby zdobyć klientów. Panel ze zgłoszeniami znajdziesz pod przyciskiem F4.")
        exports.TR_noti:create(client, "Zgłoszenie zostało pomyślnie anulowane.", "success")
    end
end
...
function updateBlipPos()
    ...
    if not isElement(guiInfo.client) then
        ...
        exports.TR_noti:create("Zgłoszenie zostało anulowane, ponieważ gracz wyszedł z serwera.", "info")
        exports.TR_jobs:createInformation("Taksówkarz", "Poczekaj na zgłoszenie, lub samemu udaj się w jakieś często odwiedzane miejsce aby zdobyć klientów. Panel ze zgłoszeniami znajdziesz pod przyciskiem F4.")
        return
    end
