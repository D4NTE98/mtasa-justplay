...
function Passenger:render()
    ...
    dxDrawText(self.isTravelMan and "Podróż taksówką" or "Wskazania taksometru", guiInfo.x + 10/zoom, guiInfo.y, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 25/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    dxDrawText(string.format("Koszt za 1km: #787878$%.2f\n#aaaaaaCzas podróży: #787878%s\n#aaaaaaPrzejechany dystans: #787878%.2fkm", self.toPayPerKM, self:getTimeInSeconds((getTickCount() - self.travelTime)/1000), self.distance), guiInfo.x + 10/zoom, guiInfo.y + 25/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", false, false, false, true)
    ...
    if self.lastDistance + 1 < self.distance then
        ...
        if self.isTravelMan then
            ...
            elseif not self.notiSend then
                exports.TR_noti:create("Nie stać cię na opłacenie dalszego kursu.", "error", 10)
                self.notiSend = true
            end
            ...
            elseif not self.notiSend then
                exports.TR_noti:create("Klienta nie stać na opłacenie dalszego kursu.", "error", 10)
                self.notiSend = true
            end
        end
    end
