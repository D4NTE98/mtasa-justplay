...
function FuelStation:checkVehicle()
    if getDistanceBetweenPoints3D(Vector3(getElementPosition(self.distributor)), Vector3(getElementPosition(localPlayer))) > 10 then
        self.fueled = 0
        self:destroy()
        exports.TR_noti:create("Oddaliłeś się zbyt daleko od dystrybutora.", "error", 6)
        return
    end
...
function FuelStation:render()
    ...
    self:drawBackground(guiInfo.x + 20/zoom, guiInfo.y + 35/zoom, guiInfo.w - 40/zoom, 20/zoom, tocolor(23,25,31, 255 * self.alpha), 15)

    if self.vehicleUpdated then
        if self.vehicleSelected then
            if self.vehicleSelected ~= self.vehicle then
                self:drawBackground(guiInfo.x + 20/zoom, guiInfo.y + 35/zoom, (guiInfo.w - 40/zoom) * (self.vehicleFuel)/self.vehicleMaxFuel, 20/zoom, tocolor(34,36,43, 255 * self.alpha), 15)
                ...
                self:drawBackground(guiInfo.x + 20/zoom, guiInfo.y + 35/zoom, (guiInfo.w - 40/zoom) * (self.vehicleFuel + self.fueled)/self.vehicleMaxFuel, 20/zoom, tocolor(34,36,43, 255 * self.alpha), 15)
    ...
    dxDrawText(self.vehicleName, guiInfo.x, guiInfo.y + 5/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 5/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", false, false, false, true)

    dxDrawText(string.format("Cena: #2cb5e9$%.2f  #ffffffDo zapłaty: #2cb5e9$%.2f", self.fuelPrice, self.fuelPrice * self.fueled), guiInfo.x, guiInfo.y + 63/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 5/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "top", false, false, false, true)
    if self.vehicleEngineType == self.fuelType then
        dxDrawText("#aaaaaaNaciśnij E #aaaaaaaby zatankować.", guiInfo.x, guiInfo.y + 20/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 5/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "bottom", false, false, false, true)
    else
        dxDrawText("#aaaaaaTen rodzaj paliwa nie pasuje do tego pojazdu.", guiInfo.x, guiInfo.y + 20/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 5/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "bottom", false, false, false, true)
    end
end
...
function FuelStation:useStation(...)
    ...
    if self.vehicleSelected then
        if self.vehicleSelected ~= self.vehicle then exports.TR_noti:create("Odłóż wąż i podnieś go ponownie aby zatankować następny samochód.", "info", 6) return end
    end
...
function fuelResponse(...)
    ...
    if arg[1] then
        ...
        exports.TR_noti:create("Transakcja została zrealizowana pomyślnie.", "success")
    else
