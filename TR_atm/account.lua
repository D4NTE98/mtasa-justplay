...
function Account:constructor(...)
    ...
    self.buttons = {}
    self.buttons.create = exports.TR_dx:createButton((sx - 200/zoom)/2, guiData.y + guiData.h - 100/zoom, 200/zoom, 40/zoom, "Załóż konto")
    self.buttons.close = exports.TR_dx:createButton((sx - 200/zoom)/2, guiData.y + guiData.h - 50/zoom, 200/zoom, 40/zoom, "Anuluj")

    self.edit = exports.TR_dx:createEdit((sx - 300/zoom)/2, guiData.y + guiData.h - 190/zoom, 300/zoom, 40/zoom, "Wpisz PIN", true, false)
...
function Account:render()
    self:animate()
    self:drawBackground(guiData.x, guiData.y, guiData.w, guiData.h, tocolor(16,18,23, 255 * self.alpha), 15)
    dxDrawImage((sx - 143/zoom)/2, guiData.y + 20/zoom, 143/zoom, 65/zoom, self.logo, 0, 0, 0, tocolor(255, 255, 255, 255 * self.alpha))

    dxDrawText("Koszt założenia konta w banku to #d4af37$200#999999.\n\nWpisz kod #2cb5e9PIN#999999 do karty, który będzie\nwykorzystywany podczas korzystania\nz bankomatów.", guiData.x + 5/zoom, guiData.y + 75/zoom, guiData.x + guiData.w - 5/zoom, guiData.y + guiData.h - 200/zoom, tocolor(153, 153, 153, 255 * self.alpha), 1/zoom, self.fonts.big, "center", "center", true, true, false, true)
    dxDrawText("Pamiętaj! PIN może składać się tylko z cyfr!", guiData.x + 5/zoom, guiData.y + guiData.h - 145/zoom, guiData.x + guiData.w - 5/zoom, guiData.y + guiData.h - 100/zoom, tocolor(153, 153, 153, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "top", true, true)
end
...
function Account:button(...)
    if arg[1] == self.buttons.create then
        ...
        if string.len(text) < 4 then exports.TR_noti:create("Kod PIN musi składać się minimalnie z 4 cyfr.", "error") return end
        if string.len(text) > 8 then exports.TR_noti:create("Kod PIN musi składać się maksymalnie z 8 cyfr.", "error") return end
        if tonumber(text) == nil then exports.TR_noti:create("Kod PIN musi składać się tylko z cyfr.", "error") return end
