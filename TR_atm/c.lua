...
local guiInfo = {
    ...
    usedButtons = {
        main = {
            left = {false, false, false, {text = "Zamknij bankomat", action = "close"}},
            right = {false, false, false, {text = "Informacje", action = "about"}},
        },
        about = {
            left = {false, false, false, {text = "Cofnij", action = "main"}},
            right = {},
        },
        card = {
            left = {},
        ...
        pin = {
            left = {false, false, false, {text = "Anuluj", action = "close"}},
            right = {false, false, false, {text = "Potwierdź", action = "submitPin"}},
        },
        pinWrong = {
            left = {},
            right = {false, false, false, {text = "Spróbuj ponownie", action = "pin"}},
        },
        options = {
            left = {false, false, false, {text = "Zakończ transakcje", action = "close"}},
            right = {false, false, {text = "Wpłać pieniądze", action = "deposit"}, {text = "Wypłać pieniądze", action = "withdraw"}},
        },
        withdraw = {
        ...
        withdrawOwn = {
            left = {false, false, false, {text = "Anuluj", action = "withdraw"}},
            right = {false, false, false, {text = "Wypłać pieniądze", action = "withdrawOwnSubmit"}},
        },
        deposit = {
        ...
        depositOwn = {
            left = {false, false, false, {text = "Anuluj", action = "deposit"}},
            right = {false, false, false, {text = "Wpłać pieniądze", action = "depositOwnSubmit"}},
        },
    }
}
...
function Atm:renderTabText()
    if self.tab == "main" then
        dxDrawText("Witaj w bankomacie BBO Bank.\nWłóż kartę i postępuj według instrukcji pokazanych na ekranie.", guiInfo.x + 150/zoom, guiInfo.y + 150/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.screen, "center", "top")

    elseif self.tab == "about" then
        dxDrawText("Bankomat BBO Bank.\nW tym bankomacie można wpłacić oraz wybrać dowonlą kwotę. Nasze bankomaty są monitorowane 24/7 aby wasze pianiądze były bezpieczne.\n\nPAMIĘTAJ!\nZakryj ręką pin podczas jego wpisywania aby nikt inny nie mógł go zobaczyć i w razie zgubienia bądź kradzieży Twojej karty nie mógł wybrać pieniędzy z twojego konta.", guiInfo.x + 150/zoom, guiInfo.y + 130/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)

    elseif self.tab == "options" then
        dxDrawText(string.format("Witaj %s w bankomacie BBO Bank.\nWybierz interesującą cię opcję.\n\nDostępne środki: $%.2f", getPlayerName(localPlayer), self.bankmoney), guiInfo.x + 150/zoom, guiInfo.y + 130/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)

    elseif self.tab == "deposit" then
        dxDrawText(string.format("Wybierz kwotę która cię interesuje.\n\nMożliwa wpłata: $%.2f", self.money), guiInfo.x + 150/zoom, guiInfo.y + 130/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)

    elseif self.tab == "depositOwn" then
        dxDrawText(string.format("Wpisz kwotę, którą chcesz wpłacić.\n\nMożliwa wpłata: $%.2f", self.money), guiInfo.x + 150/zoom, guiInfo.y + 130/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
    ...
    elseif self.tab == "withdraw" then
        dxDrawText(string.format("Wybierz kwotę która cię interesuje.\n\nDostępne środki: $%.2f", self.bankmoney), guiInfo.x + 150/zoom, guiInfo.y + 130/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)

    elseif self.tab == "withdrawOwn" then
        dxDrawText(string.format("Wpisz kwotę, którą chcesz wypłacić.\n\nDostępne środki: $%.2f", self.bankmoney), guiInfo.x + 150/zoom, guiInfo.y + 130/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)

        dxDrawRectangle((sx - 200/zoom)/2, (sy - 40/zoom)/2, 200/zoom, 40/zoom, tocolor(9, 95, 65, 255 * self.alpha))
        dxDrawText(string.format("%.2f", tonumber((self.pin or 0).."0")), (sx - 200/zoom)/2, (sy - 40/zoom)/2, (sx + 200/zoom)/2, (sy + 40/zoom)/2, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "center")

    elseif self.tab == "card" then
        dxDrawText("Karta płatnicza jest odczytywana...\nProszę chwilę poczekać.", guiInfo.x + 150/zoom, guiInfo.y + 150/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "center", true, true)

    elseif self.tab == "pinWrong" then
        dxDrawText("Autoryzacja karty płatniczej została odrzucona.\nPin do karty jest nieprawidłowy.", guiInfo.x + 150/zoom, guiInfo.y + 200/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)

    elseif self.tab == "pin" then
        dxDrawText("Karta płatnicza została odczytana pomyślnie.\nProszę wpisać pin i potwierdzić przyciskiem.", guiInfo.x + 150/zoom, guiInfo.y + 200/zoom, guiInfo.x + guiInfo.wMain - 150/zoom, guiInfo.y + guiInfo.h - 150/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
        ...
        for i = 0, 3 do
            if self:isMouseInPosition(guiInfo.x + guiInfo.wMain - 109/zoom, guiInfo.y + 123/zoom + 100/zoom * i, 90/zoom, 54/zoom) and guiInfo.usedButtons[self.tab].right[i + 1] then
                ...
                elseif option.action == "withdrawOwnSubmit" then
                    if tonumber(self.pin) == nil or tonumber(self.pin) < 1 then exports.TR_noti:create("Wpisz poprawną kwotę.", "error") return end
                    if not self:checkSlow() then return end
...
function Atm:checkSlow()
    if self.slowDown then
        if (getTickCount() - self.slowDown)/1000 < 1 then exports.TR_noti:create("Musisz odczekać chwilę przed kolejną operacją.", "error") return false end
    end
