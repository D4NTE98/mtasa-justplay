...
function BWSystem:onDeath(plr, time)
    ...
    self.type = "onBw"
    self.time = time and time or 180
...
function BWSystem:render()
    ...
    dxDrawText("JESTEŚ NIEPRZYTOMNY", guiInfo.x, guiInfo.y + 10/zoom, guiInfo.x + guiInfo.w, guiInfo.h, tocolor(43, 196, 255, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "top")
    dxDrawImageSection(guiInfo.x + 20/zoom, guiInfo.y + 40/zoom, guiInfo.w - 40/zoom, 98/zoom, self.heartX, 0, guiInfo.hearth.originalW, guiInfo.hearth.originalH, self.textures.heartBeat, 0, 0, 0, tocolor(200, 200, 200, 255 * self.alpha))

    if self.type == "onBw" then
        dxDrawText(string.format("Powoli wracasz do siebie i pełni swoich sił.\nOckniesz się za %ss.", self:secondsToClock(time)), guiInfo.x, guiInfo.y + 10/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 10/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom")
    else
        dxDrawText(string.format("Jesteś wieziony do szpitala, gdzie zostaniesz opatrzony.\nWrócisz do pełni sił za %ss.", self:secondsToClock(time)), guiInfo.x, guiInfo.y + 10/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 10/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom")
    end
