...
local guiInfo = {
  ...
  legend = [[
#d4af37←,→ #c8c8c8- wybór pojazdu
#d4af37ENTER #c8c8c8- akceptuj
#d4af37BACKSPACE #c8c8c8- wyjdź]],
...
function showVehicleGarage(querry, garageData)
  ...
  exports.TR_dx:showLoading(3000, "Wczytywanie garażu")
...
function closeVehicleGarage(state)
  ...
  exports.TR_dx:showLoading(3000, "Wczytywanie świata")
...
function render()
  ...
  drawBackground((sx - 600/zoom)/2, sy - 100/zoom, 600/zoom, 75/zoom, tocolor(23,25,31, 255), 15)
  dxDrawText(string.format("%s (ID: %d)", vehname, guiInfo.vehicles[guiInfo.selected].ID), 0, sy - 75/zoom, sx, sy - 65/zoom, tocolor(44,181,233, 255), 1/zoom, guiInfo.fonts.vehicle, "center", "bottom")

  dxDrawText("Aby odebrać pojazd naciśnij #2cb5e9ENTER#c8c8c8, aby wyjść naciśnij #2cb5e9BACKSPACE#c8c8c8.", 0, sy - 65/zoom, sx, sy - 37/zoom, tocolor(200, 200, 200, 255), 1, guiInfo.fonts.normal, "center", "bottom", false, false, false, true)
end
    ...
    for i, v in pairs(getElementsByType("player")) do
      ...
      if blockTune then
        if tonumber(blockTune) == guiInfo.vehicles[guiInfo.selected].ID then
          exports.TR_noti:create("W tej chwili nie możesz wyjąć tego pojazdu.", "error")
          return
        end
      end
    end
    ...
    for i, v in pairs(getElementsByType("vehicle")) do
      ...
      if vehData then
        if vehData.ID == guiInfo.vehicles[guiInfo.selected].ID then
          exports.TR_noti:create("Ten pojazd został już wyjęty.", "error")
          return
        end
      end
    end
...
function garageHideVehicle(garageID, marker)
  ...
  exports.TR_noti:create("Aby oddać pojazd do przechowywalni zatrzymaj się w wyznaczonym miejscu.", "info", 5)
...
function openGarageHideVehicle()
  ...
  hideData.cancel = exports.TR_dx:createButton((sx - 515/zoom)/2 + 45/zoom, (sy + 200/zoom)/2 - 50/zoom, 200/zoom, 40/zoom, "Anuluj", "grey")
  hideData.accept = exports.TR_dx:createButton((sx + 515/zoom)/2 - 245/zoom, (sy + 200/zoom)/2 - 50/zoom, 200/zoom, 40/zoom, "Oddaj pojazd", "blue")
...
function garageHideClick(btn)
  ...
  elseif btn == hideData.accept then
    closeGarageHideVehicle()
    if not getPedOccupiedVehicle(localPlayer) then exports.TR_noti:create("Nie siedzisz w żadnym pojeździe.", "error") return end
...
function renderHidding()
  ...
  drawBackground((sx - 515/zoom)/2, (sy - 200/zoom)/2, 515/zoom, 200/zoom, tocolor(16,18,23, 255 * hideData.alpha), 15)
  dxDrawText("Przechowywalnia pojazdów", (sx - 515/zoom)/2, (sy - 200/zoom)/2, (sx + 515/zoom)/2, (sy - 200/zoom)/2 + 50/zoom, tocolor(44,181,233, 255 * hideData.alpha), 1/zoom, hideData.font, "center", "center")
  dxDrawText("Pojazd można bezpłatnie odebrać w tym miejscu, lub za drobną opłatą przewozową w innym, dowolnym punkcie parkingowym.", (sx - 515/zoom)/2 + 5/zoom, (sy - 200/zoom)/2 + 35/zoom, (sx + 515/zoom)/2 - 5/zoom, (sy + 200/zoom)/2 - 65/zoom, tocolor(170, 170, 170, 255 * hideData.alpha), 1/zoom, hideData.fontSmall, "center", "center", true, true)
end
