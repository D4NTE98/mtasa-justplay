...
function JailInfo:render()
    ...
    dxDrawText("Więzienie stanowe", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 40/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")


    if getElementData(localPlayer, "afk", true) then self.jailTick = getTickCount() end
    if not self.data.time then
        dxDrawText(string.format("Funkcjonariusz: %s\nPowód zamknięcia: %s\nPozostały czas: %s", self.data.police, self.data.reason, self:secondsToClock(0)), guiInfo.x + 110/zoom, guiInfo.y + 40/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "top", true, true)
    else
        dxDrawText(string.format("Funkcjonariusz: %s\nPowód zamknięcia: %s\nPozostały czas: %s", self.data.police, self.data.reason, self:secondsToClock(self.data.time - (getTickCount() - self.jailTick)/1000)), guiInfo.x + 110/zoom, guiInfo.y + 40/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "top", true, true)
