...
function PoliceJail:constructor(...)
    ...
    self.buttons = {}
    self.buttons.putToJail = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 100/zoom, 250/zoom, 40/zoom, "Osadź więźnia", "green")
    self.buttons.exit = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Zamknij", "red")
    exports.TR_dx:setButtonVisible(self.buttons, false)

    self.edits = {}
    self.edits.time = exports.TR_dx:createEdit((sx - 250/zoom)/2, guiInfo.y + 160/zoom, 250/zoom, 40/zoom, "Czas więzienia (minuty)")
    self.edits.reason = exports.TR_dx:createEdit((sx - 250/zoom)/2, guiInfo.y + 205/zoom, 250/zoom, 40/zoom, "Powód odsiadki")
...
function PoliceJail:render()
    ...
    self:drawOptionsBackground(guiInfo.x, guiInfo.y, guiInfo.w, guiInfo.h, tocolor(16,18,23, 255 * self.alpha), 14)
    dxDrawText("Więzienie stanowe", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    dxDrawText("Aby osadzić gracza w więzieniu upewnij się czy przewinienie którego się dopuścił jest odpowiednie aby trafił za kratki, a następnie wypełnij wszystkie poniższe informacje.", guiInfo.x + 10/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
end
...
function PoliceJail:buttonClick(btn)
    ...
    elseif btn == self.buttons.putToJail then
        ...
        if not cuffed then exports.TR_noti:create("Nie masz zakutej żadnej osoby.", "error") return end

        local time = guiGetText(self.edits.time)
        local reason = guiGetText(self.edits.reason)

        if utf8.len(time) < 1 or utf8.len(reason) < 1 then exports.TR_noti:create("Musisz wypełnić wszystkie dane.", "error") return end
        local time = tonumber(time)
        if time == nil then exports.TR_noti:create("Czas jest niepoprawny.", "error") return end
        if time < 1 or time > 300 then exports.TR_noti:create("Czas powinien mieścić się pomiędzy 1-300 minut.", "error") return end

        if utf8.len(reason) < 5 or utf8.len(reason) > 50 then exports.TR_noti:create("Powód musi mieścić się w granicach 5-50 znaków.", "error") return end
        if utf8.len(reason) < 5 or utf8.len(reason) > 50 then exports.TR_noti:create("Powód musi mieścić się w granicach 5-50 znaków.", "error") return end
...
function openPoliceJail(el, md)
    ...
    exports.TR_noti:create("Nie masz zakutej żadnej osoby.", "error")
end
...
function createPoliceMarkers()
    for i, v in pairs(PoliceJails) do
        ...
        setElementData(marker, "markerData", {
            title = "Panel więzienia",
            desc = "Wejdź w marker aby osadzić skutą osobę za kratkami.",
        }, false)
