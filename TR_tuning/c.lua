...
function Tuning:close()
    ...
    exports.TR_dx:showLoading(10000, "Wczytywanie świata")
...
function Tuning:selectCategory(category, blockScroll)
    ...
    if not category then
        ...
        if not self:canUseOption(self.tuningList[self.scroll]) then exports.TR_noti:create("Ta opcja przezonaczona jest tylko dla graczy premium.", "error") return end
...
function Tuning:renderPerformance()
    ...
    dxDrawRectangle(guiInfo.perf.x, guiInfo.perf.y, guiInfo.perf.w, guiInfo.perf.h, tocolor(23,25,31, 255))
    dxDrawText("Osiągi pojazdu", guiInfo.perf.x + 10/zoom, guiInfo.perf.y, guiInfo.perf.x + guiInfo.perf.w - 20/zoom, guiInfo.perf.y + 40/zoom, tocolor(44,181,233, 255), 1/zoom, self.fonts.performance, "center", "center")
    ...
    for i, v in pairs(guiInfo.perf.list) do
        ...
        dxDrawRectangle(guiInfo.perf.x + 10/zoom, guiInfo.perf.y + 65/zoom + (i-1) * 70/zoom, guiInfo.perf.w - 20/zoom, 30/zoom, tocolor(16,18,23, 255))
        ...
        if self.defaultData.handling[v.handling] and performanceValuesMax[v.handling] then
            if self.defaultData.handling[v.handling] > vehicleHandling[v.handling] then
                ...
                dxDrawRectangle(guiInfo.perf.x + 10/zoom, guiInfo.perf.y + 65/zoom + (i-1) * 70/zoom, (guiInfo.perf.w - 20/zoom) * vehicleHandling[v.handling]/performanceValuesMax[v.handling], 30/zoom, tocolor(34,36,43, 255))
            ...
            elseif self.defaultData.handling[v.handling] < vehicleHandling[v.handling] then
                ...
                dxDrawRectangle(guiInfo.perf.x + 10/zoom, guiInfo.perf.y + 65/zoom + (i-1) * 70/zoom, (guiInfo.perf.w - 20/zoom) * self.defaultData.handling[v.handling]/performanceValuesMax[v.handling], 30/zoom, tocolor(34,36,43, 255))
        ...
        elseif v.handling == "fuelTake" then
            ...
            if defaultFuel > nowFuel then
                ...
                dxDrawRectangle(guiInfo.perf.x + 10/zoom, guiInfo.perf.y + 65/zoom + (i-1) * 70/zoom, (guiInfo.perf.w - 20/zoom) * nowFuel/performanceValuesMax[v.handling], 30/zoom, tocolor(34,36,43, 255))
            ...
            elseif defaultFuel < nowFuel then
                ...
                dxDrawRectangle(guiInfo.perf.x + 10/zoom, guiInfo.perf.y + 65/zoom + (i-1) * 70/zoom, (guiInfo.perf.w - 20/zoom) * defaultFuel/performanceValuesMax[v.handling], 30/zoom, tocolor(34,36,43, 255))
...
function Tuning:drawTuningItem(x, y, w, h, item, alpha, font, lowerPrice)
    if item.selected then
        dxDrawImage(x, y, w, h, item.icon, 0, 0, 0, tocolor(44,181,233, alpha))
        dxDrawText(item.name, x, y + h + 10/zoom, x + w, y + h, tocolor(255, 255, 255, alpha), 1/zoom, font, "center", "top")
        dxDrawText("Zamontowane", x, y + h + lowerPrice, x + w, y + h, tocolor(44,181,233, alpha), 1/zoom, font, "center", "top")
        return
    end
            ...
            if self.selected == "main" then
                if not item.avaliableAll then
                    if not self.avaliableUpgrades[item.type] then
                        exports.TR_noti:create("Nie ma żadnych ulepszeń dostępnych do tego pojazdu w tej kategorii.", "error")
                        return
                    end
...
function Tuning:buyItem(item)
    ...
    -- if item.custom
    if item.type == "setUpgrade" then --or item.type == "removeUpgrade" then
...
function Tuning:response(...)
    ...
    if arg[1] then
        exports.TR_noti:create("Pomyślnie zakupiono część.", "success")
...
function createTuneWindow(type, data)
    ...
    exports.TR_dx:showLoading(10000, "Wczytywanie tunera")
...
-- if getPlayerName(localPlayer) == "DeMoNeK_" then
