...
function onMarkerHit(el, md)
    ...
    if #inVeh > 1 then exports.TR_noti:create(plr, "W pojeździe może siedzieć tylko kierowca.", "error") return end


    local plrUID = getElementData(plr, "characterUID")
    if owners[1] ~= plrUID then exports.TR_noti:create(plr, "Tuningować pojazd może tylko jego właściciel.", "error") return end

    local tunerData = getElementData(source, "tunerData")
    if tunerData.type == "visual" then
        if getVehicleType(el) ~= "Automobile" then exports.TR_noti:create(plr, "Nie możesz wjechać do tunera tym pojazdem.", "error") return end
        ...
        if tuneData and tuneData[1] then
            ...
            --setVehicleColor(el, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255)
        else
            exports.TR_noti:create(plr, "Nie możesz wjechać do tunera tym pojazdem.", "error")
            return
        end
