...
local snowZone = createColSphere(0, 0, 0, 3500)

function onWinerEnter(col)
    if col ~= snowZone then return end
    if getElementType(source) ~= "vehicle" then return end

    if getElementModel(source) == 453 then return end

    if getElementData(source, 'derby') == true then return end
...
function setHandling()
    ...
    local originalHandling = getModelHandling(getElementModel(453))
    local handling = getVehicleHandling(453)

    setVehicleHandling(453, "tractionMultiplier", originalHandling.tractionMultiplier)
    setVehicleHandling(453, "engineAcceleration", handling.engineAcceleration + 3)
    setVehicleHandling(453, "brakeDeceleration", originalHandling.engineAcceleration)
end
setHandling()