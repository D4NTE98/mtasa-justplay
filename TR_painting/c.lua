...
function Painting:render()
    ...
    dxDrawText("Lakierowanie pojazdu", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 30/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
...
function Painting:dxDrawProgressBar(x, y, w, h, progress)
    dxDrawRectangle(x, y, w * progress/100, h, tocolor(44,181,233, 255 * self.alpha))
...
function Painting:getSideName(side)
    if side == "front" then return "Przód" end
    if side == "back" then return "Tył" end
    if side == "left" then return "Lewa strona" end
    if side == "right" then return "Prawa strona" end
end
        ...
        if clear then
            ...
            if scx and scy and dist < 20 then
                if isElement(paintingPosition.mechanic) then
                    drawTextShadowed(string.format("#2ff539[STANOWISKO LAKIERNICZE]\n#ffffffNumer Stanowiska: #888888%d\n#ffffffMechanik: #888888%s", paintingPosition.ID, paintingPosition.mechanicName), scx, scy, scx, scy, tocolor(255, 255, 255, 255), 1 * (1 - dist/20), positionsFont, "center", "center", false, false, false, true)
                else
                    drawTextShadowed(string.format("#2ff539[STANOWISKO LAKIERNICZE]\n#ffffffNumer Stanowiska: #888888%d\n#ffffffMechanik: #888888%s", paintingPosition.ID, "Brak"), scx, scy, scx, scy, tocolor(255, 255, 255, 255), 1 * (1 - dist/20), positionsFont, "center", "center", false, false, false, true)
                end
            end
        end
    end
