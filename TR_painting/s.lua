...
function requestVehiclePaint(plr, vehicle)
    ...
    exports.TR_noti:create(client, "Propozycja lakierowania pojadzu została wysłana.", "info")
end
...
function declineVehiclePaint(plr)
    ...
    exports.TR_noti:create(plr, string.format("Gracz %s odrzucił propozycję lakierowania.", getPlayerName(client)), "error")
...
function playerPayForVehiclePaint(status, data)
    if status then
        if isElement(data[1]) then
            ...
            exports.TR_noti:create(source, "Lakiernik zabiera się już do pracy. Poczekaj aż pomaluje twój pojazd.", "info", 5)
            exports.TR_noti:create(data[1], "Aby polakierować pojazd należy równomiernie pokryć go farbą z każdej strony.", "info", 5)
        else
            local veh = getPedOccupiedVehicle(source)
            setElementFrozen(veh, true)

            painting[veh] = setTimer(paintVehicleFinal, 7000, 1, veh, data[2])
            exports.TR_noti:create(source, "Lakiernik oferujący malowanie opuścił służbę. Pojazd zostanie pomalowany automatycznie...", "info", 7)
        end
    end
...
function paintVehicleFinal(veh, colors, mechanic, pay)
    ...
    elseif isElement(mechanic) and pay then
        ...
        exports.TR_noti:create(mechanic, string.format("Pojazd został pomalowany.\nZarobiłeś: $%.2f.", 100), "success")
        exports.TR_weaponSlots:takeWeapon(mechanic, 41)
        removeElementData(mechanic, "blockAction")
    end
    if driver then
        triggerClientEvent(driver, "addAchievements", resourceRoot, "vehicleColor")
        exports.TR_noti:create(driver, "Pojazd został pomalowany.", "success")
    end
