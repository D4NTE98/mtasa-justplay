...
function PaintAccept:constructor(...)
    ...
    self.buttons.repair = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h - 50/zoom, 235/zoom, 40/zoom, "Potwierdź malowanie")
...
function PaintAccept:render()
    ...
    dxDrawText("Malowanie pojazdu", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
...
function PaintAccept:drawColors()
    for color = 0, 3 do
        if color == self.selectedTab then
            dxDrawRectangle(guiInfo.x + guiInfo.w - 105/zoom, guiInfo.y + 50/zoom + 32/zoom * color, 95/zoom, 32/zoom, tocolor(42,45,51, 255 * self.alpha))
        elseif self:isMouseInPosition(guiInfo.x + guiInfo.w - 105/zoom, guiInfo.y + 50/zoom + 32/zoom * color, 95/zoom, 32/zoom) then
            dxDrawRectangle(guiInfo.x + guiInfo.w - 105/zoom, guiInfo.y + 50/zoom + 32/zoom * color, 95/zoom, 32/zoom, tocolor(34,36,43, 255 * self.alpha))
        end
        if color == 2 then
          dxDrawText("Ranty", guiInfo.x + guiInfo.w - 25/zoom, guiInfo.y + 55/zoom + 32/zoom * color, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 75/zoom + 32/zoom * color, tocolor(180, 180, 180, 255 * self.alpha), 1/zoom, self.fonts.category, "right", "center")
        elseif color == 3 then
          dxDrawText("Felgi", guiInfo.x + guiInfo.w - 25/zoom, guiInfo.y + 55/zoom + 32/zoom * color, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 75/zoom + 32/zoom * color, tocolor(180, 180, 180, 255 * self.alpha), 1/zoom, self.fonts.category, "right", "center")
        else
          dxDrawText("Kolor "..(color + 1), guiInfo.x + guiInfo.w - 25/zoom, guiInfo.y + 55/zoom + 32/zoom * color, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 75/zoom + 32/zoom * color, tocolor(180, 180, 180, 255 * self.alpha), 1/zoom, self.fonts.category, "right", "center")
        end
...
function PaintAccept:buttonClick(...)
  ...
  elseif arg[1] == self.buttons.repair then
    ...
    if changedColors == 0 then exports.TR_noti:create("Wybierz kolor jaki chcesz zmienić.", "error") return end
