...
local guiInfo = {
    ...
    fractionSpawns = {
        ...
        },
        {
            ...
            name = "SIEDZIBA FRAKCJI | LOS SANTOS",
            premium = false,
            rot = 90,
            fractionID = 2,
        },
        ...
        },
        {
            ...
            name = "EMERGENCY ROAD SERVICES | RED COUNTY",
            premium = false,
            rot = 214,
            fractionID = 5,
        },
...
function SpawnSelect:render()
    ...
    dxDrawText("Aby zmienić miejsce użyj #2CB7ECSTRZAŁEK\n#aaaaaaAby zaakceptować naciśnij #2CB7ECENTER", guiInfo.text.x, guiInfo.text.y + 80/zoom, guiInfo.text.w, guiInfo.text.h, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", false, false, false, true)
end
...
function SpawnSelect:canSpawnInLocation()
    if not self.selectedData.premium then
        self.selectedData.canSpawn = "(DOSTĘPNE DLA WSZYSTKICH)"

    elseif self.selectedData.premium == "gold" then
        self.selectedData.canSpawn = "#d6a306(DOSTĘPNE DLA GRACZY GOLD)"

    elseif self.selectedData.premium == "diamond" then
        self.selectedData.canSpawn = "#31caff(DOSTĘPNE DLA GRACZY DIAMOND)"
    end
end

function SpawnSelect:acceptSpawnSelect()
    if self.state ~= "rotate" then
        exports.TR_noti:create("Aby potwierdzić wybór musisz poczekać aż kamera znajdzie się w wybranym miejscu.", "error")
        return
    end
...
function SpawnSelect:canRespawnInPlace()
    ...
    if self.selectedData.premium == "gold" then
        ...
        exports.TR_noti:create("Musisz posiadać konto Gold lub Diamond aby móc się zrespić w tym miejscu.", "error")

    elseif self.selectedData.premium == "diamond" then
        if plrData.premium == "diamond" then return true end
        exports.TR_noti:create("Musisz posiadać konto Diamond aby móc się zrespić w tym miejscu.", "error")
    end
