...
local guiInfo = {
  ...
  legend = [[#d4af37W,A,S,D #ffffff- sterowanie kamerą / obiektem
#d4af37LPM #ffffff- stawianie obiektu / zaznaczanie
#d4af37RPM #ffffff- obracanie kamery
#d4af37↑,↓  #ffffff- podnoszenie / opuszczanie
#d4af37←,→ #ffffff- obracanie
#d4af37ENTER #ffffff- akceptuj zmiany
#d4af37BACKSPACE #ffffff- anuluj zmiany
#d4af37DELETE #ffffff- usuń obiekt]],
}
...
guiInfo.furnitureCategories = {
  ...
  {"Biurka", "desk", 19},
  {"Stoły", "table", 43},
  {"Stoliki", "nightTable", 62},
  {"Regały", "shelf", 78},
  {"Krzesła", "furniture", 89},
  {"Kanapy", "sofa", 105},
  {"Łóżka", "bed", 124},
  {"Elektronika", "tv", 138},
  {"Różne", "trash", 167},
  {"Łazienka", "bathroom", 177},
  {"Kuchnia", "oven", 187},
  {"Siłownia", "sport", 216},
}
...
function click(btn, state)
  ...
  if btn == "left" and state == "down" then
    ...
    elseif guiInfo.buildingWall and isElement(guiInfo.buildingWall) then
      if #guiInfo.buildedObjects >= guiInfo.builderDetails.maxObjects then exports.TR_noti:create("Limit budowania został osiągnięty. Nie możesz postawić już nic więcej.", "error") return end
      ...
      if hit then
        if elementHit:getData("builder:type") == "floor" then
          ...
          if not canPlace then guiInfo.responseEnabled = nil return end
          exports.TR_dx:setResponseEnabled(true, "Trwa stawianie obiektu")
    ...
    elseif guiInfo.buildingFurniture and isElement(guiInfo.buildingFurniture) then
      ...
      if guiInfo.furniture.doorIDs[model] then
        if #guiInfo.buildedObjects >= guiInfo.builderDetails.maxObjects then exports.TR_noti:create("Limit budowania został osiągnięty. Nie możesz postawić już nic więcej.", "error") return end
        if #guiInfo.buildedObjects + 3 >= guiInfo.builderDetails.maxObjects then exports.TR_noti:create("Limit budowania zostanie przekroczony. Tworzenie drzwi zabiera 3 obiekty, które nie zmieszczą się już w limicie.", "error") return end

        if getElementModel(elementHit) ~= guiInfo.modelsID.floor then return end
...
function animateGui(state, nextTable)
  ...
  if isElement(guiInfo.editingElement) then
    if guiInfo.editingElementPositionDefault then
      ...
      exports.TR_noti:create("Edytowany obiekt został przywrócony do poprzedniego stanu.", "info")
    end
...
function gui()
  ...
  if isMouseInPosition(15/zoom, sh-36/zoom, 100/zoom, 32/zoom) then
    ...
    dxDrawText("Zamknij", 45/zoom, sh-40/zoom, 110/zoom, sh, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.fonts.normal, "left", "center")
  else
    dxDrawImage(15/zoom, sh-30/zoom, 20/zoom, 20/zoom, "files/icons/close.png", 0, 0, 0, tocolor(255, 255, 255, 255))
    dxDrawText("Zamknij", 45/zoom, sh-40/zoom, 110/zoom, sh, tocolor(255, 255, 255, 200), 1/zoom, guiInfo.fonts.normal, "left", "center")
  end
  ...
  if isMouseInPosition(150/zoom, sh-36/zoom, 85/zoom, 32/zoom) then
    ...
    dxDrawText("Zapisz", 180/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.fonts.normal, "left", "center")
  else
    dxDrawImage(150/zoom, sh-30/zoom, 20/zoom, 20/zoom, "files/icons/save.png", 0, 0, 0, tocolor(255, 255, 255, 255))
    dxDrawText("Zapisz", 180/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 200), 1/zoom, guiInfo.fonts.normal, "left", "center")
  end
  ...
  if isMouseInPosition(270/zoom, sh-36/zoom, 115/zoom, 32/zoom) then
    ...
    dxDrawText("Przetestuj", 300/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.fonts.normal, "left", "center")
  else
    dxDrawImage(270/zoom, sh-30/zoom, 20/zoom, 20/zoom, "files/icons/person.png", 0, 0, 0, tocolor(255, 255, 255, 255))
    dxDrawText("Przetestuj", 300/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 200), 1/zoom, guiInfo.fonts.normal, "left", "center")
  end
  ...
  if isMouseInPosition(415/zoom, sh-36/zoom, 150/zoom, 32/zoom) or guiInfo.selectedGuiList == "default" then
    ...
    dxDrawText("Projektowanie", 445/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.fonts.normal, "left", "center")
  else
    dxDrawImage(415/zoom, sh-30/zoom, 20/zoom, 20/zoom, "files/icons/build.png", 0, 0, 0, tocolor(255, 255, 255, 255))
    dxDrawText("Projektowanie", 445/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 200), 1/zoom, guiInfo.fonts.normal, "left", "center")
  end
  ...
  if isMouseInPosition(595/zoom, sh-36/zoom, 150/zoom, 32/zoom) or guiInfo.selectedGuiList == "building" then
    ...
    dxDrawText("Meblowanie", 630/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.fonts.normal, "left", "center")
  else
    dxDrawImage(600/zoom, sh-30/zoom, 20/zoom, 20/zoom, "files/icons/furniture.png", 0, 0, 0, tocolor(255, 255, 255, 255))
    dxDrawText("Meblowanie", 630/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 200), 1/zoom, guiInfo.fonts.normal, "left", "center")
  end
  ...
  if isMouseInPosition(760/zoom, sh-36/zoom, 165/zoom, 32/zoom) or guiInfo.selectedGuiList == "textures" then
    ...
    dxDrawText("Teksturowanie", 795/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.fonts.normal, "left", "center")
  else
    dxDrawImage(765/zoom, sh-30/zoom, 20/zoom, 20/zoom, "files/icons/painting.png", 0, 0, 0, tocolor(255, 255, 255, 255))
    dxDrawText("Teksturowanie", 795/zoom, sh-40/zoom, 220/zoom, sh, tocolor(255, 255, 255, 200), 1/zoom, guiInfo.fonts.normal, "left", "center")
  end
  ...
  if guiInfo.confirmInteriorQuit then
    drawBackground((sw-420/zoom)/2, (sh-140/zoom)/2, 420/zoom, 140/zoom, tocolor(23,25,31, 255), 5)
    dxDrawText("Wyjście z edytora spowoduje usunięcie\nwszystkich niezapisanych zmian.\n#2CB7ECCzy na pewno chcesz wyjść?", (sw-410/zoom)/2, (sh-140/zoom)/2, (sw+410/zoom)/2, (sh+35/zoom)/2, tocolor(200, 200, 200, 255), 1/zoom, guiInfo.fonts.normal, "center", "center", true, true, false, true)
  end
end
...
function openInteriorBuilder(int, idHome, homeBuilded, homePos, oryginalWalls, oryginalFloor, exitPOS, gangObjects)
  setTimer(function()
    ...
    guiInfo.quitCancle = exports.TR_dx:createButton((sw - 200/zoom)/2 - 105/zoom, sh/2 + 25/zoom, 200/zoom, 40/zoom, "Anuluj")
    guiInfo.quitAccept = exports.TR_dx:createButton((sw - 200/zoom)/2 + 105/zoom, sh/2 + 25/zoom, 200/zoom, 40/zoom, "Wyjdź")
...
function closeInteriorBuilder()
  exports.TR_dx:showLoading(4000, "Wczytywanie świata")
  setTimer(function()
...
function setPlayerTestingInterior()
  if guiInfo.testingByPlayer then
    ...
    clearBuildingObjects()
    guiInfo.backNoti = exports.TR_noti:create("Aby opuścić tryb testowania naciśnij ENTER.", "info", false, true)
...
function cantPlaceMessage()
  exports.TR_noti:create("Nie możesz tego tutaj postawić.", "error")
  return false
end
...
addCommandHandler("photo", function(cmd, type)
  ...
  if type == "all" then
    ...
    print("Generowanie zdjęć wszystkich obiektów potrwa ".. (#guiInfo.furnitureList * 1000)/1000 .. "s")
  else
