...
function createExchanges()
    for i, v in pairs(VehicleExchanges) do
        ...
        setElementData(marker, "markerData", {
            ...
            desc = string.format("Wystawianie pojazdu\n$%d - $%d", v.minPrice, v.maxPrice)
        })
...
function buyVehicleExchange(veh)
    if not canBuyVehicle(client) then exports.TR_noti:create(client, "Osiągnąłeś limit po. Ulepsz swoje konto bądź powiększ limit pojazdów.", "error") return end

    local exchangeData = getElementData(veh, "exchangeData")
    if not exchangeData then return exports.TR_noti:create(client, "Te auto nie jest już na sprzedaż.", "info") end
    ...
    if checkBought and checkBought[1] then
        if checkBought[1].exchangePrice == "skurwiel do wyjebania" then
            exports.TR_noti:create(client, "Te auto zostało już zakupione.", "error")
            return
        end
    end
    ...
    if exchangeData.price > money then return exports.TR_noti:create(client, "Nie posiadasz przy sobie tylu pieniędzy.", "error") end
    if exports.TR_core:takeMoneyFromPlayer(client, exchangeData.price) then
        ...
        if isElement(seller) then
            if getElementData(seller, "characterUID") == exchangeData.ownerUID then
                ...
                exports.TR_noti:create(seller, "Twój pojazd wystawiony na giełdzie został właśnie sprzedany.", "success")
            else
        ...
        exports.TR_noti:create(client, "Auto zostało pomyślnie zakupione.", "success")
        ...
        exports.TR_discord:sendChannelMsg("vehicleBuy", {
          ...
          text = string.format("zakupił pojazd na giełdzie o ID %d od gracza %s za kwotę $%.2f.", vehData.ID, exchangeData.owner, exchangeData.price),
        })
