...
function Exchange:render()
    ...
    dxDrawText("Giełda samochodowa", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    dxDrawText("Wpisz cenę za jaką chcesz wystawić pojazd na giełdzie.", guiInfo.x + 10/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
...
function Exchange:proceed()
    ...
    if tonumber(text) == nil then return exports.TR_noti:create("Wpisana cena jest niepoprawna.", "error") end

    local price = math.floor(tonumber(text) * 100)/100
    if price == 0 then return exports.TR_noti:create("Wpisana cena jest zbyt niska.", "error") end
    if price < self.minPrice then return exports.TR_noti:create("Wpisana cena jest zbyt niska.", "error") end
    if price > self.maxPrice then return exports.TR_noti:create("Wpisana cena jest zbyt wysoka.", "error") end

    exports.TR_noti:create("Pojazd został pomyślnie wystawiony. Zaparkuj go na wolnym miejscu.", "success")
