...
function Info:renderTarget(veh)
    ...
    if exchangeData.price == "skurwiel do wyjebania" then
        self:removeInfo(veh)
    ...
    self:drawBackground(0, 0, guiInfo.w, self.infos[veh].height, tocolor(23,25,31, 255), 11)

    dxDrawText(vehName or "", 0, 5, guiInfo.w, self.infos[veh].height, tocolor(44,181,233, 255), 1, self.fonts.name, "center", "top")

    dxDrawText(string.format("UID Pojazdu: %d", vehID or ""), 10, 30, guiInfo.w, self.infos[veh].height, tocolor(200, 200, 200, 255), 1, self.fonts.info, "left", "top")
    dxDrawText(string.format("Przebieg: %.2fkm", exchangeData.mileage or 0.26), 10, 45, guiInfo.w, self.infos[veh].height, tocolor(200, 200, 200, 255), 1, self.fonts.info, "left", "top")
    dxDrawText(string.format("Silnik: %s", exchangeData.engineCapacity or "ERROR"), 10, 60, guiInfo.w, self.infos[veh].height, tocolor(200, 200, 200, 255), 1, self.fonts.info, "left", "top")

    dxDrawText("Kolor:", 10, 75, guiInfo.w, self.infos[veh].height, tocolor(200, 200, 200, 255), 1, self.fonts.info, "left", "top")
    ...
    dxDrawText("Światła:", 10, 90, guiInfo.w, self.infos[veh].height, tocolor(200, 200, 200, 255), 1, self.fonts.info, "left", "top")
    dxDrawRectangle(67, 94, 10, 10, tocolor(lights.x, lights.y, lights.z, 255))

    dxDrawText(string.format("Ulepszenia: %s", upgrades), 10, 105, guiInfo.w - 10, self.infos[veh].height - 10, tocolor(200, 200, 200, 255), 1, self.fonts.info, "left", "top", true, true)


    dxDrawText(string.format("$%.2f", exchangeData.price or 0), 10, 45, guiInfo.w - 10, self.infos[veh].height - 30, tocolor(44,181,233, 255), 1, self.fonts.name, "center", "bottom")
    dxDrawText(string.format("Sprzedający: %s", exchangeData.owner or "Ktoś"), 10, 45, guiInfo.w - 10, self.infos[veh].height - 10, tocolor(200, 200, 200, 255), 1, self.fonts.info, "center", "bottom")
