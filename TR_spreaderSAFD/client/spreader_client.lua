...
getComponentID = function(element)
    ...
    if (getBodyType(vehicle)) == "2 drzwi" then
    ...
    elseif (getBodyType(vehicle)) == "2 drzwi, mniejsze" then
        if (relativePos() >= 140) and (relativePos() <= 220) then
            return 0;
        end;
    ...
    elseif (getBodyType(vehicle)) == "4 drzwi" then
        if (relativePos() >= 140) and (relativePos() <= 220) then
            return 0;
        end;
    ...
    elseif (getBodyType(vehicle)) == "Van" then
        if (relativePos() >= 140) and (relativePos() <= 220) then
            return 0;
        end;
    ...
    elseif (getBodyType(vehicle)) == "Ciezarowe" then
...
progressBarRender = function()
    ...
    drawBackground(guiInfo.x, guiInfo.y, guiInfo.w, guiInfo.h, tocolor(23,25,31, 255), 25)

    dxDrawText("Rozpieranie", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 40/zoom, tocolor(44,181,233, 255), 1/zoom, guiInfo.fonts.main, "center", "center")
    drawBackground(guiInfo.x + 10/zoom, guiInfo.y + 40/zoom, guiInfo.w - 20/zoom, 30/zoom, tocolor(34,36,43, 255), 15)
    drawBackground(guiInfo.x + 12/zoom, guiInfo.y + 42/zoom, (guiInfo.w - 24/zoom) * progress, 26/zoom, tocolor(44,181,233, 255), 15)
    ...
    dxDrawText("Po napełnieniu się wskaźnika rozpieranie zostanie zakończone.", guiInfo.x, guiInfo.y + 70/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h, tocolor(170, 170, 170, 255), 1/zoom, guiInfo.fonts.small, "center", "center")
end;
