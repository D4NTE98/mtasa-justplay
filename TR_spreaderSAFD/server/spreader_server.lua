	...
	if (component) then
		...
		if (getElementType(vehicle) == 'vehicle') then
			...
			if (isPedInVehicle(source)) then
				outputChatBox('Co ty chciałeś w pojeżdzie coś ciąć?', source);
				return;
			end;

			if (getElementData(source, 'rozpieranie') == true) then
				outputChatBox('Trwa już rozpieranie!', source);
				return;
			end;

			if (getVehicleDoorState(vehicle, component) == 4) then
				outputChatBox('Nie możesz wyważyć tego komponentu bo go nie ma!', source);
				return;
			end;
			...
			outputChatBox('Trwa rozpieranie...', source);
			...
			timer2[player] = setTimer(function()
				...
				outputChatBox('Rozpieranie zakończone.', player);
