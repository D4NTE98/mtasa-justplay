...
local guiInfo = {
    ...
    card = {
        ...
        prizes = [[
 #aaaaaa- Karta #d45555powtórki
 #aaaaaa- Karta #7651b8używki #aaaaaa(5min - 10min)
 #aaaaaa- Karta #f28e1cimprezy #aaaaaa(5min - 10min)
 #aaaaaa- Karta #4c9c3dpieniędzy #aaaaaa($150 - $500)
 #aaaaaa- Karta #d6a306Gold #aaaaaa(1dzień - 5dni)
 #aaaaaa- Karta #31caffDiamond #aaaaaa(1dzień - 5dni)
        ]],
    },

    categories = {
        {"Moja postać", "player"},
        {"Opis postaci", "description"},
        {"Cechy postaci", "skills"},
        {"Dostępne pojazdy", "vehicle"},
        {"Dostępne posiadłości", "house"},
        {"Osiągnięcia", "medal"},
        {"Znajomi", "friends"},
        {"Logi konta", "logs"},
        {"Otrzymane kary", "penalties"},
        {"Dzienna loteria", "cards"},
        {"Zadania dzienne", "tasks"},
        {"Kod referencyjny", "reference"},
        {"Sklep Premium", "points"},
        {"Ustawienia gry", "settings"},
        {"Ustawienia grafiki", "graphic"},
    },
    ...
    settingsList = {
        ["game"] = {
            ...
            {
                type = "option",
                value = "Minimalistyczny chat",
                switch = "chatMinified",
            },
            {
                type = "option",
                value = "Ogłoszenia graczy",
                switch = "advertisement",
            },
            {
                type = "option",
                value = "Kompas minimapy",
                switch = "windRose",
            },
            {
                type = "",
            ...
            {
                type = "option",
                value = "Wiadmości prywatne",
                switch = "smsOff",
            },
            {
                type = "option",
                value = "Wiadomości premium",
                switch = "premium",
            },
            {
                type = "option",
                value = "Nowe dźwięki silników",
        ...
        ["graphic"] = {
            ...
            {
                type = "option",
                value = "Realistyczna woda",
                switch = "water",
            },
            {
                type = "option",
                value = "Realistyczne niebo",
                switch = "sky",
            },
            {
                type = "option",
                value = "Realistyczne kolory",
                switch = "colors",
            },
            {
                type = "option",
                value = "Realistyczne tekstury",
                switch = "textures",
            },
            {
                type = "option",
                value = "Realistyczna karoseria",
                switch = "vehicles",
            },
            {
                type = "option",
                value = "Realistyczna noc",
                switch = "realisticNight",
            },
            {
                type = "option",
                value = "Dalekie renderowanie",
                switch = "farClipDistance",
            },
            {
                type = "option",
                value = "Rozmycie podczas jazdy",
                switch = "blurLevel",
            },
            {
                type = "",
    ...
    skills = {
        ["positive"] = {
            {
                name = "Siła",
                desc = "Stopień umięśnienia postaci.",
                icon = "strenght",
                index = 1,
                hoverInfo = {
                    {"PLUSY POSIADANIA CECHY", "info"},
                    {"•	 Noszenie większej liczby przedmiotów", "plus"},
                    {"•	 Zwiększenie obrażeń wręcz i bronią białą", "plus"},
                    {""},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień umięśnienia postaci", "text"},
                    {"należy podnosić ciężary na siłowni.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Cecha samoczynnie zmniejsza się co jakiś", "text"},
                    {"czas jeśli nic nie trenujemy.", "text"},
                },
            },
            {
                name = "Wytrzymałość",
                desc = "Stopień wytrzymałości postaci.",
                icon = "lungs",
                index = 2,
                hoverInfo = {
                    {"PLUSY POSIADANIA CECHY", "info"},
                    {"•	 Większy wdech przed zanurkowaniem", "plus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień wytrzymałości", "text"},
                    {"postaci, należy jak najwięcej biegać", "text"},
                    {"sprintem lub skorzystać z bieżni na siłowni.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Cecha samoczynnie zmniejsza się co jakiś", "text"},
                    {"czas jeśli nie ruszamy się zbyt często.", "text"},
                },
            },
            {
                name = "Prowadzenie pojazdów",
                desc = "Stopień wyszkolenia w prowadzeniu.",
                icon = "steer",
                index = 3,
                hoverInfo = {
                    {"PLUSY POSIADANIA CECHY", "info"},
                    {"•	 Kolizje zmniejszają obrażenia pojazdu", "plus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień wyszkolenia", "text"},
                    {"w prowadzeniu należy jak najwięcej", "text"},
                    {"prowadzić pojazdy różnego typu.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Cecha samoczynnie zmniejsza się co jakiś", "text"},
                    {"czas jeśli nie prowadzimy żadnego pojazdu.", "text"},
                },
            },
            {
                name = "Strzelanie z broni",
                desc = "Stopień przeszkolenia w strzelaniu.",
                icon = "weapon",
                index = 4,
                hoverInfo = {
                    {"PLUSY POSIADANIA CECHY", "info"},
                    {"•	 Zwiększenie obrażeń z broni palnej", "plus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień przeszkolenia", "text"},
                    {"w strzelaniu należy jak najwięcej", "text"},
                    {"strzelać z różnego rodzaju broni.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Cecha samoczynnie zmniejsza się co jakiś", "text"},
                    {"czas jeśli nie strzelamy z broni.", "text"},
                },
            },
            {
                name = "Wiedza medyczna",
                desc = "Stopień przeszkolenia medycznego.",
                icon = "medicine",
                index = 5,
                hoverInfo = {
                    {"PLUSY POSIADANIA CECHY", "info"},
                    {"•	 Zmniejszenie otrzymywanych obrażeń", "plus"},
                    {"•	 Przedmioty leczące są bardziej efektywne", "plus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień przeszkolenia", "text"},
                    {"medycznego należy udzielać pomocy", "text"},
                    {"medycznej innym graczom.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Cecha samoczynnie zmniejsza się co jakiś", "text"},
                    {"czas jeśli nie udzielamy pomocy medycznej.", "text"},
                },
            },
        },
        ["negative"] = {
            {
                name = "Otyłość",
                desc = "Stopień otyłości postaci.",
                icon = "fat",
                index = 6,
                hoverInfo = {
                    {"MINUSY POSIADANIA CECHY", "info"},
                    {"•	 Brak możliwości sprintu", "minus"},
                    {"•	 Brak możliwości skakania", "minus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień otyłości", "text"},
                    {"postaci należy jeść dużo jedzenia.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Aby zmniejszyć stopień otyłości należy", "text"},
                    {"ćwiczyć na siłowni.", "text"},
                },
            },
            {
                name = "Hazard",
                desc = "Stopień uzależnienia od hazardu.",
                icon = "casino",
                index = 7,
                hoverInfo = {
                    {"MINUSY POSIADANIA CECHY", "info"},
                    {"•	 Spontaniczne wejścia ALL-IN podczas", "minus"},
                    {"   gry w kasynie", "minus"},
                    {"•	 Kupowanie większej ilości żetonów", "minus"},
                    {"   w kasynie", "minus"},
                    {"", "text"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień uzależnienia", "text"},
                    {"od hazardu należy często grać w kasynie.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Aby zmniejszyć stopień uzależnienia", "text"},
                    {"od hazardu należy odstawić grę w kasynie.", "text"},
                    {"Przy wyższym poziomie uzależnienia", "text"},
                    {"możesz się z niego wyleczyć u psychologa.", "text"},
                },
            },
            {
                name = "Alkoholizm",
                desc = "Stopień uzależnienia od alkoholu.",
                icon = "cheers",
                index = 8,
                hoverInfo = {
                    {"MINUSY POSIADANIA CECHY", "info"},
                    {"•	 Konsekwentna utrata życia", "minus"},
                    {"•	 Wieczne życie na kacu z efektem bycia", "minus"},
                    {" 	  pijanym", "minus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień uzależnienia", "text"},
                    {"od alkoholu należy często pić alkohol.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Aby zmniejszyć stopień uzależnienia", "text"},
                    {"od alkoholu należy odstawić picie.", "text"},
                    {"Przy wyższym poziomie uzależnienia", "text"},
                    {"możesz się z niego wyleczyć u psychologa.", "text"},
                },
            },
            {
                name = "Nikotynizm",
                desc = "Stopień uzależnienia od nikotyny.",
                icon = "smoking",
                index = 9,
                hoverInfo = {
                    {"MINUSY POSIADANIA CECHY", "info"},
                    {"•	 Konsekwentna utrata życia", "minus"},
                    {"•	 Blokuje możliwość zwiększenia", "minus"},
                    {"   wytrzymałości postaci", "minus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień uzależnienia", "text"},
                    {"od nikotyny należy często palić papierosy.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Aby zmniejszyć stopień uzależnienia", "text"},
                    {"od nikotyny należy rzucić palenie.", "text"},
                    {"Przy wyższym poziomie uzależnienia", "text"},
                    {"możesz się z niego wyleczyć u psychologa.", "text"},
                },
            },
            {
                name = "Narkomania",
                desc = "Stopień uzależnienia od narkotyków.",
                icon = "pills",
                index = 10,
                hoverInfo = {
                    {"MINUSY POSIADANIA CECHY", "info"},
                    {"•	 Konsekwentna utrata życia", "minus"},
                    {"•	 Losowe efekty narkotyczne bez zażywania", "minus"},
                    {" 	  narkotyków", "minus"},
                    {"", "plus"},
                    {"JAK WBIJAĆ CECHĘ", "info"},
                    {"Aby zwiększyć stopień uzależnienia", "text"},
                    {"od narkotyków należy często zażywać", "text"},
                    {"narkotyki.", "text"},
                    {""},
                    {"JAK ZBIJAĆ CECHĘ", "info"},
                    {"Aby zmniejszyć stopień uzależnienia", "text"},
                    {"od natkotyków należy odstawić ich zażywanie.", "text"},
                    {"Przy wyższym poziomie uzależnienia", "text"},
                    {"możesz się z niego wyleczyć u psychologa.", "text"},
                },
            },
    ...
    shopItems = {
        {
            item = "gold", -- przedmiot
            iconName = "crown", -- nazwa tekstury
            quantityAvailable = {5, 10, 15}, -- dostępne ilości
            prices = {
                [5] = 500,
                [10] = 950,
                [15] = 1400,
...
function Dashboard:open(...)
    ...
    self.pointsPreview = nil
    self.pointsSelected = 1
...
function Dashboard:render()
    ...
    dxDrawRectangle(guiInfo.bg.x, guiInfo.bg.y, guiInfo.bg.w, guiInfo.bg.h, tocolor(16,18,23, 255 * self.alpha))
    dxDrawRectangle(guiInfo.category.x, guiInfo.category.y, guiInfo.category.w, guiInfo.category.h, tocolor(23,25,31, 255 * self.alpha))
...
function Dashboard:renderCategory()
    if not self.loaded then
        ...
        dxDrawText("Trwa wczytywanie danych...", guiInfo.bg.x + (guiInfo.bg.w - 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 + 10/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
        return
    end

    local tab = guiInfo.categories[self.tab][2]
    if tab == "player" then
        dxDrawText("INFORMACJE O KONCIE", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 - 30/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText(string.format("UID: #2cb5e9%s", self.playerData.UID), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Serial właściciela: #2cb5e9%s", self.revealData and self.playerData.serial or "Ukryte"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 25/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Adres IP właściciela: #2cb5e9%s", self.revealData and self.playerData.createIP or "Ukryte"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 50/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Aktualny adres IP gracza: #2cb5e9%s", self.revealData and self.playerData.ip or "Ukryte"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 75/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)

        dxDrawText("INFORMACJE O POSTACI", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 140/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText(string.format("Nick: #2cb5e9%s", self.playerData.name), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 170/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("ID Skina: #2cb5e9%s", tostring(self.playerData.skin)), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 195/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Data dołączenia: #2cb5e9%s", self.playerData.created), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 220/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Łączny czas online: #2cb5e9%s", self:getTimeInSeconds(self.playerData.online + (getTickCount() - self.onlineTime)/1000)), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 245/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Aktualny czas online: #2cb5e9%s", self:getTimeInSeconds((getTickCount() - self.onlineTime)/1000)), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 270/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Gotówka przy sobie: #2cb5e9$%.2f", self.playerData.money), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 295/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Pieniądze na koncie: #2cb5e9%s", self.playerData.bankcode and string.format("$%.2f", self.playerData.bankmoney) or "Brak konta"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 320/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Posiadane licencje: #2cb5e9%s", self.playerData.licence or "Brak"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 345/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Posiadane prawa jazdy: #2cb5e9%s", self.playerData.vehicleLicence), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 370/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Doświadczenie postaci: #2cb5e9%d pkt", getElementData(localPlayer, "characterPoints") or 0), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 395/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)

        dxDrawText("Pokazuj rendery", guiInfo.bg.x + guiInfo.bg.w/2 + 60/zoom, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 + 100/zoom, guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 + 130/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "center", false, false, false, true)
    ...
    elseif tab == "friends" then
        dxDrawText("LISTA ZNAJOMYCH", sx/2 - 460/zoom, sy/2 - 400/zoom, sx/2 + 600/zoom, sy, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top")
        ...
        if #self.friendsData > 0 then
            ...
            for k = 1, 5 * 2 do
                ...
                if v then
                    if k%2 == 1 then
                        ...
                        dxDrawText(string.format("%s (UID: %d)", v.username, v.UID), sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top")

                        if not v.friendsFor then
                            if v.isTarget then
                                dxDrawText("Oczekuje na akceptację...", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")
                                ...
                                if self:isMouseInPosition(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom) then
                                    ...
                                    dxDrawText("Akceptuj", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 - 250/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                else
                                    self:drawBackground(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(37, 67, 37, 255 * self.alpha), 4)
                                    dxDrawText("Akceptuj", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 - 250/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                end
                                ...
                                if self:isMouseInPosition(sx/2 - 245/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 200/zoom, 30/zoom) then
                                    ...
                                    dxDrawText("Odrzuć", sx/2 - 245/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 - 145/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                else
                                    self:drawBackground(sx/2 - 245/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(67, 37, 37, 255 * self.alpha), 4)
                                    dxDrawText("Odrzuć", sx/2 - 245/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 - 145/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                end

                            else
                                dxDrawText("Zaproszenie zostało wysłane...", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

                                if self:isMouseInPosition(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom) then
                                    self:drawBackground(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(87, 37, 37, 255 * self.alpha), 4)
                                    dxDrawText("Anuluj", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 - 250/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                else
                                    self:drawBackground(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(67, 37, 37, 255 * self.alpha), 4)
                                    dxDrawText("Anuluj", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 - 250/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                end
                            end
                        else
                            dxDrawText(string.format("Ostatnio online: %s", v.lastOnline), sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")
                            dxDrawText(string.format("Znajomi od: %s", v.friendsFor), sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 48/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")
                            ...
                            if self:isMouseInPosition(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, 200/zoom, 30/zoom) then
                                ...
                                dxDrawText("Usuń ze znajomych", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, sx/2 - 150/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 105/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                            else
                                self:drawBackground(sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, 200/zoom, 30/zoom, tocolor(67, 37, 37, 255 * self.alpha), 4)
                                dxDrawText("Usuń ze znajomych", sx/2 - 350/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, sx/2 - 150/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 105/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                            end
                        end
                        ...
                        dxDrawText(string.format("%s (UID: %d)", v.username, v.UID), sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top")

                        if not v.friendsFor then
                            if v.isTarget then
                                dxDrawText("Oczekuje na akceptację...", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")
                                ...
                                if self:isMouseInPosition(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom) then
                                    ...
                                    dxDrawText("Akceptuj", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 + 400/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                else
                                    self:drawBackground(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(37, 67, 37, 255 * self.alpha), 4)
                                    dxDrawText("Akceptuj", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 + 400/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                end
                                ...
                                if self:isMouseInPosition(sx/2 + 405/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 200/zoom, 30/zoom) then
                                    ...
                                    dxDrawText("Odrzuć", sx/2 + 405/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 + 505/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                else
                                    self:drawBackground(sx/2 + 405/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(67, 37, 37, 255 * self.alpha), 4)
                                    dxDrawText("Odrzuć", sx/2 + 405/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 + 505/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                end

                            else
                                dxDrawText("Zaproszenie zostało wysłane...", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

                                if self:isMouseInPosition(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom) then
                                    self:drawBackground(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(87, 37, 37, 255 * self.alpha), 4)
                                    dxDrawText("Anuluj", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 + 400/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                else
                                    self:drawBackground(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, 100/zoom, 30/zoom, tocolor(67, 37, 37, 255 * self.alpha), 4)
                                    dxDrawText("Anuluj", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 55/zoom, sx/2 + 400/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 85/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                                end
                            end
                        else
                            dxDrawText(string.format("Ostatnio online: %s", v.lastOnline), sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")
                            dxDrawText(string.format("Znajomi od: %s", v.friendsFor), sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 48/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")
                            ...
                            if self:isMouseInPosition(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, 200/zoom, 30/zoom) then
                                ...
                                dxDrawText("Usuń ze znajomych", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, sx/2 + 500/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 105/zoom, tocolor(255, 255, 255, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                            else
                                self:drawBackground(sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, 200/zoom, 30/zoom, tocolor(67, 37, 37, 255 * self.alpha), 4)
                                dxDrawText("Usuń ze znajomych", sx/2 + 300/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 75/zoom, sx/2 + 500/zoom, sy/2 - 340/zoom + (i-1) * 120/zoom + 105/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                            end
                        end
            ...
            dxDrawText("Nie posiadasz jeszcze żadnych znajomych!\nNie krępuj się i dodaj jakiegoś użytkownika!", sx/2 - 460/zoom, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 - 20/zoom, sx/2 + 600/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
        end

        dxDrawText("DODAJ ZNAJOMEGO", sx/2 - 460/zoom, sy/2 + 320/zoom, sx/2 + 600/zoom, sy, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top")

    elseif tab == "medal" then
        dxDrawText("OSIĄGNIĘCIA", sx/2 - 460/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 - 70/zoom, sx/2 + 600/zoom, sy, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top")
            ...
            if v then
                if k%2 == 1 then
                    if v.earned then
                        ...
                        dxDrawImage(sx/2 - 460/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom, 80/zoom, 80/zoom, "files/images/medal.png", 0, 0, 0, tocolor(44,181,233, 255 * self.alpha))
                        ...
                        dxDrawText(string.format("Zdobyto: %s", v.earned), sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

                        self:drawBackground(sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom, 25/zoom, tocolor(23,25,31, 255 * self.alpha), 4)
                        self:drawBackground(sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom * percent/100, 25/zoom, tocolor(44,181,233, 150 * self.alpha), 4)
                        dxDrawText(string.format("Zdobyło %.2f%% (%d/%d) graczy", percent * 100, self.earnedAchievementCount[v.ID] or 0, self.totalPlayers), sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, guiInfo.bg.x + 610/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 78/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                        ...
                        dxDrawText(v.notEarned, sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 28/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 150 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

                        self:drawBackground(sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom, 25/zoom, tocolor(23,25,31, 255 * self.alpha), 4)
                        dxDrawText("Zdobyło ??? graczy", sx/2 - 350/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, guiInfo.bg.x + 610/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 78/zoom, tocolor(140, 140, 140, 150 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                    end

                else
                    if v.earned then
                        local percent = (self.earnedAchievementCount[v.ID] or 0)/self.totalPlayers
                        dxDrawImage(sx/2 + 190/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom, 80/zoom, 80/zoom, "files/images/medal.png", 0, 0, 0, tocolor(44,181,233, 255 * self.alpha))
                        ...
                        dxDrawText(string.format("Zdobyto: %s", v.earned), sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 28/zoom, sx/2 + 700/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

                        self:drawBackground(sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom, 25/zoom, tocolor(23,25,31, 255 * self.alpha), 4)
                        self:drawBackground(sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom * percent/100, 25/zoom, tocolor(44,181,233, 150 * self.alpha), 4)
                        dxDrawText(string.format("Zdobyło %.2f%% (%d/%d) graczy", percent, self.earnedAchievementCount[v.ID] or 0, self.totalPlayers), sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, sx/2 + 600/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 78/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                        ...
                        dxDrawText(v.notEarned, sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 28/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 150 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

                        self:drawBackground(sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom, 25/zoom, tocolor(23,25,31, 255 * self.alpha), 4)
                        dxDrawText("Zdobyło ??? graczy", sx/2 + 300/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 53/zoom, sx/2 + 600/zoom, sy/2 - 120/zoom * guiInfo.achievementsCount/2 + (i-1) * 120/zoom + 78/zoom, tocolor(140, 140, 140, 150 * self.alpha), 1/zoom, self.fonts.small, "center", "center")
                    end
        ...
        dxDrawText(string.format("Zdobyte osiągnięcia: %d%% (%d/%d)", self.achievementsEarned/#self.achievements * 100, self.achievementsEarned, #self.achievements), sx/2 - 460/zoom, sy/2 + 120/zoom * guiInfo.achievementsCount/2, sx/2 + 600/zoom, sy, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.premium, "center", "top")

    elseif tab == "skills" then
        dxDrawText("CECHY POZYTYWNE", guiInfo.bg.x + 200/zoom, sy/2 - 120/zoom * #guiInfo.skills["positive"]/2 - 50/zoom, sx/2 + 50/zoom, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
        ...
        for i, v in pairs(guiInfo.skills["positive"]) do
            ...
            dxDrawText(v.desc, sx/2 - 350/zoom, sy/2 - 120/zoom * #guiInfo.skills["positive"]/2 + (i-1) * 120/zoom + 28/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

            self:drawBackground(sx/2 - 350/zoom, sy/2 - 120/zoom * #guiInfo.skills["positive"]/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom, 25/zoom, tocolor(23,25,31, 255 * self.alpha), 4)
            self:drawBackground(sx/2 - 350/zoom, sy/2 - 120/zoom * #guiInfo.skills["positive"]/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom * percent, 25/zoom, tocolor(44,181,233, 150 * self.alpha), 4)
        ...
        dxDrawText("CECHY NEGATYWNE", sx/2 + 190/zoom, sy/2 - 120/zoom * #guiInfo.skills["negative"]/2 - 50/zoom, sx/2 + 690/zoom, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
        ...
        for i, v in pairs(guiInfo.skills["negative"]) do
            ...
            dxDrawText(v.desc, sx/2 + 300/zoom, sy/2 - 120/zoom * #guiInfo.skills["negative"]/2 + (i-1) * 120/zoom + 28/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.goldDate, "left", "top")

            self:drawBackground(sx/2 + 300/zoom, sy/2 - 120/zoom * #guiInfo.skills["negative"]/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom, 25/zoom, tocolor(23,25,31, 255 * self.alpha), 4)
            self:drawBackground(sx/2 + 300/zoom, sy/2 - 120/zoom * #guiInfo.skills["negative"]/2 + (i-1) * 120/zoom + 53/zoom, 300/zoom * percent, 25/zoom, tocolor(44,181,233, 150 * self.alpha), 4)
        ...
        dxDrawText("Najedź na cechę aby dowiedzieć się więcej.", sx/2 - 460/zoom, sy/2 + 120/zoom * #guiInfo.skills["negative"]/2, sx/2 + 600/zoom, sy, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.premium, "center", "top")
    ...
    elseif tab == "description" then
        dxDrawText("ZAPISANE OPISY", guiInfo.bg.x + 300/zoom, guiInfo.bg.y + 150/zoom, guiInfo.bg.x + guiInfo.bg.w - 300/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)

        for i = 1, 5 do
            local desc = self.descList[i]
            if desc then
                dxDrawRectangle(guiInfo.bg.x + 300/zoom, guiInfo.bg.y + 190/zoom + 75/zoom * (i-1), guiInfo.bg.w - 600/zoom, 70/zoom, tocolor(23,25,31, 255 * self.alpha))
        ...
        dxDrawText("TWORZENIE OPISU", guiInfo.bg.x + 300/zoom, guiInfo.bg.y + 620/zoom, guiInfo.bg.x + guiInfo.bg.w - 300/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
        dxDrawText("PODGLĄD", guiInfo.bg.x + 300/zoom, guiInfo.bg.y + 780/zoom, guiInfo.bg.x + guiInfo.bg.w - 300/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.premium, "center", "top", false, false, false, true)
    ...
    elseif tab == "vehicle" and not self.vehiclePreview then
        if #self.vehicleData < 1 then
            ...
            dxDrawText(string.format("Nie posiadasz żadnego pojazdu.\nMaksymalna liczba pojazdów jaką możesz\nposiadać na tym koncie wynosi %d.", self.playerData.maxVehicles), guiInfo.bg.x + (guiInfo.bg.w - 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 + 10/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
            ...
            dxDrawText(string.format("Znudzony posiadanymi pojazdami?\nMoże nowe auto, albo chociaż jakiś tuning?\nPosiadasz już %d/%d pojazdów.", self.playerData.vehicleCount, self.playerData.maxVehicles), guiInfo.bg.x + guiInfo.bg.w - 350/zoom, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 - 15/zoom, guiInfo.bg.x + guiInfo.bg.w - 222/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")

        end

    elseif tab == "vehicle" and self.vehiclePreview then
        dxDrawText("INFORMACJE O POJEŹDZIE", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText(string.format("ID: #2cb5e9%s", self.vehicleData[self.vehiclePreview].ID), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 30/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Własność: #2cb5e9%s", self.vehicleData[self.vehiclePreview].isRented and "Pojazd udostępniony" or "Twój pojazd"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 55/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("UID właściciela: #2cb5e9%d", self.vehicleData[self.vehiclePreview].ownedPlayer), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 80/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)

        dxDrawText("DANE TECHNICZNE", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 140/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText(string.format("Model: #2cb5e9%s", self.vehicleData[self.vehiclePreview].model), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 170/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Nazwa: #2cb5e9%s", self:getVehicleName(self.vehicleData[self.vehiclePreview].model)), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 195/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        ...
        dxDrawText("OSTATNI KIEROWCY", guiInfo.bg.x + guiInfo.bg.w/2 + 400/zoom, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
    ...
    elseif tab == "house" and not self.houseSelected then
        if #self.houseData < 1 then
            dxDrawImage(guiInfo.bg.x + (guiInfo.bg.w - 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2 - 10/zoom, 128/zoom, 128/zoom, "files/images/sad.png", 0, 0, 0, tocolor(180, 180, 180, 255 * self.alpha))
            dxDrawText(string.format("Nie posiadasz żadnej posiadłości.\nMaksymalnie możesz posiadać %d %s.", self.playerData.maxHouse, self.playerData.maxHouse == 1 and "posiadłość" or "posiadłości"), guiInfo.bg.x + (guiInfo.bg.w - 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 + 10/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
            ...
            for i = 1, guiInfo.logs.visible do
                if self.houseData[i + self.scroll] then
                    ...
                    dxDrawText(string.format("Lokalizacja: #2cb5e9%s", self.houseData[i + self.scroll].pos), guiInfo.bg.x + 164/zoom, guiInfo.bg.y + (i-1) * guiInfo.logs.h + 55/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
                end
            end
            dxDrawImage(guiInfo.bg.x + guiInfo.bg.w - 350/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2 - 35/zoom, 128/zoom, 128/zoom, "files/images/bored.png", 0, 0, 0, tocolor(180, 180, 180, 255 * self.alpha))
            dxDrawText(string.format("Widoki z mieszkania cię nudzą?\nZmiana lokum powinna być zbawieniem!\nPosiadasz już %d/%d %s.", self.playerData.houseCount, self.playerData.maxHouse, self.playerData.maxHouse == 1 and "posiadłość" or "posiadłości"), guiInfo.bg.x + guiInfo.bg.w - 350/zoom, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 - 15/zoom, guiInfo.bg.x + guiInfo.bg.w - 222/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
    ...
    elseif tab == "house" and self.houseSelected then
        ...
        dxDrawText("INFORMACJE O POSIADŁOŚCI", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 20/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText(string.format("ID: #2cb5e9%d", self.houseData[self.houseSelected].ID), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 50/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Własność: #2cb5e9%s", self.houseData[self.houseSelected].rent and "Posiadłość udostępniona" or "Twoja posiadłość" ), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 75/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("UID właściciela: #2cb5e9%s", self.houseData[self.houseSelected].owner), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 100/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)

        dxDrawText("DANE POSIADŁOŚCI", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 160/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText(string.format("Typ posiadłości: #2cb5e9%s", self.houseData[self.houseSelected].size), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 190/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Cena za dzień: #2cb5e9$%s", self.houseData[self.houseSelected].price), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 215/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Lokalizacja: #2cb5e9%s", self.houseData[self.houseSelected].pos), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 240/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Wielkość posiadłości: #2cb5e9%dm×%dm", self.houseData[self.houseSelected].interiorSize, self.houseData[self.houseSelected].interiorSize), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 265/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Data wygaśnięcia czynszu: #2cb5e9%s", self.houseData[self.houseSelected].date), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 290/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
    ...
    elseif tab == "logs" and not self.logSelected then
        for i = 1, guiInfo.logs.visible do
            if self.logs[i + self.scroll] then
                ...
                dxDrawText(string.format("#2cb5e9%s", self.logs[i + self.scroll].text), guiInfo.bg.x + 164/zoom, guiInfo.bg.y + (i-1) * guiInfo.logs.h + 55/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
            end
        end
        ...
        dxDrawText("Wszystko jest w porządku?\nTo znak, że twoje konto jest bezpieczne.", guiInfo.bg.x + guiInfo.bg.w - 350/zoom, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 - 15/zoom, guiInfo.bg.x + guiInfo.bg.w - 222/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
    ...
    elseif tab == "logs" and self.logSelected then
        ...
        dxDrawText(string.format("Tekst: #2cb5e9%s", self.logs[self.logSelected].text), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 125/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Data: #2cb5e9%s", self.logs[self.logSelected].date), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 150/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Serial: #2cb5e9%s", self.revealData and (self.logs[self.logSelected].serial and self.logs[self.logSelected].serial or "Nie połączony") or "Ukryte"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 175/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Adres IP: #2cb5e9%s", self.revealData and (self.logs[self.logSelected].ip and self.logs[self.logSelected].ip or "Nie połączony") or "Ukryte"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 200/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
    ...
    elseif tab == "penalties" and not self.penaltySelected then
        if #self.penalties < 1 then
            ...
            dxDrawText("Nie posiadasz żadnych kar.\nJesteśmy z Ciebie bardzo dumni!", guiInfo.bg.x + (guiInfo.bg.w - 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 + 10/zoom, guiInfo.bg.x + (guiInfo.bg.w + 128/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
            ...
            for i = 1, guiInfo.logs.visible do
                if self.penalties[i + self.scroll] then
                    ...
                    dxDrawText(string.format("#2cb5e9%s", self.penalties[i + self.scroll].reason), guiInfo.bg.x + 164/zoom, guiInfo.bg.y + (i-1) * guiInfo.logs.h + 55/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
                end
            end
            ...
            dxDrawText("Uczyniłeś coś nieodpowiedniego?\nJest nam przykro z powodu Twojego zachowania.", guiInfo.bg.x + guiInfo.bg.w - 350/zoom, guiInfo.bg.y + (guiInfo.bg.h + 128/zoom)/2 - 15/zoom, guiInfo.bg.x + guiInfo.bg.w - 222/zoom, guiInfo.bg.y + (guiInfo.bg.h - 128/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top")
        end
    ...
    elseif tab == "penalties" and self.penaltySelected then
        ...
        dxDrawText(string.format("Powód: #2cb5e9%s", self.penalties[self.penaltySelected].reason), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 125/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Data: #2cb5e9%s %s", self.penalties[self.penaltySelected].time, self.penalties[self.penaltySelected].timeEnd and "- "..self.penalties[self.penaltySelected].timeEnd or ""), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 150/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Serial: #2cb5e9%s", self.revealData and (self.penalties[self.penaltySelected].serial and self.penalties[self.penaltySelected].serial or "Nie połączony") or "Ukryte"), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 175/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText(string.format("Administrator: #2cb5e9%s", self.penalties[self.penaltySelected].admin), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 400/zoom)/2 + 200/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
    ...
    elseif tab == "cards" and self.playingCards then
        ...
        if self.canPickCard then
            dxDrawText("Wybierz kartę i zgarnij nagrodę.", guiInfo.bg.x, guiInfo.bg.y + (guiInfo.bg.h - guiInfo.card.h)/2 + guiInfo.card.h + 30/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y +  guiInfo.bg.h, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
        end
    ...
    elseif tab == "cards" and not self.playingCards then
        ...
        dxDrawText("ZASADY GRY", guiInfo.bg.x + (guiInfo.bg.w - guiInfo.card.w)/2 - guiInfo.card.w + 450/zoom, guiInfo.bg.y + (guiInfo.bg.h - guiInfo.card.h)/2 - 30/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        dxDrawText("W grę można zagrać raz na 24h.\nPo rozpoczęciu ujrzysz trzy losowe karty z \ndostępnymi nagrodami. Po chwili karty \nzostaną odwrócone i przetasowane. \nNastępnie Twoim zadaniem jest odwrócenie \njednej z nich. Nagroda z odwróconej karty \ntrafi na Twoje konto.", guiInfo.bg.x + (guiInfo.bg.w - guiInfo.card.w)/2 - guiInfo.card.w + 450/zoom, guiInfo.bg.y + (guiInfo.bg.h - guiInfo.card.h)/2, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "top", false, false, false, true)
        dxDrawText("DOSTĘPNE NAGRODY", guiInfo.bg.x + (guiInfo.bg.w - guiInfo.card.w)/2 - guiInfo.card.w + 450/zoom, guiInfo.bg.y + (guiInfo.bg.h - guiInfo.card.h)/2 + 200/zoom, guiInfo.bg.x + guiInfo.bg.w, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        ...
        if self.playerData.cardTime then
            dxDrawText("Następne darmowe zagranie \notrzymasz za "..self:getTimeInSeconds(self.playerData.cardTime - (getTickCount() - self.openedTick)/1000), guiInfo.bg.x + (guiInfo.bg.w - guiInfo.card.w)/2 - guiInfo.card.w + 450/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2 - 5/zoom, guiInfo.bg.x + (guiInfo.bg.w - guiInfo.card.w)/2 - guiInfo.card.w + 700/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "top", false, false, false, true)
        end
    ...
    elseif tab == "reference" then
        ...
        if self.referenceProgress >= 0.5 then -- Center
            dxDrawRectangle(guiInfo.bg.x + guiInfo.bg.w/2 - 2/zoom, guiInfo.bg.y + guiInfo.bg.h/2 - 10/zoom, 4/zoom, 40/zoom, tocolor(44,181,233, 255 * self.alpha))
        ...
        if self.referenceProgress >= 1 then -- End
            dxDrawRectangle(guiInfo.bg.x + guiInfo.bg.w/2 + guiInfo.reference.w - 4/zoom, guiInfo.bg.y + guiInfo.bg.h/2 - 30/zoom, 4/zoom, 60/zoom, tocolor(44,181,233, 255 * self.alpha))
        ...
        dxDrawRectangle(guiInfo.bg.x + guiInfo.bg.w/2 - guiInfo.reference.w, guiInfo.bg.y + guiInfo.bg.h/2 - 30/zoom, 4/zoom, 60/zoom, tocolor(44,181,233, 255 * self.alpha)) -- Start
        dxDrawRectangle(guiInfo.bg.x + guiInfo.bg.w/2 - guiInfo.reference.w, guiInfo.bg.y + guiInfo.bg.h/2 - 10/zoom, 10/zoom, 20/zoom, tocolor(44,181,233, 255 * self.alpha))
        dxDrawRectangle(guiInfo.bg.x + guiInfo.bg.w/2 - guiInfo.reference.w, guiInfo.bg.y + guiInfo.bg.h/2 - 10/zoom, guiInfo.reference.w * 2 * self.referenceProgress, 20/zoom, tocolor(44,181,233, 255 * self.alpha))

        dxDrawText("JAK TO DZIAŁA", guiInfo.bg.x + guiInfo.bg.w/2 - guiInfo.reference.w + 260/zoom, guiInfo.bg.y + (guiInfo.bg.h - guiInfo.card.h)/2 - 40/zoom, guiInfo.bg.x + guiInfo.bg.w/2 - guiInfo.reference.w - 200/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "top", false, false, false, true)
        ...
        if self:isMouseInPosition(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 65/zoom, 250/zoom, 55/zoom) then
            dxDrawText(string.format("Twój kod: #d4af37%s", self.referenceCode), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + guiInfo.bg.h/2 + 70/zoom, guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top", false, false, false, true)
        else
            dxDrawText(string.format("Twój kod: #2cb5e9%s", self.referenceCode), guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + guiInfo.bg.h/2 + 70/zoom, guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "top", false, false, false, true)
        end

        dxDrawText("(Naciśnij aby skopiować)", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + guiInfo.bg.h/2 + 92/zoom, guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(140, 140, 140, 255 * self.alpha), 1/zoom, self.fonts.small, "center", "top", false, false, false, true)

        self:updateReferenceButtons()

        if not self.playerData.hasReferenceUsed then
            dxDrawText("SKORZYSTAJ Z KODU", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + guiInfo.bg.h/2 + 260/zoom, guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
        end
    ...
    elseif tab == "settings" then
        ...
        dxDrawText("ZMIANA HASŁA", guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 - 5/zoom, guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
        dxDrawText("ZMIANA E-MAILA", guiInfo.bg.x + 1150/zoom, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2, guiInfo.bg.x + 1400/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top", false, false, false, true)
    ...
    elseif tab == "points" and not self.pointsPreview then
        dxDrawImage(guiInfo.bg.x+(guiInfo.bg.w/2)-177/zoom, (guiInfo.bg.h/2)-242/zoom, 354/zoom, 484/zoom, "files/images/choosed_status.png")

        local filePath = string.format("files/images/%s.png", guiInfo.shopItems[self.pointsSelected].iconName or "")
        if fileExists(filePath) then
            dxDrawImage(guiInfo.bg.x+(guiInfo.bg.w/2)-64/zoom, (guiInfo.bg.h/2)-242/zoom, 128/zoom, 128/zoom, filePath)
        end

        dxDrawText(guiInfo.shopItems[self.pointsSelected].item, guiInfo.bg.x+(guiInfo.bg.w/2)-177/zoom, (guiInfo.bg.h/2)-242/zoom, guiInfo.bg.x+(guiInfo.bg.w/2)-177/zoom+354/zoom, (guiInfo.bg.h/2)-242/zoom+484/zoom)

        if guiInfo.shopItems[self.pointsSelected-1] then
            dxDrawImage(guiInfo.bg.x+(guiInfo.bg.w/2)-507/zoom, (guiInfo.bg.h/2)-192/zoom, 280/zoom, 386/zoom, "files/images/status.png") -- lewo
            dxDrawText(guiInfo.shopItems[self.pointsSelected-1].item, guiInfo.bg.x+(guiInfo.bg.w/2)-507/zoom, (guiInfo.bg.h/2)-192/zoom, guiInfo.bg.x+(guiInfo.bg.w/2)-507/zoom+280/zoom, (guiInfo.bg.h/2)-192/zoom+386/zoom)

            filePath = string.format("files/images/%s.png", guiInfo.shopItems[self.pointsSelected-1].iconName or "")
            if fileExists(filePath) then
                dxDrawImage(guiInfo.bg.x+(guiInfo.bg.w/2)-416/zoom, (guiInfo.bg.h/2)-192/zoom, 96/zoom, 96/zoom, filePath)
            end

        end
        if guiInfo.shopItems[self.pointsSelected+1] then
            dxDrawImage(guiInfo.bg.x+(guiInfo.bg.w/2)+227/zoom, (guiInfo.bg.h/2)-192/zoom, 280/zoom, 386/zoom, "files/images/status.png") -- prawo

            filePath = string.format("files/images/%s.png", guiInfo.shopItems[self.pointsSelected+1].iconName or "")
            if fileExists(filePath) then
                dxDrawImage(guiInfo.bg.x+(guiInfo.bg.w/2)+319/zoom, (guiInfo.bg.h/2)-192/zoom, 96/zoom, 96/zoom, filePath)
            end

            dxDrawText(guiInfo.shopItems[self.pointsSelected+1].item, guiInfo.bg.x+(guiInfo.bg.w/2)+227/zoom, (guiInfo.bg.h/2)-192/zoom, guiInfo.bg.x+(guiInfo.bg.w/2)+227/zoom+280/zoom, (guiInfo.bg.h/2)-192/zoom+386/zoom)
        end
    end
end
...
function Dashboard:renderCategories()
    for i, v in pairs(guiInfo.categories) do
        if i == self.tab then
            dxDrawRectangle(guiInfo.category.x, guiInfo.category.y + guiInfo.account.h + (i-1) * 50/zoom, guiInfo.category.w, 50/zoom, tocolor(34,36,43, 255 * self.alpha))

            if fileExists(string.format("files/images/%s.png", v[2])) then
                dxDrawImage(guiInfo.category.x + 10/zoom, guiInfo.category.y + guiInfo.account.h + 9/zoom + (i-1) * 50/zoom, 32/zoom, 32/zoom, string.format("files/images/%s.png", v[2]), 0, 0, 0, tocolor(44,181,233, 255 * self.alpha))
            end
            dxDrawText(v[1], guiInfo.category.x + 52/zoom, guiInfo.category.y + guiInfo.account.h + (i-1) * 50/zoom, guiInfo.category.x + guiInfo.category.w, guiInfo.category.y + guiInfo.account.h + 50/zoom + (i-1) * 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.category, "left", "center")
    ...
    for i, v in pairs(guiInfo.card.cards) do
        if self:isMouseInPosition(v.x + v.w/2, guiInfo.bg.y + (guiInfo.bg.h - guiInfo.card.h)/2 - 20/zoom, guiInfo.card.w - v.w, guiInfo.card.h) and self.canPickCard then
            ...
            if self.cardPrices[self.selectedCard].type == "repeat" then
                ...
                exports.TR_dx:setButtonText(self.buttons.playCards, string.format("Zagraj (%d)", self.playerData.cardPlays))
        ...
        elseif self:isMouseInPosition(guiInfo.bg.x, guiInfo.bg.y, guiInfo.bg.w, guiInfo.bg.h) then
            ...
            elseif tab == "reference" then
                if self:isMouseInPosition(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 65/zoom, 250/zoom, 55/zoom) then
                    ...
                    exports.TR_noti:create("Kod został skopiowany do schowka.", "success")
                end
                ...
                for i = 1, 5 do
                    ...
                    if desc then
                        if self:isMouseInPosition(guiInfo.bg.x + guiInfo.bg.w - 300/zoom - 35/zoom, guiInfo.bg.y + 202/zoom + 75/zoom * (i-1), 16/zoom, 16/zoom) then
                            ...
                            if blockColor and string.find(desc, "#%x%x%x%x%x%x") then exports.TR_noti:create("Nie możesz wybrać tego opisu, ponieważ posiada on kolory a ty nie posiadasz konta premium.", "error") return end
                            for i, v in pairs(guiInfo.suspectedWords) do
                                if string.find(desc, v) then
                                    exports.TR_noti:create("Opis zawiera nieodpowiednie słowa.", 'error')
                                    return
                                end
                            end
                            ...
                            exports.TR_noti:create("Opis został pomyślnie ustawiony.", "success")
                        ...
                        elseif self:isMouseInPosition(guiInfo.bg.x + guiInfo.bg.w - 300/zoom - 35/zoom, guiInfo.bg.y + 232/zoom + 75/zoom * (i-1), 16/zoom, 16/zoom) then
                            ...
                            exports.TR_noti:create("Opis został pomyślnie usunięty.", "success")
                            break
                        end
                    end
                end
            ...
            elseif tab == "points" then
                if not self.pointsPreview then
                    if self.pointsSelected then 
                        if guiInfo.shopItems[self.pointsSelected-1] and self:isMouseInPosition(guiInfo.bg.x+(guiInfo.bg.w/2)-507/zoom, (guiInfo.bg.h/2)-192/zoom, 280/zoom, 386/zoom) then
                            self.pointsSelected = self.pointsSelected - 1
                        elseif  guiInfo.shopItems[self.pointsSelected+1] and self:isMouseInPosition(guiInfo.bg.x+(guiInfo.bg.w/2)+227/zoom, (guiInfo.bg.h/2)-192/zoom, 280/zoom, 386/zoom) then
                            self.pointsSelected = self.pointsSelected + 1
                        end
                    end
                end
...
function Dashboard:buttonClick(...)
    ...
    if arg[1] == self.buttons.revealHidden then
        ...
        exports.TR_dx:setButtonText(self.buttons.revealHidden, self.revealData and "Ukryj wrażliwe dane" or "Pokaż wrażliwe dane")

    elseif arg[1] == self.buttons.showLogs then
        self.revealData = not self.revealData
        exports.TR_dx:setButtonText(self.buttons.showLogs, self.revealData and "Ukryj wrażliwe dane" or "Pokaż wrażliwe dane")

    elseif arg[1] == self.buttons.showPenalty then
        self.revealData = not self.revealData
        exports.TR_dx:setButtonText(self.buttons.showPenalty, self.revealData and "Ukryj wrażliwe dane" or "Pokaż wrażliwe dane")
    ...
    elseif arg[1] == self.buttons.localizeVehicle then
        ...
        if not veh or self.vehicleData[self.vehiclePreview].parking == "Parking" then
            exports.TR_noti:create("Nie możesz namierzyć pojazdu, który znajduje się w przechowywalni.", "error")
            return
        end

        if getElementInterior(localPlayer) ~= 0 or getElementDimension(localPlayer) ~= 0 then
            exports.TR_noti:create("Nie możesz namierzyć pojazdu będąc w budynku.", "error")
            return
        end
    ...
    elseif arg[1] == self.buttons.playCards then
        if self.playerData.cardPlays < 1 then
            exports.TR_noti:create("Nie możesz zagrać, ponieważ nie posiadasz żadnych gier.", "error")
            return
        end
        self:playCardGame()
    ...
    elseif arg[1] == self.buttons.useReferences then
        ...
        if string.len(code) < 3 then exports.TR_noti:create("Kod jest nieprawidłowy.", "error") return end
        if self.referenceCode == code then exports.TR_noti:create("Nie możesz skorzystać z własnego kodu.", "error") return end
    ...
    elseif arg[1] == self.buttons.descSave then
        ...
        if #self.descList >= limit then exports.TR_noti:create("Osiągnąłeś maksymalną liczbę opisów.", "error") return end
        if blockColor and string.find(text, "#%x%x%x%x%x%x") then exports.TR_noti:create("Koloru może używać tylko gracz premium.", "error") return end
        ...
        self:saveCharacterDescriptions()
        exports.TR_noti:create("Opis został pomyślnie dodany.", "success")


    elseif arg[1] == self.buttons.addFriend then
        local text = guiGetText(self.edits.addFriend)
        if string.len(text) < 3 then exports.TR_noti:create("Wpisany nick musi składać się z przynajmniej 3 znaków.", "error") return end
        if string.lower(getPlayerName(localPlayer)) == string.lower(text) then exports.TR_noti:create("Nie możesz dodać do znajomych samego siebie.", "error") return end
...
function Dashboard:getRandomCard()
    ...
    if rand == 100 then
        ...
        card.amountText = string.format("%d %s", card.amount, card.amount == 1 and " dzień" or " dni")
    ...
    elseif rand < 95 and rand >= 80 then
        ...
        card.amountText = string.format("%s min", card.amount)
    ...
    elseif rand < 65 and rand >= 55 then
        ...
        card.amountText = "Zagraj jeszcze raz"
...
function Dashboard:onFriendLogin(plr, username)
    ...
    for i, v in pairs(self.friendsData) do
        if v.username == username then
            ...
            exports.TR_noti:create(string.format("Twój znajomy %s właśnie się zalogował! Jeśli chcesz do niego napisać, użyj komendy:\n/sms %d (treść)", username, id), "info", 10)
        end
    end
end
    ...
    for i, v in pairs(data) do
        ...
        if isElement(plr) then
            if getElementData(plr, "characterUID") then
                lastOnlineDate = "Teraz, ID: "..getElementData(plr, "ID")
            end
        end
...
function Dashboard:setData(data, data2, referencedPlayers)
    ...
    if tab == "player" then
        ...
        if data.licence then
            ...
            self.playerData.licence = string.len(licences) > 1 and string.sub(licences, 0, string.len(licences) - 2) or "Brak"
            self.playerData.vehicleLicence = string.sub(vehicleLicence, 0, string.len(vehicleLicence) - 2)
        else
            self.playerData.licence = "Brak"
            self.playerData.vehicleLicence = "Brak"
        end
        ...
        if data then
            for i, v in pairs(data) do
                ...
                if not v.parking then
                    ...
                    self.vehicleData[#self.vehicleData].parking = veh and getZoneName(Vector3(getElementPosition(veh))) or "Nieznana lokalizacja"

                elseif tonumber(v.parking) == 100 then
                    self.vehicleData[#self.vehicleData].parking = "Parking Policyjny"
                else
                    self.vehicleData[#self.vehicleData].parking = "Parking"
                end
...
function Dashboard:updateReferenceButtons()
    ...
    elseif self.referenceProgress == progress then
        ...
        elseif self.referenceProgress < 1 then
            self:drawBackground(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 130/zoom, 250/zoom, 40/zoom, tocolor(23,25,31, 255 * self.alpha), 5)
            dxDrawText("Odbierz nagrodę", guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 130/zoom, guiInfo.bg.x + (guiInfo.bg.w + 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 170/zoom, tocolor(140, 140, 140, 255 * self.alpha), 1/zoom, self.fonts.category, "center", "center")
        end
...
function Dashboard:switchSelect(...)
    exports.TR_dx:setResponseEnabled(true, "Aplikowanie zmian")
    ...
    elseif arg[1] == self.switches.snow then
        ...
        setElementData(localPlayer, 'snow', state)
...
function Dashboard:loadVehicleDashboardInspect(...)
    ...
    if #arg[1] > 0 then
        ...
        table.insert(self.drivers, "Brak ostatnich kierowców")
    end
...
function Dashboard:createButtons()
    self.buttons = {}
    self.buttons.revealHidden = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 + 50/zoom, 250/zoom, 40/zoom, "Pokaż wrażliwe dane")
    self.buttons.localizeVehicle = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 - 20/zoom, 250/zoom, 40/zoom, "Namierz pojazd")
    self.buttons.backPreview = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 + 30/zoom, 250/zoom, 40/zoom, "Wróć do listy")
    self.buttons.showLogs = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 - 140/zoom, 250/zoom, 40/zoom, "Pokaż wrażliwe dane")
    self.buttons.backLogs = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 - 90/zoom, 250/zoom, 40/zoom, "Wróć do listy")
    self.buttons.showPenalty = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 - 140/zoom, 250/zoom, 40/zoom, "Pokaż wrażliwe dane")
    self.buttons.backPenalty = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 - 90/zoom, 250/zoom, 40/zoom, "Wróć do listy")
    self.buttons.backHouse = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w/2, guiInfo.bg.y + (guiInfo.bg.h + 400/zoom)/2 - 50/zoom, 250/zoom, 40/zoom, "Wróć do listy")
    self.buttons.playCards = exports.TR_dx:createButton(guiInfo.bg.x + (guiInfo.bg.w - guiInfo.card.w)/2 - guiInfo.card.w + 450/zoom, guiInfo.bg.y + (guiInfo.bg.h + guiInfo.card.h)/2 - 50/zoom, 250/zoom, 40/zoom, string.format("Zagraj (%d)", self.playerData.cardPlays))
    self.buttons.changePassword = exports.TR_dx:createButton(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 195/zoom, 250/zoom, 40/zoom, "Zmień")
    self.buttons.changeMail = exports.TR_dx:createButton(guiInfo.bg.x + 1150/zoom, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 190/zoom, 250/zoom, 40/zoom, "Zmień")
    self.buttons.getReferences = exports.TR_dx:createButton(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 130/zoom, 250/zoom, 40/zoom, "Odbierz nagrodę")
    self.buttons.useReferences = exports.TR_dx:createButton(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 350/zoom, 250/zoom, 40/zoom, "Skorzystaj z kodu")

    self.buttons.descSave = exports.TR_dx:createButton(guiInfo.bg.x + guiInfo.bg.w - guiInfo.bg.x - 250/zoom, guiInfo.bg.y + 715/zoom, 250/zoom, 40/zoom, "Zapisz opis", "green")
    self.buttons.descDefaultColor = exports.TR_dx:createButton(guiInfo.bg.x + 300/zoom, guiInfo.bg.y + 715/zoom, 250/zoom, 40/zoom, "Wstaw domyślny kolor")

    self.buttons.addFriend = exports.TR_dx:createButton(sx/2 + 130/zoom, sy/2 + 370/zoom, 250/zoom, 40/zoom, "Dodaj znajomego")
    exports.TR_dx:setButtonVisible(self.buttons, false)
...
function Dashboard:createEdits()
    self.edits = {}
    self.edits.currPassword = exports.TR_dx:createEdit(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 35/zoom, 250/zoom, 40/zoom, "Wpisz aktualne hasło", true)
    self.edits.newPassword = exports.TR_dx:createEdit(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 85/zoom, 250/zoom, 40/zoom, "Wpisz nowe hasło", true)
    self.edits.newPasswordRe = exports.TR_dx:createEdit(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 135/zoom, 250/zoom, 40/zoom, "Powtórz nowe hasło", true)

    self.edits.currEmail = exports.TR_dx:createEdit(guiInfo.bg.x + 1150/zoom, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 40/zoom, 250/zoom, 40/zoom, "Wpisz aktualny email")
    self.edits.newEmail = exports.TR_dx:createEdit(guiInfo.bg.x + 1150/zoom, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 90/zoom, 250/zoom, 40/zoom, "Wpisz nowy email")
    self.edits.newEmailRe = exports.TR_dx:createEdit(guiInfo.bg.x + 1150/zoom, guiInfo.bg.y + (guiInfo.bg.h - 300/zoom)/2 + 140/zoom, 250/zoom, 40/zoom, "Powtórz nowy email")

    self.edits.charDesc = exports.TR_dx:createEdit(guiInfo.bg.x + 300/zoom, guiInfo.bg.y + 660/zoom, guiInfo.bg.w - guiInfo.bg.x - 300/zoom, 45/zoom, "Wpisz nowy opis")

    self.edits.referenceCode = exports.TR_dx:createEdit(guiInfo.bg.x + (guiInfo.bg.w - 250/zoom)/2, guiInfo.bg.y + guiInfo.bg.h/2 + 300/zoom, 250/zoom, 40/zoom, "Wpisz kod polecający")

    self.edits.addFriend = exports.TR_dx:createEdit(sx/2 - 230/zoom, sy/2 + 370/zoom, 350/zoom, 40/zoom, "Wpisz nick gracza")
...
function Dashboard:changePassword()
    ...
    if string.len(currPas) < 1 or string.len(newPass) < 1 or string.len(newPassRe) < 1 then
        exports.TR_noti:create("Żadne pole nie może być puste.", "error")
        return
    end
    if not string.checkLen(newPass, 3, 40) or not string.checkLen(newPassRe, 3, 40) then
        exports.TR_noti:create("Hasło powinno zawierać od 3 do 40 znaków.", "error")
        return
    end
    if newPass ~= newPassRe then
        exports.TR_noti:create("Hasła nie pasują do siebie.", "error")
        return
    end
    if calculateStrength(newPass) <= 50 then
        exports.TR_noti:create("Nowe hasło nie jest wystarczająco bezpieczne.", "error")
        return
    end
    if currPas == newPass then
        exports.TR_noti:create("Nowe hasło nie może być takie jak aktualne.", "error")
        return
    end
...
function Dashboard:changeMail()
    ...
    if not isValidMail(newMail) then
        exports.TR_noti:create("Adres email jest niepoprawny.", "error")
        return
    end
    if newMail ~= newMailRe then
        exports.TR_noti:create("Adresy email nie pasują do siebie.", "error")
        return
    end
    if currMail == newMail then
        exports.TR_noti:create("Nowy adres email nie może być taki sam jak aktualny.", "error")
        return
    end
...
function Dashboard:loadSettings()
    ...
    for i, _ in pairs(guiInfo.defaultSettings) do
        ...
        elseif i == "snow" and state < 1 then
            ...
            setElementData(localPlayer, 'snow', true)
...
function updateCardPlays(amount)
    if amount == 1 then
        exports.TR_noti:create("Pomyślnie zakupiłeś dodatkowe zagranie w karty.", "success", 5)
    else
        exports.TR_noti:create("Pomyślnie zakupiłeś dodatkowe 10 zagrań w karty.", "success", 5)
    end
