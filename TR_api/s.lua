...
function givePremium(username, days, premium, blockMsg)
    ...
    if premium == 1 then
        ...
        if isElement(plr) then
            exports.TR_noti:create(plr, "Przedmiot został pomyślnie zakupiony. Powinien się on pojawić po chwili w ekwipunku.", "success", 15)
        end
        return true
    ...
    elseif premium == 2 then
        ...
        local plr = getPlayerFromName(username)
        if isElement(plr) then
            exports.TR_noti:create(plr, "Przedmiot został pomyślnie zakupiony. Powinien się on pojawić do chwili w ekwipunku.", "success")
        end
        return true
    ...
    elseif premium == 5 then
        ...
        local plr = getPlayerFromName(username)
        if isElement(plr) then
            exports.TR_noti:create(plr, "Pomyślnie zakupiłeś dodatkowe miejsce na pojazd.", "success")
        end
        return true
    ...
    elseif premium == 6 then
        ...
        local plr = getPlayerFromName(username)
        if isElement(plr) then
            exports.TR_noti:create(plr, "Pomyślnie zakupiłeś dodatkowe miejsce na posiadłość.", "success")
        end
        return true
    end
end
