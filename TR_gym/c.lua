function playerPayForGymTicket(state)
    ...
    if state then
        ...
        exports.TR_noti:create("Karnet na siłownię został pomyślnie zakupiony.", "success")
...
function canUseGym(element)
    if not isGymEquipmentFree(element) then exports.TR_noti:create("Ktoś już ćwiczy na tym urządzeniu.", "error") return end
    if not hasPlayerPermission() then exports.TR_noti:create("Nie posiadasz uprawnień aby móc ćwiczyć na tym sprzęcie.", "error") return end
