...
function AdminLogos:constructor(...)
    ...
    self.buttons = {}
    self.buttons.exit = exports.TR_dx:createButton(guiInfo.x + (guiInfo.w - 250/zoom)/2, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Zapisz i przeładuj")
...
function AdminLogos:destroy()
    ...
    if reloadResource then
        ...
        exports.TR_noti:create("Wszystkie zmiany zostały pomyślnie zapisane.", "success")
    end
end

function AdminLogos:render()
    self:drawBackground(guiInfo.x, guiInfo.y, guiInfo.w, guiInfo.h, tocolor(16,18,23, 255), 15)
    dxDrawText("Zgłoszenia zmian logo organizacji", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255), 1/zoom, self.fonts.main, "center", "center")

    dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom, guiInfo.w, 2/zoom, tocolor(34,36,43, 255))
    dxDrawText("Nazwa organizacji", guiInfo.x + 20/zoom, guiInfo.y + 60/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 80/zoom, tocolor(170, 170, 170, 255), 1/zoom, self.fonts.text, "left", "top")
    dxDrawText("Stare logo", guiInfo.x + 250/zoom, guiInfo.y + 60/zoom, guiInfo.x + 350/zoom, guiInfo.y + 80/zoom, tocolor(170, 170, 170, 255), 1/zoom, self.fonts.text, "center", "top")
    dxDrawText("Nowe logo", guiInfo.x + 450/zoom, guiInfo.y + 60/zoom, guiInfo.x + 550/zoom, guiInfo.y + 80/zoom, tocolor(170, 170, 170, 255), 1/zoom, self.fonts.text, "center", "top")
    dxDrawText("Akcja", guiInfo.x + 600/zoom, guiInfo.y + 60/zoom, guiInfo.x + 800/zoom, guiInfo.y + 80/zoom, tocolor(170, 170, 170, 255), 1/zoom, self.fonts.text, "center", "top")
            ...
            if v.newImage then
                ...
                if v.invalidFormat then
                    ...
                    if self:isMouseInPosition(guiInfo.x + 655/zoom, guiInfo.y + 124/zoom + (i-1) * 110/zoom, 32/zoom, 32/zoom) then
                        ...
                        dxDrawRectangle(cx, cy, 200/zoom, 40/zoom, tocolor(34,36,43, 255), true)
                        dxDrawText("Te logo nie spełnia wszystkich warunków (zła wielkość obrazu).", cx, cy, cx + 200/zoom, cy + 40/zoom, tocolor(255, 255, 255, 200), 1/zoom, self.fonts.loading, "center", "center", false, true, true)
                    end
                else
                    if self:isMouseInPosition(guiInfo.x + 655/zoom, guiInfo.y + 124/zoom + (i-1) * 110/zoom, 32/zoom, 32/zoom) then
                ...
                dxDrawText("Trwa wczytywanie...", guiInfo.x + 450/zoom, guiInfo.y + 153/zoom + (i-1) * 110/zoom, guiInfo.x + 550/zoom, guiInfo.y + 90/zoom + i * 110/zoom, tocolor(120, 120, 120, 255), 1/zoom, self.fonts.loading, "center", "top")
            end
        end
    end

    if #self.orgRequests > 3 then
        dxDrawText(string.format("I %d zgłoszenie więcej...", #self.orgRequests - 3), sx/2, guiInfo.y, sx/2, guiInfo.y + guiInfo.h - 55/zoom, tocolor(120, 120, 120, 255), 1/zoom, self.fonts.loading, "center", "bottom")
    end
    ...
    for i, v in pairs(self.orgRequests) do
        if v.name == arg[1] then
            if arg[2] then
                ...
                v.invalidFormat = width ~= 500 and true or height ~= 500 and true or false
            end
            break
        end
    end
...
function AdminLogos:response(...)
    if arg[1] == "declined" then
        ...
        exports.TR_noti:create(string.format("Nowe logo organizacji %s zostało odrzucone.", org.name), "error")
    ...
    elseif arg[1] == "saved" then
        ...
        exports.TR_noti:create(string.format("Nowe logo organizacji %s zostało załadowane.", org.name), "success")
        table.remove(self.orgRequests, self.selectedIndex)
    ...
    elseif arg[1] == "saveError" then
        ...
        exports.TR_noti:create(string.format("Wystąpił błąd podczas zapisywania nowego loga organizacji %s.", org.name), "error")
        table.remove(self.orgRequests, self.selectedIndex)
        self.loadedNewLogo = true
    end
