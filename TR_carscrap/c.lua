...
function Weights:updateWeight(sphere, target)
    ...
    dxDrawText("READINGS:", 7, 10, guiInfo.w, guiInfo.h, tocolor(0, 0, 0, 255), 1, self.fonts.weight, "center", "top")
    dxDrawText(string.format("TOTAL WEIGHT: %.3fT", totalWeight/1000), 7, 105, guiInfo.w, guiInfo.h, tocolor(255, 0, 0, 255), 1, self.fonts.weight, "center", "top")

    dxDrawText("WAGMEISTER", 7, 0, guiInfo.w, guiInfo.h - 5/zoom, tocolor(0, 0, 0, 255), 1.2, "default-bold", "center", "bottom")
    dxSetRenderTarget()
end
...
function checkVehiclesOnWeight(pedName)
    ...
    if #vehicles < 1 then exports.TR_chat:showCustomMessage(pedName, "A to trzeba było tak od razu! Pakuj się tym swoim ustrojstwem na wagę! Zaraz ogarniemy!", "files/images/npc.png") return end
