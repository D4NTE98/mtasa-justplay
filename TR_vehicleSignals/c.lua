...
local settings = {
    ...
    hornSounds = {
        ...
        [502] = "files/sounds/fire/horn.wav",
        [573] = "files/sounds/fire/horn.wav",
        [411] = "files/sounds/police/horn.wav",
    },
...
function VehicleSignals:renderGui()
    self:drawBackground(settings.gui.x, settings.gui.y, settings.gui.w, settings.gui.h, tocolor(23,25,31, 255), 15)

    local siren, sound = self:getSirenVehicle()
    if not siren then
        dxDrawImage(settings.gui.x + 22/zoom, settings.gui.y + (settings.gui.h - 64/zoom)/2 - 2/zoom, 64/zoom, 64/zoom, "files/images/siren.png", 0, 0, 0, tocolor(44,181,233, 255))
    else
        ...
        dxDrawImage(settings.gui.x + 22/zoom, settings.gui.y + (settings.gui.h - 64/zoom)/2 - 2/zoom, 64/zoom, 64/zoom, "files/images/siren.png", 0, 0, 0, tocolor(44,181,233, 255))
        dxDrawImage(settings.gui.x + 22/zoom - self.sirenLights/2, settings.gui.y + (settings.gui.h - 64/zoom)/2 - 2/zoom - self.sirenLights/2, 64/zoom + self.sirenLights, 64/zoom + self.sirenLights, "files/images/siren_lights.png", 0, 0, 0, tocolor(44,181,233, 255))
    end

    dxDrawText(sound and "Dźwięk włączony (x)" or "Dźwięk wyłączony (x)", settings.gui.x + settings.gui.w - 5/zoom, settings.gui.y + 9/zoom, settings.gui.x + settings.gui.w - 5/zoom, settings.gui.y + settings.gui.h - 5/zoom, tocolor(140, 140, 140, 255), 1/zoom, self.fonts.small, "right", "bottom")
...
function VehicleSignals:drawSirenText(text, i, siren)
    if not siren and i == 0 then
        dxDrawText(text, settings.gui.x + 115/zoom, settings.gui.y + 9/zoom + i * 20/zoom, settings.gui.x + 140/zoom, settings.gui.y + 20/zoom, tocolor(44,181,233, 255), 1/zoom, self.fonts.siren, "left", "top")
    elseif i == siren then
        dxDrawText(text, settings.gui.x + 115/zoom, settings.gui.y + 9/zoom + i * 20/zoom, settings.gui.x + 140/zoom, settings.gui.y + 20/zoom, tocolor(44,181,233, 255), 1/zoom, self.fonts.siren, "left", "top")
    else
