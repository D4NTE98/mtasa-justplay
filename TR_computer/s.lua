...
function payComputerPlayers(fractionID)
    ...
    if canPayPlayers and canPayPlayers[1] then
        ...
        triggerClientEvent(client, "responseComputer", resourceRoot, false, "Nie możesz jeszcze wypłacić pieniędzy pracownikom. Możesz to robić co 24h.", "error")
    end
end
...
function payComputerUpgrade(fractionID, upgrade)
    ...
    if upgradeCount and upgradeCount[1] then
        ...
        local upgradeText = ""
        if upgrade == 1 then toPay = count * 15000; upgradeText = "Ulepszył ilość pracowników." end
        if upgrade == 2 then toPay = count * 20000; upgradeText = "Ulepszył ilość pojazdów." end
        if upgrade == 3 then toPay = (count * 50000) + 50000; upgradeText = "Ulepszył wysokość zarobku." end

        if toPay > money then
            triggerClientEvent(client, "responseComputer", resourceRoot, false, "Organizacja nie posiada tyle pieniędzy aby zakupić to ulepszenie.", "error")
            return
        end
...
function addMoneyToOrganization(fractionID, amount)
    local characterUID = getElementData(client, "characterUID")

    if exports.TR_core:takeMoneyFromPlayer(client, amount) then
        exports.TR_mysql:querry("UPDATE tr_organizationsPlayers SET allPaid = allPaid + ? WHERE playerUID = ? LIMIT 1", amount, characterUID)
        exports.TR_mysql:querry("UPDATE tr_organizations SET money = money + ? WHERE ID = ? LIMIT 1", amount, fractionID)
        exports.TR_mysql:querry("INSERT INTO tr_computerLogs (text, name, owner, type) VALUES (?, ?, ?, ?)", string.format("Wpłacił $%.2f na rzecz organizacji.", amount), getPlayerName(client), fractionID, "org")

        local _, update = exports.TR_mysql:querry("UPDATE tr_organizationsEarnings SET totalEarn = totalEarn + ? WHERE orgID = ? AND day = DATE(NOW()) LIMIT 1", string.format("%.2f", amount), fractionID)
        if update < 1 then
            exports.TR_mysql:querry("INSERT INTO `tr_organizationsEarnings`(`orgID`, `totalEarn`, `day`) VALUES (?, ?, NOW())", fractionID, string.format("%.2f", amount))
        end

        triggerClientEvent(client, "responseComputer", resourceRoot, true)
    else
        triggerClientEvent(client, "responseComputer", resourceRoot, false, "Nie udało się wpłacić pieniędzy na organizację.", "error")
    end
end
addEvent("addMoneyToOrganization", true)
addEventHandler("addMoneyToOrganization", resourceRoot, addMoneyToOrganization)

function addMoneyToPlayerByOrganization(fractionID, amount, playerName)
    local hasBank = exports.TR_mysql:querry("SELECT bankcode FROM tr_accounts WHERE username = ? LIMIT 1", playerName)
    if hasBank and hasBank[1] then
        if hasBank[1].bankcode then
            exports.TR_mysql:querry("UPDATE tr_accounts SET bankMoney = bankMoney + ? WHERE username = ? LIMIT 1", amount, playerName)
            exports.TR_mysql:querry("UPDATE tr_organizations SET money = money - ? WHERE ID = ? LIMIT 1", amount, fractionID)
            exports.TR_mysql:querry("INSERT INTO tr_computerLogs (text, name, owner, type) VALUES (?, ?, ?, ?)", string.format("Wypłacił dodatkowe wynagrodzenie graczowi %s w wysokości $%.2f.", playerName, amount), getPlayerName(client), fractionID, "org")
    ...
    triggerClientEvent(client, "responseComputer", resourceRoot, false, "Ten pracownik nie posiada konta w banku.", "error")
end
addEvent("addMoneyToPlayerByOrganization", true)
addEventHandler("addMoneyToPlayerByOrganization", resourceRoot, addMoneyToPlayerByOrganization)

function changeComputerLogo(fractionID, newLogo)
    local hasLogoApproval = exports.TR_mysql:querry("SELECT ID FROM tr_organizations WHERE ID = ? AND logoRequest IS NOT NULL LIMIT 1", fractionID)
    if hasLogoApproval and hasLogoApproval[1] then
        triggerClientEvent(client, "responseComputer", resourceRoot, false, "Twoja organizacja ma niezaakceptowaną poprzednią próbę zmiany logotypu.", "error")
        return
    end

    exports.TR_mysql:querry("UPDATE tr_organizations SET logoRequest = ? WHERE ID = ? LIMIT 1", newLogo, fractionID)
    triggerClientEvent(client, "responseComputer", resourceRoot, true)
end
addEvent("changeComputerLogo", true)
addEventHandler("changeComputerLogo", resourceRoot, changeComputerLogo)



function loadGangDarkwebData(type)
    local data = exports.TR_mysql:querry("SELECT text, location, hour FROM tr_darkweb WHERE type = ? AND DATE(added) = CURDATE()", type)
    triggerClientEvent(client, "responseComputerData", resourceRoot, "darkweb", data)
end
addEvent("loadGangDarkwebData", true)
addEventHandler("loadGangDarkwebData", resourceRoot, loadGangDarkwebData)

function addGangDarkwebData(type, location, text, hour)
    exports.TR_mysql:querry("INSERT INTO tr_darkweb (type, text, location, hour, added) VALUES (?, ?, ?, ?, CURDATE())", type, text, location, hour)
    local data = exports.TR_mysql:querry("SELECT text, location, hour FROM tr_darkweb WHERE type = ? AND DATE(added) = CURDATE()", type)
    triggerClientEvent(client, "responseComputerData", resourceRoot, "darkweb", data)
end
addEvent("addGangDarkwebData", true)
addEventHandler("addGangDarkwebData", resourceRoot, addGangDarkwebData)