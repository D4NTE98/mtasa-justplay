...
function startDynamicSky()
	...
	shaderTable.skyboxTropos = dxCreateShader ( "files/shaders/shader_dynamicSky2tropos.fx", 2, 0, false, "object" )
	if dynamicSkySettings.stratosBottomStars then shaderTable.skyboxStratos = dxCreateShader ( "files/shaders/shader_dynamicSky2up.fx", 2, 0, false, "object" )
	    else shaderTable.skyboxStratos = dxCreateShader ( "files/shaders/shader_dynamicSky2stratos.fx", 2, 0, false, "object" ) end
	if dynamicSkySettings.stratosBottomStars then shaderTable.skyboxBottom = dxCreateShader ( "files/shaders/shader_dynamicSky2down.fx", 3, 0, false, "object" )
		else shaderTable.skyboxBottom = dxCreateShader ( "files/shaders/shader_dynamicSky2bottom.fx", 3, 0, false, "object" ) end
	shaderTable.clear = dxCreateShader ( "files/shaders/shader_clear.fx", 3, 0, false, "world" )
	textureTable.cloud = dxCreateTexture ( "files/images/clouds.dds", "dxt5" )
	textureTable.normal = dxCreateTexture ( "files/images/clouds_normal.jpg", "dxt5" )
	textureTable.skybox = dxCreateTexture ( "files/images/skybox.dds", "dxt5" )
	moonPhase = getCurrentMoonPhase()
	textureTable.moon = dxCreateTexture ( "files/images/moon/"..toint( 20 - toint( moonPhase * 20 ) )..".png" )
	...
	modelTable.txd = engineLoadTXD( 'files/txd/skybox_model.txd' )
	engineImportTXD( modelTable.txd, dynamicSkySettings.modelID)
	modelTable.dff = engineLoadDFF( 'files/dff/skybox_model.dff', dynamicSkySettings.modelID )
...
function renderTime()
	if not dsEffectEnabled or not shaderTable.skyboxTropos or not shaderTable.skyboxBottom or not shaderTable.skyboxStratos or not shaderTable.clear then return end
