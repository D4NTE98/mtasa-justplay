...
function render()
    ...
    for index, v in pairs(shaderSettings.effectInfos) do
        local icon = v.icon
        if v.feature then
            dxDrawImage(guiInfo.x, guiInfo.y + i * guiInfo.size, guiInfo.size, guiInfo.size, string.format("files/images/icons/%s.png", icon or "marijuana"))
            drawTextShadowed("Permanentny", guiInfo.x + guiInfo.size + 10/zoom , guiInfo.y + i * guiInfo.size, guiInfo.x + guiInfo.size + 10/zoom +guiInfo.size, guiInfo.y + i * guiInfo.size + guiInfo.size, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.font, "left", "center")
            ...
            if (getTickCount() - v.tick)/300000 >= 1 then
                ...
                if icon == "beer" then
                    exports.TR_noti:create("Przez uzależnienie od alkoholu straciłeś trochę życia.", "info")
                elseif icon == "marijuana" then
                    exports.TR_noti:create("Przez uzależnienie od narkotyków straciłeś trochę życia.", "info")
                end
            end
        else
            dxDrawImage(guiInfo.x, guiInfo.y + i * guiInfo.size, guiInfo.size, guiInfo.size, string.format("files/images/icons/%s.png", icon or "marijuana"))
            drawTextShadowed(secondsToClock((v.timeToEnd - getTickCount())/1000), guiInfo.x + guiInfo.size + 10/zoom , guiInfo.y + i * guiInfo.size, guiInfo.x + guiInfo.size + 10/zoom +guiInfo.size, guiInfo.y + i * guiInfo.size + guiInfo.size, tocolor(255, 255, 255, 255), 1/zoom, guiInfo.font, "left", "center")
        ...
        if v.effectTime then
            if (getTickCount() - v.effectTick)/v.effectTime >= 1 then
                ...
                if index == "marijuana" then
                    triggerServerEvent("onPlayerCustomCommand", localPlayer, "me", "Śmieje się bez powodu")
