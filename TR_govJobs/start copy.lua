...
function GovJobsStart:render()
    ...
    dxDrawText("Praca urzędowa", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    dxDrawText("Aby rozpocząć pracę w tym miejscu należy zatrudnić się w urzędzie. Pamiętaj, że należy przynajmniej raz na 24h pojawić się w pracy aby nie stracić swojej posady.", guiInfo.x + 20/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + 110/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
    dxDrawText("Po zakończonej służbie należy zakończyć pracę, aby móc przystąpić do pracy dorywczej.", guiInfo.x + 20/zoom, guiInfo.y + 120/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(170, 60, 60, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "top", true, true)
end
...
function GovJobsStart:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.start then
        ...
        if self.jobID ~= self.currentJob and not isAdmin then exports.TR_noti:create("Nie możesz rozpocząc pracy w tym miejscu, ponieważ nie jesteś tutaj zatrudniony.", "error") return end
