...
function GovJobs:render()
    ...
    dxDrawText("Praca urzędowa", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
...
function GovJobs:renderList()
    ...
    for i, v in pairs(self.jobs) do
        if i == self.selectedJob then
            dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 400/zoom, 40/zoom, tocolor(44,181,233, 200 * self.alpha))
        elseif self:isMouseInPosition(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 400/zoom, 40/zoom) then
            dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 400/zoom, 40/zoom, tocolor(34,36,43, 255 * self.alpha))
        else
            dxDrawRectangle(guiInfo.x, guiInfo.y + 50/zoom + 40/zoom * (i-1), 400/zoom, 40/zoom, tocolor(42,45,51, 255 * self.alpha))
        end
...
function GovJobs:renderJobData()
    ...
    dxDrawText(string.format("Miejsca: %d/%d", self.selectedData.placesTaken, self.selectedData.placesAvaliable), guiInfo.x + 50/zoom, guiInfo.y + 220/zoom, guiInfo.x + guiInfo.w - 50/zoom, guiInfo.y + 190/zoom, tocolor(212, 175, 55, 255 * self.alpha), 1/zoom, self.fonts.list, "center", "top")

    dxDrawText("Informacje:", guiInfo.x + 50/zoom, guiInfo.y + 255/zoom, guiInfo.x + guiInfo.w - 50/zoom, guiInfo.y + 190/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "top", false, false, false, true)
    dxDrawText(string.format("Lokalizacja: #cccccc%s", self.selectedData.place), guiInfo.x + 50/zoom, guiInfo.y + 280/zoom, guiInfo.x + guiInfo.w - 50/zoom, guiInfo.y + 190/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.list, "center", "top", false, false, false, true)
    dxDrawText(string.format("Średnie zarobki: #cccccc$%s", tostring(self.selectedData.payment)), guiInfo.x + 50/zoom, guiInfo.y + 300/zoom, guiInfo.x + guiInfo.w - 50/zoom, guiInfo.y + 190/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.list, "center", "top", false, false, false, true)
    dxDrawText(string.format("Wymagania: #cccccc%s", self.selectedData.requirements or "Brak wymagań"), guiInfo.x + 50/zoom, guiInfo.y + 320/zoom, guiInfo.x + guiInfo.w - 50/zoom, guiInfo.y + 190/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.list, "center", "top", false, false, false, true)
...
function GovJobs:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.accept and self.selectedData then
        if self.currentJob and self.currentJob ~= self.selectedData.ID then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ jesteś już gdzieś zatrudniony.", "error") return end
        if self.selectedData.requirements then
            ...
            elseif string.find(self.selectedData.requirements, "Licencja Lotnicza") then
...
function GovJobs:selectJob(...)
    if not arg[1] then
        ...
        exports.TR_dx:setButtonText(self.buttons.exit, "Zamknij")
        return
    end
    ...
    if self.currentJob == self.selectedData.ID then
        exports.TR_dx:setButtonText(self.buttons.accept, "Zwolnij się")
    else
        exports.TR_dx:setButtonText(self.buttons.accept, "Zatrudnij się")
    end
    exports.TR_dx:setButtonText(self.buttons.exit, "Wróć")
end
...
function GovJobs:reponse(...)
  ...
  if arg[1] == "get" then
    if arg[2] > 0 then
        ...
        exports.TR_noti:create("Pomyślnie zatrudniłeś się w pracy.", "success")
    else
        exports.TR_noti:create("Nie udało się zatrudnić w pracy, ponieważ wszystkie miejsca są już zajęte.", "error")
    end
  ...
  elseif arg[1] == "release" then
    ...
    self:selectJob(self.selectedJob)
    self.selectedData.placesTaken = self.selectedData.placesTaken - 1
    exports.TR_noti:create("Pomyślnie zwolniłeś się z pracy.", "success")
  end
end
