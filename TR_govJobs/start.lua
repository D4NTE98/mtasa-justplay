...
function GovJobsStart:constructor(...)
    ...
    self.buttons = {}
    self.buttons.exit = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, 235/zoom, 40/zoom, "Zamknij", "grey")
    self.buttons.start = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h - 50/zoom, 235/zoom, 40/zoom, jobID == self.jobID and "Zakończ pracę" or "Rozpocznij pracę", "blue")
...
function GovJobsStart:render()
    ...
    dxDrawText(self.jobName, guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    dxDrawText("Opis pracy:", guiInfo.x + 20/zoom, guiInfo.y + 190/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", true, true, false, true)
    ...
    dxDrawText("Wynagrodzenie:", guiInfo.x + 20/zoom, guiInfo.y + 290/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", true, true, false, true)
    ...
    dxDrawText("Wymagania:", guiInfo.x + 20/zoom, guiInfo.y + 350/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", true, true, false, true)
    ...
    dxDrawText("Ilość miejsc:", guiInfo.x + 20/zoom, guiInfo.y + 410/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.title, "center", "top", true, true, false, true)
    ...
    elseif arg[1] == self.buttons.start then
        if self.tickStart then
            if (getTickCount() - self.tickStart)/2000 <= 1 then
                exports.TR_noti:create("Przed zmianą statusu musisz odczekać 2 sekundy.", "error")
                return
            end
        end
        ...
        if not job then
            ...
            if (self.freeSlots and self.freeSlots or 0) >= self.slots then
                exports.TR_noti:create("Nie możesz rozpocząć pracy, ponieważ wszystkie miejsca są już zajęte.", "error")
                return
            end
...
function GovJobsStart:reponse(...)
  ...
  if arg[1] == "start" then
    ...
    exports.TR_noti:create("Pomyślnie rozpocząłeś pracę.", "success")

    exports.TR_dx:setButtonText(self.buttons.start, "Zakończ pracę")
  ...
  elseif arg[1] == "end" then
    ...
    exports.TR_dx:setButtonText(self.buttons.start, "Rozpocznij pracę")
