...
function createNPCs()
    ...
    exports.TR_npc:addDialogueText(dialogue, "Dzień dobry. Ile za wypożyczenie kutra?", {pedResponse = string.format("Koszt wypożyczenia takiego kutra na godzinę to $%d.", settings.rentPrice)})
    exports.TR_npc:addDialogueText(dialogue, "Super cena. Biorę!", {pedResponse = "", responseTo = "Dzień dobry. Ile za wypożyczenie kutra?", trigger = "rentFishingBoat"})
    exports.TR_npc:addDialogueText(dialogue, "Może jednak innym razem.", {pedResponse = "To do następnego.", responseTo = "Dzień dobry. Ile za wypożyczenie kutra?"})
    exports.TR_npc:addDialogueText(dialogue, "Do widzenia.", {pedResponse = "Naaaarrrrazie."})
...
function rentFishingBoat(ped)
    ...
    if not plrData.licence then triggerClientEvent(client, "showCustomMessage", resourceRoot, pedName, "Nie posiadasz licencji żeglarskiej. Nie mogę ci wypożyczyć kutra bez takowiej licencji.", "files/images/npc.png") return end
    if not plrData.licence["BOAT"] then triggerClientEvent(client, "showCustomMessage", resourceRoot, pedName, "Nie posiadasz licencji żeglarskiej. Nie mogę ci wypożyczyć kutra bez takowiej licencji.", "files/images/npc.png") return end
...
function resourceStop()
    for i, v in pairs(getElementsByType("vehicle", resourceRoot)) do
        ...
        if isElement(owner) then
            ...
            exports.TR_noti:create(owner, "Pieniądze za kuter zostały doliczone do twojego konta.", "success")
        end
