...
function DeathMatch:constructor()
    ...
    self.christmasZone = createColSphere(-2082.3974609375, 258.189453125, 35.7947, 10)
...
function DeathMatch:canFight()
    ...
    if isElementWithinColShape(localPlayer, self.christmasZone) then self.inZone = true return true end
    self.inZone = false
    return false
end
...
setTimer(function()
    if self and not self.inZone then
        blockUpdate(true)
...
setTimer(function()
    ...
    if getElementData(localPlayer, 'characterUID') == 10 then 
        toggleControl("fire", true)
        toggleControl("action", true)
        toggleControl("aim_weapon", true)
    end 
