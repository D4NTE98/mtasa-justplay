...
function changePlayerPassword(password, newPassword)
    ...
    if passwordCheck and passwordCheck[1] then
        if (passwordCheck[1]["password"] == teaEncode(password, 'keY%@')) then
        --if passwordVerify(password, passwordCheck[1]["password"]) then
            exports.TR_mysql:querry("UPDATE `tr_accounts` SET password = ? WHERE `UID` = ? LIMIT 1", teaEncode(password, 'keY%@'), uid)
            ...
            triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Hasło zostało pomyślnie zmienione.", "success", {serial = getPlayerSerial(client), ip = getPlayerIP(client), type = "password", text = "Hasło zostało pomyślnie zmienione."})
        else
            triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Aktualne hasło jest niepoprawne.", "error")
        end
    else
        triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Nie udało się zmienić hasła.", "error")
    end
end
...
function changePlayerEmail(email, newEmail)
    ...
    if emailCheck and emailCheck[1] then
        if email == emailCheck[1]["email"] then
            ...
            triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Adres email został pomyślnie zmieniony.", "success", {serial = getPlayerSerial(client), ip = getPlayerIP(client), type = "email", text = "Adres email został pomyślnie zmieniony."})
        else
            triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Aktualny adres email jest niepoprawny.", "error")
        end
    else
        triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Nie udało się zmienić adresu email.", "error")
    end
end
...
function useDashboardReferenceCode(reference)
    ...
    if reference then
        ...
        if tonumber(referenceUID) == nil or string.len(referenceUID) < 1 then
            exports.TR_noti:create(client, "Kod jest nieprawidłowy.", "error")
            return
        end
        if referenceUID == uid then
            exports.TR_noti:create(client, "Nie możesz skorzystać z własnego kodu.", "error")
            return
        end
        ...
        exports.TR_noti:create(client, "Kod został pomyślnie wykorzystany.", "success")
    ...
    if target and target[1] then
        ...
        if send and send[1] then
            if send[1].friendsFor then
                triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Jesteście już znajomymi.", "info")
            else
                triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Ten gracz został już przez ciebie zaproszony.", "info")
            end
            return
        end
        ...
        if recieved and recieved[1] then
            if recieved[1].friendsFor then
                ...
                triggerClientEvent(client, "setDashboardResponse", resourceRoot, "Masz już oczekujące zaproszenie od tego gracza.", "info")
            end
            return
        end
        ...
        loadPlayerFriends(client)
        triggerClientEvent(client, "setDashboardResponse", resourceRoot, string.format("Gracz %s został pomyślnie zaproszony do znajomych.", name), "success")

        local target = getPlayerFromName(name)
        if isElement(target) then
            if getElementData(target, "characterUID") then
                exports.TR_noti:create(target, string.format("Gracz %s wysłał ci zaproszenie do znajomych.", getPlayerName(client)), "info")
            end
        end
    else
...
function acceptPlayerFriend(targetUID, username)
    ...
    loadPlayerFriends(client)
    triggerClientEvent(client, "setDashboardResponse", resourceRoot, string.format("Gracz %s został dodany do znajomych.", username), "success")
end
...
function removePlayerFriend(targetUID, username, canceled)
    ...
    if not canceled then
        triggerClientEvent(client, "setDashboardResponse", resourceRoot, string.format("Gracz %s został usunięty ze znajomych.", username), "success")
    else
        triggerClientEvent(client, "setDashboardResponse", resourceRoot, string.format("Gracz %s został usunięty z listy.", username), "success")
    end
end
