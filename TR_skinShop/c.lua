...
function SkinShop:constructor(...)
    exports.TR_dx:showLoading(3000, "Wczytywanie sklepu")
...
function SkinShop:open()
    ...
    self.buttons = {}
    self.buttons.buy = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 210/zoom, guiInfo.y + guiInfo.h - 50/zoom, 200/zoom, 40/zoom, "Zakup skin", "blue")
    self.buttons.exit = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, 200/zoom, 40/zoom, "Zamknij sklep")
...
function SkinShop:render()
    ...
    self:drawBackground(guiInfo.x, guiInfo.y, guiInfo.w, guiInfo.h, tocolor(16,18,23, 255), 11)
    ...
    dxDrawText("ID Skina: #2cb5e9"..self.skinModel, guiInfo.x + 10/zoom, guiInfo.y + 15/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 10/zoom, tocolor(255, 255, 255, 255), 1/zoom, self.font, "center", "top", false, false, false, true)
    dxDrawText(string.format("Cena: #2cb5e9$%.2f", self.skinPrice), guiInfo.x + 10/zoom, guiInfo.y + 40/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 10/zoom, tocolor(255, 255, 255, 255), 1/zoom, self.font, "center", "top", false, false, false, true)
end
...
function SkinShop:response(...)
    ...
    if arg[1] then exports.TR_noti:create("Skin został pomyślnie zakupiony i dodany do ekwipunku.", "success") end
end
...
function SkinShop:canBuySkin()
    ...
    if self.shopName == "kc" then
        ...
        exports.TR_noti:create("Aby zakupić ten skin potrzebujesz konta Gold lub Diamond.", "error")
        return false

    elseif self.shopName == "gnocchi" then
        if plrData.premium == "diamond" then return true end
        exports.TR_noti:create("Aby zakupić ten skin potrzebujesz konta Diamond.", "error")
        return false
    end
    return true
end
