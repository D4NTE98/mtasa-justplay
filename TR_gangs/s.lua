...
function retakeZone(zoneID, ownedGang, color)
    ...
    retakeZones[zoneID] = {
        ...
        timer = setTimer(updateZoneTime, 60000, 32, zoneID),
        points = 0,
        times = 0,
    }
...
function updateZoneTime(zoneID)
    ...
    if retakeZones[zoneID].points >= 30 then
        ...
        exports.TR_mysql:querry("UPDATE `tr_organizations` SET money=money+? WHERE ID = ? LIMIT 1", 5000, retakeZones[zoneID].newData.ownedGang)
        exports.TR_mysql:querry("UPDATE `tr_gangZones` SET `ownedGang`= ?, `protectTime` = DATE_ADD(NOW(), INTERVAL 6 HOUR) WHERE ID = ? LIMIT 1", retakeZones[zoneID].newData.ownedGang, zoneID)

        retakeZones[zoneID] = nil
        updateZones()
        points = nil

    elseif retakeZones[zoneID].times >= 30 then
        if isTimer(retakeZones[zoneID].timer) then killTimer(retakeZones[zoneID].timer) end
...
function useSpayItem(zoneID, itemID)
    ...
    local s = exports.TR_mysql:querry('select * from tr_organizations where ID = ? LIMIT 1', getElementData(client, "characterOrgID"))
    if s and #s > 0 then
        if s[1].verifedRP ~= 1 then
            exports.TR_noti:create(client, "Twój gang nie został zweryfikowany pod wzgledem RP.", "error")
            return
        end
    end

    if retakeZones[zoneID] then
        exports.TR_noti:create(client, "Ta strefa jest w tym momencie przejmowana.", "error")
    ...
    if currentData then
        if currentData.ownedGang == getElementData(client, "characterOrgID") then
            exports.TR_noti:create(client, "Ta strefa należy już do twojego gangu.", "error")
            triggerClientEvent(client, "equipmentResponse", resourceRoot)
            return
        end
    end
    ...
    if canTakeZone and canTakeZone[1] then
        ...
        exports.TR_noti:create(client, "Aby przejąć dzielnicę utrzymuj się na jej terenie przez 30 minut. Teren strefy oznaczony jest kolorem białym.", "success", 10)
    else
        triggerClientEvent(client, "equipmentResponse", resourceRoot)
        exports.TR_noti:create(client, "Ta strefa jest świeżo przejęta i posiada na sobie ochronę.", "error")
        return
    end
end
...
function switchCorners()
    ...
    for i, v in pairs(createdZones) do
        ...
        if isElementWithinColShape(source, col) then
            ...
            if not data then exports.TR_noti:create(source, "Nie możesz rozpocząć sprzedaży, ponieważ nie znajdujesz się na swoim terenie.", "error", 7) return end
            if data.ownedGang ~= orgID then exports.TR_noti:create(source, "Nie możesz rozpocząć sprzedaży, ponieważ nie znajdujesz się na swoim terenie.", "error", 7) return end
            isInZone = data.ownedGang
            zoneID = getElementData(v, "zoneID")
            break
        end
    end
    if not isInZone then exports.TR_noti:create(source, "Nie możesz rozpocząć sprzedaży, ponieważ nie znajdujesz się na swoim terenie.", "error", 7) return end
    ...
    if #getTodayCorners >= 1 then
        exports.TR_noti:create(source, "Nie możesz rozpocząć sprzedaży, ponieważ już sprzedawałeś w tej dzielnicy.", "error", 7)
        return
    end
...
-- For now
local messages = {
    "Ty co najwyżej to możesz w łeb dostać. Spadaj leszczu!",
    "Nie ma opcji, że coś ci sprzedam.",
    "A nie przekopać cię czasem? Spierdalaj mi stąd.",
    "Człowieku ty wiesz z kim ty rozmawiasz? Wydupiaj w podskokach zanim tobą przecioram podłogę.",
}
...
exports.TR_npc:addDialogueText(dialogueDealer, "Chce kupić broń.", {pedResponse = "", trigger = "onGangStoreOpen", triggerData = "weapon"})
exports.TR_npc:addDialogueText(dialogueDealer, "Chce kupić lekkie dragi.", {pedResponse = "", trigger = "onGangStoreOpen", triggerData = "drugsLow"})
exports.TR_npc:addDialogueText(dialogueDealer, "Chce kupić ciężkie dragi.", {pedResponse = "", trigger = "onGangStoreOpen", triggerData = "drugsHigh"})
exports.TR_npc:addDialogueText(dialogueDealer, "Chce kupić kominiarkę.", {pedResponse = "", trigger = "onGangStoreOpen", triggerData = "mask"})
exports.TR_npc:addDialogueText(dialogueDealer, "Chce kupić spray.", {pedResponse = "", trigger = "onGangStoreOpen", triggerData = "spray"})

exports.TR_npc:addDialogueText(dialogueDealer, "Nara.", {pedResponse = "Nara."})
...
function drugStore(npc, type)
    ...
    if type == "weapon" then
        local canBuyWeapon = exports.TR_mysql:querry("SELECT ID FROM tr_organizations WHERE ID = ? AND orgType = ? LIMIT 1", orgID, "gang")
