...
function SlotMachine:constructor()
    ...
    self.screenText = "Naciśnij START aby rozpocząć grę."
    ...
    guiInfo.betIcon = dxGetTextWidth("Zakład: "..self.playBet, 1/zoom, self.fonts.bet)/2
    return true
end
...
function SlotMachine:render()
    ...
    dxDrawText("Zakład: "..self.playBet, guiInfo.x + 162/zoom, guiInfo.y + 495/zoom, guiInfo.x + guiInfo.w - 168/zoom, guiInfo.y + 500/zoom, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.bet, "center", "top")
...
function SlotMachine:drawLegend()
    ...
    dxDrawText("Legenda symboli:", 20/zoom, sy - 412/zoom, guiInfo.x + guiInfo.w - 168/zoom, sy - 427/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.legend, "left", "bottom")
    ...
    for i, v in pairs(guiInfo.iconLegend) do
        ...
        dxDrawText("Mnożnik: x"..v.multiplayer, 90/zoom, sy - 417/zoom + 61/zoom * (i-1), guiInfo.x + guiInfo.w - 168/zoom, sy - 422/zoom + 61/zoom * i, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.multiplayer, "left", "bottom")
    end
end
...
function SlotMachine:onClick(...)
    ...
    if self:isMouseInPosition(guiInfo.x + guiInfo.start.x, guiInfo.y + guiInfo.start.y, guiInfo.start.w, guiInfo.start.h) then
        ...
        if chips < self.playBet then exports.TR_noti:create("Nie posiadasz wystarczającej liczby żetonów. Możesz je zakupić w wyznaczonym miejscu lub u krupiera bądź barmana.", "error", 8) return end
...
function SlotMachine:startGame()
    ...
    self.playing = true
    self.screenText = "Trwa losowanie..."
...
function SlotMachine:submitGame(symbol)
    if self.rolls[1].symbol == self.rolls[2].symbol and self.rolls[1].symbol == self.rolls[3].symbol then
        ...
        self.screenText = string.format("WYGRANA! Wygrałeś %d żetonów!", self.toWin)
        ...
        self.screenText = "Niestety nie udało się wygrać. Spróbuj jeszcze raz."
    end
...
function createSlotMachines()
    for i, v in pairs(SlotMachines) do
        ...
        setElementData(marker, "markerData", {
            ...
            desc = "Wejdź w marker aby rozpocząć grę.",
        }, false)
