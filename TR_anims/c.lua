...
function Animations:render()
	...
	dxDrawText("Dostępne animacje", guiInfo.x + 10/zoom, guiInfo.y + 10/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 30/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, guiInfo.fonts.title, "center", "top")
...
function Animations:drawAnimations()
	...
	if self.selectedCategory == "Styl chodzenia" then
	...
	if #animations[self.selectedCategory] > guiInfo.visibleCount - 1 then
        ...
        dxDrawRectangle(guiInfo.x + 200/zoom, guiInfo.y + 50/zoom, 3/zoom, 445/zoom, tocolor(34,36,43, 255 * self.alpha))
        dxDrawRectangle(guiInfo.x + 200/zoom, guiInfo.y + 50/zoom + barY, 3/zoom, barHeight, tocolor(42,45,51, 255 * self.alpha))
    else
        dxDrawRectangle(guiInfo.x + 200/zoom, guiInfo.y + 50/zoom, 3/zoom, 445/zoom, tocolor(42,45,51, 255 * self.alpha))
    end
	dxDrawText("Aby dodać lub usunąć animację z ulubionych naciśnij prawym przyciskiem myszy na jej nazwę.", guiInfo.x, guiInfo.y + guiInfo.h - 30/zoom, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h, tocolor(170, 170, 170, 150 * self.alpha), 1/zoom, guiInfo.fonts.info, "center", "center")
end
...
function Animations:addToFavourite(animation, forceWalk, save)
	...
	if self.selectedCategory == "Styl chodzenia" or forceWalk then
		animations["Ulubione"][#animations["Ulubione"]].isWalkingStyle = true
	end

	if save then
		self:saveFavourites()
		exports.TR_noti:create("Animacja została dodana z ulubionych.", "info")
	end
end
...
function Animations:removeFromFavourite(animation)
	...
	exports.TR_noti:create("Animacja została usunięta z ulubionych.", "info")
end
...
function Animations:loadFavourites()
	...
	for anim, _ in pairs(animations) do
		for i, v in pairs(animations[anim]) do
			if saved[v[1]] then
				self:addToFavourite(v, anim == "Styl chodzenia")
			end
		end
	end
end
...
function Animations:startAnim(animation)
	...
	if self.selectedCategory == "Styl chodzenia" or animation.isWalkingStyle then
		...
		exports.TR_noti:create("Styl chodzenia został zmieniony.", "animate")
		return
	else
	...
	exports.TR_noti:create("Aby zatrzymać animację naciśnij ENTER.", "animate")
end
...
local motorcycles = {
	[581] = true, [509] = true, [481] = true, [462] = true, [521] = true, [463] = true, [510] = true, [522] = true, [461] = true, [448] = true, [468] = true, [586] = true
}
...
function Animations:canUseAnimation(animation)
	...
	if getElementData(localPlayer, "blockAnim") then exports.TR_noti:create("Nie możesz w tym momencie zmienić animacji.", "error") return false end
	if isElementInWater(localPlayer) then exports.TR_noti:create("Nie możesz użyć animacji w wodzie.", "error") return false end

	if self.animDelay then
		if (getTickCount() - self.animDelay)/5000 < 1 then
			exports.TR_noti:create("Musisz odczekać 3 sekundy przed kolejną zmianą animacji.", "error")
			return false
		end
	end
	...
	if animation.type then
		if animation.type == "gold" then
			if plrData.premium ~= "gold" and plrData.premium ~= "diamond" then
				exports.TR_noti:create("Ta animacja jest dostępna tylko dla graczy z aktywnym statusem Gold lub Diamond.", "error")
				return false
			end

		elseif animation.type == "diamond" then
			if plrData.premium ~= "diamond" then
				exports.TR_noti:create("Ta animacja jest dostępna tylko dla graczy z aktywnym statusem Diamond.", "error")
				return false
			end
		end
	end
	...
	if getPedOccupiedVehicle(localPlayer) and self.selectedCategory ~= "Samochód" then
		exports.TR_noti:create("Nie możesz użyć tej animacji w pojeździe.", "error")
		return false
	elseif getPedOccupiedVehicle(localPlayer) and self.selectedCategory == "Samochód" and motorcycles[getElementModel(getPedOccupiedVehicle(localPlayer))] then
		exports.TR_noti:create("Nie możesz użyć animacji w tym pojeździe", "error")
		return false
	elseif self.selectedCategory == "Samochód" and not getPedOccupiedVehicle(localPlayer) then
		exports.TR_noti:create("Tę animację możesz użyć tylko w pojeździe.", "error")
		return false
	end
