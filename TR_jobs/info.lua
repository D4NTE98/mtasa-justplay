...
function Info:render()
    ...
    dxDrawText(self.jobName, guiInfo.x + 10/zoom, guiInfo.y, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + 25/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    ...
    dxDrawText("Czas w pracy: "..self:getTimeInSeconds(self.totalTime), guiInfo.x + 10/zoom, guiInfo.y + 25/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 2/zoom, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom")
...
function Info:renderTarget()
    ...
    if screenX and screenY then
        ...
        dxDrawImage(screenX - 16/zoom * scale, screenY - 16/zoom * scale, 32/zoom * scale, 32/zoom * scale, "files/images/target.png", 0, 0, 0, tocolor(44,181,233, 255))
...
function Info:checkDistance()
    ...
    if getDistanceBetweenPoints2D(Vector2(getElementPosition(localPlayer)), distance.markerPos) > distance.limit then
        exports.TR_noti:create("Praca została zakończona, ponieważ oddaliłeś się zbyt daleko.", "info")
        setPlayerJob(nil)
