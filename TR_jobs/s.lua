...
function giveJobPayment(amount, isTime, paymentType, jobID, blockPoints)
    ...
    if isTime then
        exports.TR_noti:create(client, string.format("Za przepracowanie 10min zarobiłeś $%.2f.", payment), "money")
    else
        if pointsAdd then
            exports.TR_noti:create(client, string.format("Za wykonaną pracę zarobiłeś $%.2f oraz %d pkt doświadczenia.", payment, pointsAdd), "money")
        else
            exports.TR_noti:create(client, string.format("Za wykonaną pracę zarobiłeś $%.2f.", payment), "money")
        end
    end
    if ticket then exports.TR_noti:create(client, string.format("Wypłata została pomniejszona o kwotę $%.2f, ze względu na nieopłacone mandaty. Do opłacenia mandatów pozostało $%.2f.", ticket, ticketFull), "info", 5) end
...
addCommandHandler("zarobki", function(plr, cmd, value)
    ...
    if not val then exports.TR_noti:create(plr, string.format("Aktualny mnożnik zarobku wynosi %.1f", settings.paymentBonus), "info") return end

    settings.paymentBonus = val
    exports.TR_noti:create(plr, string.format("Mnożnik zaeobku został ustawiony na %.1f", settings.paymentBonus), "info")
end)
...
function joinJobRoom(jobID, roomOwner, maxPlayers)
    if not settings.jobRooms[jobID] then
        ...
        exports.TR_noti:create(client, "Taki pokój nie istnieje.", "error")
        return
    end
    ...
    for i, v in pairs(settings.jobRooms[jobID]) do
        if roomOwner == v.owner then
            if #v.players >= maxPlayers then
                triggerClientEvent(client, "responseJobWindow", resourceRoot, true)
                exports.TR_noti:create(client, "Ten pokój jest pełny.", "error")
                return
            end
