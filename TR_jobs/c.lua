...
function Jobs:constructor(jobID, playerJobData, playerLicenceBlocked)
    ...
    self.buttons = {}
    self.buttons.close = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Zamknij")
    self.buttons.goBack = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Wróć")

    self.buttons.createRoom = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Stwórz pokój")
    self.buttons.setReady = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Gotowy")
    self.buttons.startJob = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, self.jobID == self.currentJob and "Zakończ pracę" or "Rozpocznij pracę")

    -- self.buttons.accept = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 100/zoom, 254/zoom, 40/zoom, "Zakup ulepszenie", "green")
    -- self.buttons.decline = exports.TR_dx:createButton((sx - 250/zoom)/2, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Anuluj", "red")
    self.buttons.accept = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Zakup ulepszenie", "blue")
    self.buttons.decline = exports.TR_dx:createButton(guiInfo.x + 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, 254/zoom, 40/zoom, "Anuluj", "grey")
    exports.TR_dx:setButtonVisible(self.buttons, false)

    self.edits = {}
    self.edits.text = exports.TR_dx:createEdit(guiInfo.x + 20/zoom, guiInfo.y + guiInfo.h - 50/zoom, guiInfo.w - 140/zoom, 40/zoom, "Wpisz wiadomość")
...
function Jobs:getJobDetails()
    ...
    self.desc = details.desc or "Brak opisu"
    self.req = details.require or "Brak wymagań"
...
function Jobs:render()
    ...
    if self.selectedTab == "info" then
        dxDrawText(self.jobName, guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

        dxDrawText("Opis pracy:", guiInfo.x + 20/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 20/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "top")
        ...
        dxDrawText("Wymagania:", guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + 198/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "top")
        ...
        dxDrawText("Wynagrodzenie:", guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + 256/zoom, guiInfo.x + guiInfo.w - 10/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.info, "left", "bottom")
        ...
        if self:isMouseInPosition(guiInfo.x + guiInfo.w - 128/zoom, guiInfo.y + guiInfo.h - 80/zoom, 18/zoom, 18/zoom) then
            ...
            self:drawBackground(x, y, width, height, tocolor(23,25,31, 255 * self.alpha), 11, true)
            dxDrawText("Podana kwota jest PEWNĄ KWOTĄ ZAROBKOWĄ przez okres 1h podczas nieposiadania żadnych ulepszeń. Jeśli po kupnie jakiegoś ulepszenia punkty będą dawać mniej - tylko Ci się tak wydaje. W ogólnym rozrachunku i tak zarobisz więcej!", x + 10/zoom, y + 10/zoom, x + width - 10/zoom, y + height - 10/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "bottom", true, true, true)
        ...
        if self.img then
            dxDrawRectangle(guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + 50/zoom, 254/zoom, 134/zoom, tocolor(34,36,43, 255 * self.alpha))
            ...
            dxDrawRectangle(guiInfo.x + guiInfo.w - 364/zoom, guiInfo.y + 50/zoom, 254/zoom, 134/zoom, tocolor(34,36,43, 255 * self.alpha))
            dxDrawRectangle(guiInfo.x + guiInfo.w - 362/zoom, guiInfo.y + 52/zoom, 250/zoom, 130/zoom, tocolor(16,18,23, 255 * self.alpha))
            dxDrawText("Tu powinno być zdjęcie, lecz budżet pracodwacy na to nie zezwalał.", guiInfo.x + guiInfo.w - 354/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 120/zoom, guiInfo.y + 180/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "center", true, true)
        end

    elseif self.selectedTab == "rank" and self.loadingData then
        dxDrawText("Najlepsi gracze", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
        ...
        dxDrawText("Trwa wczytywanie rankingu...", guiInfo.stand.x, guiInfo.y + (guiInfo.h + 40/zoom)/2, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top")
    ...
    elseif self.selectedTab == "rank" and not self.loadingData then
        ...
        dxDrawText(self.ranking[1] or "Brak", guiInfo.stand.x + 182/zoom, guiInfo.y + 30/zoom, guiInfo.stand.x + 367/zoom, guiInfo.stand.y - 5/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom", true, true)
        dxDrawText(self.ranking[2] or "Brak", guiInfo.stand.x + 367/zoom, guiInfo.y + 50/zoom, guiInfo.stand.x + 547/zoom, guiInfo.stand.y + 25/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom", true, true)
        dxDrawText(self.ranking[3] or "Brak", guiInfo.stand.x + 2/zoom, guiInfo.y + 50/zoom, guiInfo.stand.x + 182/zoom, guiInfo.stand.y + 55/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom", true, true)
        ...
        for i = 1, 4 do
            ...
            if v then
                dxDrawRectangle(x, guiInfo.stand.y + guiInfo.stand.h + 15/zoom + (row-1) * 40/zoom, guiInfo.stand.w/2 - 5/zoom, 30/zoom, tocolor(42,45,51, 255 * self.alpha))
                ...
                dxDrawRectangle(x, guiInfo.stand.y + guiInfo.stand.h + 15/zoom + (row-1) * 40/zoom, guiInfo.stand.w/2 - 5/zoom, 30/zoom, tocolor(42,45,51, 255 * self.alpha))
                dxDrawText(string.format("%d. Brak", i + 3), x + 5/zoom, guiInfo.stand.y + guiInfo.stand.h + 15/zoom + (row-1) * 40/zoom, x + guiInfo.stand.w/2 - 5/zoom, guiInfo.stand.y + guiInfo.stand.h + 45/zoom + (row-1) * 40/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.parts, "left", "center")
            end
        ...
        dxDrawText(string.format("Punkty w rankingu: %d", self.playerJobData.totalPoints), guiInfo.stand.x, guiInfo.stand.y, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.parts, "left", "bottom")

        if self:isMouseInPosition(guiInfo.stand.x + guiInfo.stand.w - 160/zoom, guiInfo.y + guiInfo.h - 37/zoom, 162/zoom, 24/zoom) then
            dxDrawText("Odbierz nagrody", guiInfo.stand.x, guiInfo.stand.y, guiInfo.stand.x + guiInfo.stand.w - 28/zoom, guiInfo.y + guiInfo.h - 15/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.parts, "right", "bottom")
            ...
            dxDrawText("Odbierz nagrody", guiInfo.stand.x, guiInfo.stand.y, guiInfo.stand.x + guiInfo.stand.w - 28/zoom, guiInfo.y + guiInfo.h - 15/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.parts, "right", "bottom")
    ...
    elseif self.selectedTab == "upgrade" then
        dxDrawText("Ulepszenia", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
        ...
        for i, v in pairs(self.avaliableUpgrades) do
            ...
            dxDrawText(v.name..string.format(" (%s)", self.playerJobData.upgrades[i] and "Posiadane" or  tostring(v.price).."pkt"), guiInfo.x + 80/zoom, guiInfo.y + 53/zoom + (i-1) * 70/zoom, 45/zoom, 45/zoom, tocolor(220, 220, 220, alpha * self.alpha), 1/zoom, self.fonts.info, "left", "top")
        ...
        dxDrawText(string.format("Dostępne punkty: %d", self.playerJobData.points), guiInfo.stand.x, guiInfo.stand.y, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "bottom")


    elseif self.selectedTab == "acceptUpgrade" then
        dxDrawText("Potwierdzenie zakupu", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    ...
    elseif self.selectedTab == "getJobPrizes" and self.loadingData then
        dxDrawText("Twoje nagrody", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

        self.loadingDataRot = self.loadingDataRot + 4
        if self.loadingDataRot >= 360 then self.loadingDataRot = self.loadingDataRot - 360 end

        dxDrawImage(guiInfo.x + (guiInfo.w - guiInfo.iconSize - 60/zoom)/2 - 40/zoom/2, guiInfo.y + (guiInfo.h - 40/zoom)/2 - 20/zoom, 40/zoom, 40/zoom, self.textures.loading, self.loadingDataRot, 0, 0, tocolor(120, 120, 120, 255 * self.alpha))
        dxDrawText("Trwa wczytywanie nagród...", guiInfo.stand.x, guiInfo.y + (guiInfo.h + 40/zoom)/2, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top")
    ...
    elseif self.selectedTab == "getJobPrizes" and not self.loadingData then
        ...
        dxDrawText("Nagrody przyznawane są za TOP7 na każdej pracy. Ilość punktów rankingowych podczas wyliczania nagrody zostaje zresetowana do 0 aby przez kolejny miesiąc każdy miał równe szanse na wygraną.", guiInfo.x + 10/zoom, guiInfo.y + 235/zoom, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + guiInfo.h - 60/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.desc, "center", "top", true, true)

    elseif self.selectedTab == "payment" then
        dxDrawText("Wybór płatności", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
        ...
        dxDrawText("W gotówce", sx/2 - 128/zoom - 60/zoom, y + 148/zoom, sx/2 - 68/zoom, guiInfo.y + 50/zoom, tocolor(255, 255, 255, alpha * self.alpha), 1/zoom, self.fonts.main, "center", "top")
        ...
        dxDrawText("Na konto", sx/2 + 60/zoom, y + 148/zoom, sx/2 + 188/zoom, sx/2 + 188/zoom, tocolor(255, 255, 255, alpha * self.alpha), 1/zoom, self.fonts.main, "center", "top")

        dxDrawText("Zarobione pieniądze w pracach będą wypłacane w formie wybranej powyżej.", guiInfo.stand.x, guiInfo.stand.y, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "bottom")

    elseif self.selectedTab == "selectRoom" then
        dxDrawText("Wybierz pokój", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
        ...
        if self.loadingGroupList then
            ...
            dxDrawText("Trwa wczytywanie pokoi...", guiInfo.stand.x, guiInfo.y + (guiInfo.h + 40/zoom)/2, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.tab, "center", "top")
        else
            if self.jobRooms then
                for i = 1, 4 do
                    ...
                    if v then
                        if self:isMouseInPosition(guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 62/zoom * (i-1), guiInfo.w - guiInfo.iconSize - 80/zoom, 60/zoom) then
                            dxDrawRectangle(guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 62/zoom * (i-1), guiInfo.w - guiInfo.iconSize - 80/zoom, 60/zoom, tocolor(42,45,51, 255 * self.alpha))
                        else
                            dxDrawRectangle(guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 62/zoom * (i-1), guiInfo.w - guiInfo.iconSize - 80/zoom, 60/zoom, tocolor(34,36,43, 255 * self.alpha))
                        end

                        dxDrawText(string.format("Pokój gracza %s", v.owner), guiInfo.x + 20/zoom, guiInfo.y + 60/zoom + 62/zoom * (i-1), guiInfo.x + guiInfo.w - 200/zoom, guiInfo.y + 50/zoom + 62/zoom * (i-1), tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.parts, "left", "top")
                        ...
                        dxDrawText(string.format("W pokoju: %s", inRoom), guiInfo.x + 20/zoom, guiInfo.y + 60/zoom + 62/zoom * (i-1), guiInfo.x + guiInfo.w - 200/zoom, guiInfo.y + 100/zoom + 62/zoom * (i-1), tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "bottom")
                    end
                end
                ...
                if #self.jobRooms > 4 then
                    ...
                    dxDrawRectangle(guiInfo.x + guiInfo.w - guiInfo.iconSize - 72/zoom, guiInfo.y + 50/zoom, 4/zoom, 246/zoom, tocolor(34,36,43, 255 * self.alpha))
                    dxDrawRectangle(guiInfo.x + guiInfo.w - guiInfo.iconSize - 72/zoom, guiInfo.y + 50/zoom + self.barY, 4/zoom, self.barHeight, tocolor(23,25,31, 255 * self.alpha))
                end

            else
                dxDrawText("Nie ma jeszcze żadnych pokoi.\nMożesz utworzyć pokój jako pierwszy :)", guiInfo.stand.x, guiInfo.y + guiInfo.h/2, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h/2, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "center")
            end
        end

    elseif self.selectedTab == "group" then
        dxDrawText(string.format("Pokój gracza %s", self.currentGroup.owner), guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
        ...
        for i, v in pairs(self.currentGroup.players) do
            ...
            dxDrawText(v.ready and "GOTOWY DO PRACY" or v.name == self.currentGroup.owner and "CZEKA NA POTWIERDZENIE GOTOWOŚCI" or "NIEGOTOWY DO PRACY", guiInfo.x + 20/zoom, guiInfo.y + 60/zoom + 62/zoom * (i-1), guiInfo.x + guiInfo.w - 200/zoom, guiInfo.y + 100/zoom + 62/zoom * (i-1), tocolor(color[1], color[2], color[3], 255 * self.alpha), 1/zoom, self.fonts.tab, "left", "bottom")
    ...
    elseif self.selectedTab == "chat" then
        dxDrawText("Chat pokoju", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w - 100/zoom, guiInfo.y + 50/zoom, tocolor(44,181,233, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
        ...
        if self.currentGroup.messages then
            ...
            dxDrawText("Nie ma jeszcze żadnych wiadomości.\nMożesz napisać coś jako pierwszy :)", guiInfo.stand.x, guiInfo.y + guiInfo.h/2, guiInfo.stand.x + guiInfo.stand.w, guiInfo.y + guiInfo.h/2, tocolor(120, 120, 120, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "center")
        end
    end

    dxDrawRectangle(guiInfo.x + guiInfo.w - guiInfo.iconSize - 60/zoom, guiInfo.y, 2/zoom, guiInfo.h, tocolor(34,36,43, 170 * self.alpha))
        ...
        elseif self:isMouseInPosition(guiInfo.x, guiInfo.y, guiInfo.w - 100/zoom, guiInfo.h) then
            ...
            elseif self.selectedTab == "payment" then
                if self:isMouseInPosition(sx/2 - 128/zoom - 60/zoom - 15/zoom, guiInfo.y + (guiInfo.h - 128/zoom)/2 - 20/zoom, 158/zoom, 190/zoom) then
                    ...
                    exports.TR_noti:create("Forma wypłaty została ustawiona na gotówkę.", "success")
                ...
                elseif self:isMouseInPosition(sx/2 + 60/zoom - 15/zoom, guiInfo.y + (guiInfo.h - 128/zoom)/2 - 20/zoom, 158/zoom, 190/zoom) then
                    ...
                    exports.TR_noti:create("Forma wypłaty została ustawiona na przelew.", "success")
                end
            ...
            elseif self.selectedTab == "selectRoom" then
                if self.jobRooms then
                    for i = 1, 4 do
                        ...
                        if v then
                            if self:isMouseInPosition(guiInfo.x + 10/zoom, guiInfo.y + 50/zoom + 62/zoom * (i-1), guiInfo.w - guiInfo.iconSize - 80/zoom, 60/zoom) then
                                exports.TR_dx:setResponseEnabled(true, "Trwa dołączanie do pokoju")
...
function Jobs:selectTab(tab)
    ...
    elseif tab == "acceptUpgrade" then
        ...
        exports.TR_dx:setButtonText(self.buttons.accept, "Zakup ulepszenie")
        exports.TR_dx:setButtonText(self.buttons.decline, "Anuluj")

    elseif tab == "getJobPrizes" then
        if self.playerPrizes then
            exports.TR_dx:setButtonVisible({self.buttons.accept, self.buttons.decline}, true)
            exports.TR_dx:setButtonText(self.buttons.accept, "Odbierz nagrody")
            exports.TR_dx:setButtonText(self.buttons.decline, "Anuluj")
        else
    ...
    elseif tab == "selectRoom" then
        exports.TR_dx:setButtonText(self.buttons.goBack, "Wróć")
    ...
    elseif tab == "group" then
        exports.TR_dx:setButtonText(self.buttons.goBack, "Opuść pokój")
        ...
        if self.currentGroup then
            if self.currentGroup.owner == getPlayerName(localPlayer) then
                exports.TR_dx:setButtonText(self.buttons.setReady, "Rozpocznij pracę")
            else
                exports.TR_dx:setButtonText(self.buttons.setReady, "Zgłoś gotowość")
            end
        end
...
function Jobs:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.startJob then
        ...
        elseif not self.currentJob then
            ...
            exports.TR_noti:create("Nie możesz rozpocząc pracy w tym miejscu, ponieważ jesteś już gdzieś zatrudniony.", "error")
        end

    elseif arg[1] == self.buttons.accept then
        if self.selectedTab == "getJobPrizes" then
            if not self.playerPrizes then exports.TR_noti:create("Nie posiadasz żadnej nagrody do wypłacenia.", "error") return end
            if tonumber(self.playerPrizes) == nil then exports.TR_noti:create("Nie posiadasz żadnej nagrody do wypłacenia.", "error") return end
            if tonumber(self.playerPrizes) < 1 then exports.TR_noti:create("Nie posiadasz żadnej nagrody do wypłacenia.", "error") return end
            ...
            if self.currentJob then
                exports.TR_noti:create("Nie możesz zakupić ulepszenia w trakcie pracy.", "error")
                return
            end
            if self.selectedUpgrade.price > self.playerJobData.points then
                exports.TR_noti:create("Nie masz wystarczającej liczby punktów aby zakupić to ulepszenie.", "error")
                return
            end
    ...
    elseif arg[1] == self.buttons.setReady then
        ...
        if getPlayerName(localPlayer) == self.currentGroup.owner then
            if self.multipleWork > #self.currentGroup.players then
                exports.TR_noti:create(string.format("W pokoju jest zbyt mało graczy. Aby rozpocząć pracę w pokoju musi się znajdować minimalnie %d graczy.", self.multipleWork), "error", 4)
                return
            end

            for i, v in pairs(self.currentGroup.players) do
                if not v.ready and v.name ~= self.currentGroup.owner then
                    exports.TR_noti:create("Nie wszyscy są gotowi do rozpoczęcia pracy. Poczekaj aż każda osoba zgłosi gotowość.", "error", 4)
                    return
                end
            end
...
function Jobs:startJob()
    if getElementData(localPlayer, "waitingEvent") then
        exports.TR_noti:create("Nie możesz rozpocząć pracy podczas oczekiwania na event, na który jesteś zapisany.", "error", 8)
        return
    end
    
    if self.req then
        if string.find(self.req, "kat. A") then
            if not self.licences["a"] then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ nie spełniasz wszystkich wymagań.", "error") return end
            if self.hasBlockedLicence then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ posiadasz zabrane prawo jazdy.", "error") return end

        elseif string.find(self.req, "kat. B") then
            if not self.licences["b"] then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ nie spełniasz wszystkich wymagań.", "error") return end
            if self.hasBlockedLicence then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ posiadasz zabrane prawo jazdy.", "error") return end

        elseif string.find(self.req, "kat. C") then
            if not self.licences["c"] then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ nie spełniasz wszystkich wymagań.", "error") return end
            if self.hasBlockedLicence then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ posiadasz zabrane prawo jazdy.", "error") return end

        elseif string.find(self.req, "Licencja lotnicza") then
            if not self.licences["fly"] then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ nie spełniasz wszystkich wymagań.", "error") return end
            if self.hasBlockedLicence then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ posiadasz zabrane prawo jazdy.", "error") return end

        elseif string.find(self.req, "Licencja nurka") then
            if not self.licences["WATER"] then exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ nie spełniasz wszystkich wymagań.", "error") return end

        end


        if string.find(self.req, "Doświadczenie:") then
            local text = string.match(self.req, "Doświadczenie: %d+")
            ...
            if tonumber(plrPoints) < pointsNeeded then
                exports.TR_noti:create("Nie możesz zatrudnić się w tej pracy, ponieważ nie spełniasz wszystkich wymagań.", "error", 8)
                return
            end
        end
    end
...
function Jobs:updateGroups(...)
    ...
    if self.selectedRoom then
                        ...
                        if plr.name == plrName then
                            ...
                            if plrName == v.owner then
                                ...
                                if not plr.ready then
                                    ...
                                    exports.TR_dx:setButtonText(self.buttons.setReady, "Zgłoś brak gotowości")
                                end
                            end
                            return
                        end
        ...
        self:selectTab("selectRoom")
        exports.TR_noti:create("Zostałeś wyrzucony lub właściciel wyszedł z pokoju.", "error")
    end
end
...
function Jobs:response(...)
    ...
    self.currentJob = getPlayerJob()
    exports.TR_dx:setButtonText(self.buttons.startJob, self.jobID == self.currentJob and "Zakończ pracę" or "Rozpocznij pracę")
end

function Jobs:responseUpgrade()
    self:selectTab("upgrade")
    exports.TR_dx:setResponseEnabled(false)
    exports.TR_noti:create("Ulepszenie zostało pomyślnie zakupione.", "success")
end

function Jobs:responsePrizes()
    self.playerPrizes = 0
    exports.TR_dx:setResponseEnabled(false)
    exports.TR_noti:create("Nagrody zostały pomyślnie wypłacone.", "success")
end
...
function Jobs:savePayment(payment)
    ...
    if not xml then
        ...
        local node = xmlCreateSubNode(xml, "payment")
...
function openLockerInfo()
    if guiInfo.jobType == "mechanic" then
        createInformation("Warsztat samochodowy", "Udaj się do przebieralni i przebierz się w odpowiedni strój roboczy.")

    elseif guiInfo.jobType == "taxi" then
        createInformation("Taksówkarz", "Przebierz się w strój roboczy.")
    end
end
